// 旧形式ハッシュ（固定ソルト SHA-256 の hex 文字列）を PBKDF2 でラップする。
// ユーザーの平文パスワードなしでオフライン移行できる hash-of-hash 方式。
// 保存フォーマット: `pbkdf2-wrapped-legacy:<iterations>:<base64(salt)>:<base64(hash)>`
//
// 検証時は平文から旧形式ハッシュを再計算し、その hex を PBKDF2 の入力として照合する。
// ログイン時に純正 PBKDF2（toPasswordHash）へ昇格されるまでの中間形式。

import { bytesToBase64, derivePbkdf2 } from "@/domain/auth/to-password-hash"
...
// 旧形式 hex ハッシュを PBKDF2 でラップした保存文字列を生成する。
