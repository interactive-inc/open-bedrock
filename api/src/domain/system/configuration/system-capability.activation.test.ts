...
import { InvalidSystemCapabilityActivationError } from "@system/domain/configuration/invalid-system-capability-activation.error"
import { SystemCapabilityActivation } from "@system/domain/configuration/system-capability.activation"
import { describe, expect, test } from "bun:test"

describe("SystemCapabilityActivation", () => {
  test("共通supersetから有効なcapabilityをcanonical順で確定する", () => {
    const activation = SystemCapabilityActivation.create({
  ...
  test("shape・未知名・重複をfail closedで拒否する", () => {
    ...
    for (const input of invalidInputs) {
      expect(SystemCapabilityActivation.create(input)).toBeInstanceOf(
