...
describe("toStableSystemAuditJson", () => {
  test("serializes primitives, arrays, and nested null without changing their meaning", () => {
    expect(toStableSystemAuditJson([null, true, false, 12.5, "text"])).toBe(
      '[null,true,false,12.5,"text"]',
    )
    expect(toStableSystemAuditJson(null)).toBeNull()
  })
  ...
  test("orders every object by Unicode code point and serializes manually", () => {
    ...
    expect(toStableSystemAuditJson(value)).toBe('{"nested":{"a":2,"token":"[REDACTED]"},"z":1}')
  })

  test("keeps integer-like keys in code point order instead of JSON property order", () => {
    expect(toStableSystemAuditJson({ 2: "two", 10: "ten", 1: "one" })).toBe(
      '{"1":"one","10":"ten","2":"two"}',
    )
  })
  ...
  test("orders BMP keys before astral keys according to Unicode code points", () => {
    ...
    expect(toStableSystemAuditJson({ [astral]: "astral", [bmp]: "bmp", a: "ascii" })).toBe(
  ...
  test("redacts every normalized secret-key variant but preserves token_version", () => {
    ...
    const serialized = requireSerializedJson(toStableSystemAuditJson(value))
  ...
  test("redacts secrets recursively through arrays and objects", () => {
    expect(
      toStableSystemAuditJson({ entries: [{ nested: { RefreshToken: "raw" } }], safe: "visible" }),
  ...
  test("allows null-prototype plain objects", () => {
    ...
    expect(toStableSystemAuditJson(value)).toBe('{"a":2,"z":1}')
  })

  test("allows non-cyclic shared references", () => {
    const shared = { a: 1 }

    expect(toStableSystemAuditJson({ left: shared, right: shared })).toBe(
      '{"left":{"a":1},"right":{"a":1}}',
    )
  })
  ...
  test("does not mutate the source while ordering and redacting", () => {
    ...
    expect(toStableSystemAuditJson(value)).toBe(
  ...
  test("serializes an array Proxy without executing its get trap", () => {
    ...
    expect(toStableSystemAuditJson(value)).toBe('["first",{"safe":true}]')
  ...
  test("validates a redacted shared DAG once without generating its discarded projection", () => {
    ...
    expect(toStableSystemAuditJson({ password: makeSharedDag(98, leaf) })).toBe(
      '{"password":"[REDACTED]"}',
    )
    expect(ownKeysCalls).toBe(1)
  })
  ...
  test("allows exactly 65,536 UTF-8 bytes", () => {
    const serialized = requireSerializedJson(toStableSystemAuditJson({ value: "x".repeat(65_524) }))
  ...
  test("uses UTF-8 bytes, not JavaScript string length, at the multi-byte boundary", () => {
    ...
    const serialized = requireSerializedJson(toStableSystemAuditJson({ value: exactlyAtLimit }))
