...
import { countLocalD1Queries } from "@tests/d1/support/count-local-d1-queries"
...
async function createTestDb(): Promise<TestDb> {
  const counted = countLocalD1Queries(await pool.next())
  const db = counted.database
...
async function seedAuditEvent(
  ...
  await db
    ...
    .bind(
      input.eventId,
      input.requestId,
      // D1はJSの数値をREALで束縛するため、TEXT列のAccount IDは文字列で渡す。
...
async function failSelfAudit(db: D1Database): Promise<void> {
  await execSql(
    db,
    `CREATE TRIGGER fail_audit_self_insert
...
describe("GET /audit-events", () => {
  test("authorizes by live audit:read permission, independently of roles and export", async () => {
    ...
    expect((await request(db, "/company/audit-events", await token(1))).status).toBe(200)
    expect((await request(db, "/company/audit-events", await token(2))).status).toBe(200)
    expect((await request(db, "/company/audit-events", await token(3))).status).toBe(403)
    expect((await request(db, "/company/audit-events", await token(4))).status).toBe(403)
  })
  ...
  test("applies grants and revocations after token issuance", async () => {
    ...
    expect((await request(db, "/company/audit-events", bearer)).status).toBe(403)
    await grantPermission(db, 4, "late-reader", "audit:read")
    expect((await request(db, "/company/audit-events", bearer)).status).toBe(200)
    await db
      .prepare(
  ...
  test("filters custom vocabulary in a half-open range and paginates only with opaque cursors", async () => {
    ...
    const filtered = await request(
      db,
      "/company/audit-events?actor_account_id=-41&action=custom.action&target_type=custom_target&target_id=private-target&outcome=succeeded&from=2026-01-01T00:00:00Z&to=2026-01-02T00:00:00Z",
      bearer,
    )
    ...
    const first = await request(db, "/company/audit-events?limit=1", bearer)
    ...
    const second = await request(
      db,
      `/company/audit-events?limit=1&cursor=${encodeURIComponent(firstBody.next_cursor ?? "")}`,
      bearer,
    )
    ...
    const rebound = await request(
      db,
      `/company/audit-events?limit=2&cursor=${encodeURIComponent(firstBody.next_cursor ?? "")}`,
      bearer,
    )
    ...
    const filterRebound = await request(
      db,
      `/company/audit-events?limit=1&action=custom.changed&cursor=${encodeURIComponent(firstBody.next_cursor ?? "")}`,
      bearer,
    )
  ...
  test("rejects malformed queries before SQL and leaves no self-audit", async () => {
    ...
    const response = await request(state.db, "/company/audit-events?limit=1&limit=1", bearer)

    expect(response.status).toBe(400)
    expect(await response.json()).toMatchObject({ code: "audit_invalid_query" })
    expect(state.queries()).toBe(10)
  ...
  test.each(["", "not-a-version-two-cursor"])(
    ...
    async (cursor) => {
      ...
      const response = await request(
        state.db,
        `/company/audit-events?cursor=${encodeURIComponent(cursor)}`,
        await token(1),
      )
  ...
  test("does not expose internal, JSON, IP or mutable identity fields", async () => {
    const { db } = await createTestDb()
    const response = await request(db, "/company/audit-events?action=custom.action", await token(1))
  ...
  test("appends a bounded searched event after reading and correlates its request ID", async () => {
    ...
    const response = await request(
      db,
      "/company/audit-events?target_id=private-target&limit=1",
      await token(1),
    )
  ...
  test("audits a denied malformed query without validating or copying it", async () => {
    ...
    const response = await request(
      db,
      "/company/audit-events?direction=private-direction&target_id=private-target",
      await token(4),
    )
    const audit = await latestAudit(db)
  ...
  test("prioritizes unavailable when success or denied self-audit cannot append", async () => {
    ...
    const successResponse = await request(success.db, "/company/audit-events", await token(1))
    ...
    const deniedResponse = await request(
      denied.db,
      "/company/audit-events?direction=private",
      await token(4),
    )
  ...
  test("fails closed before query validation when the shared request clock is invalid", async () => {
    const { db } = await createTestDb()
    const authorizedMalformed = await requestWithContext({
      db,
      jwtSecret,
      path: "/company/audit-events?unknown=1",
      token: await token(1),
      now: "not-a-date",
    })
    expect(authorizedMalformed.status).toBe(401)
    expect(await authorizedMalformed.json()).toEqual({
    ...
    expect(await success.json()).toEqual({ error: "account authorization is unavailable" })

    const denied = await requestWithContext({
      db,
      jwtSecret,
      path: "/company/audit-events?unknown=private",
      token: await token(4),
      now: "not-a-date",
    })
    expect(denied.status).toBe(401)
    expect(await denied.json()).toEqual({ error: "account authorization is unavailable" })
  })
  ...
  test("sets no-store on a handled unanticipated 500 before route execution", async () => {
    ...
    const response = await app.request("/company/audit-events", {}, brokenBindings)
  ...
  test("sets no-store on success, errors, rate limiting and CORS preflight", async () => {
    const { db } = await createTestDb()
    const requests = [
      await request(db, "/company/audit-events", await token(1)),
      await request(db, "/company/audit-events?unknown=1", await token(1)),
      await request(db, "/company/audit-events", null),
      await request(db, "/company/audit-events", await token(4)),
    ]
    await failSelfAudit(db)
    requests.push(await request(db, "/company/audit-events", await token(1)))
    ...
    requests.push(await app.request("/company/audit-events", {}, bindings))
    requests.push(
      await app.request(
