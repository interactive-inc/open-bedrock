...
async function seedExternalIdentity(
  ...
  await db.batch([
    db
      ...
      .bind(identityId, String(accountId), subject, nowEpoch * 1_000),
    db
      .prepare(
      ...
      .bind(identityId, email, nowEpoch * 1_000),
  ])
}
...
describe("POST /auth/identity/login", () => {
  test("issues tokens for a provisioned external identity", async () => {
    ...
    // jti が使用済みとして記録されている。
    const jti = await db
      .prepare("SELECT jti FROM system_identity_login_tokens WHERE jti = 'login-jti-1'")
      .first<string>("jti")
