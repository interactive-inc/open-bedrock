...
describe("GET /dashboard", () => {
  test("returns 200 with deterministic aggregated counts", async () => {
    const response = await request("/company/dashboard", await adminToken())
  ...
  test("returns 403 for member role", async () => {
    const response = await request("/company/dashboard", await memberToken())

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/company/dashboard", null)

    expect(response.status).toBe(401)
  })

  test("returns 401 with an invalid bearer token", async () => {
    const response = await request("/company/dashboard", "not-a-real-token")
