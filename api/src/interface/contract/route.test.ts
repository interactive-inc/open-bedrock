...
import { seedPartners } from "@/infrastructure/seed/seed-partners"
import { seedContracts } from "@/infrastructure/seed/seed-contracts"
...
describe("GET /contracts", () => {
  ...
  test("returns 403 for a viewer without read:all (member)", async () => {
    const response = await request("/contracts", await tokenFor(5, "member"))

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/contracts", null)
...
describe("POST /contracts", () => {
  ...
  test("returns 403 for a member", async () => {
    const response = await request("/contracts", await tokenFor(5, "member"), "POST", {
      partner_id: 1,
      title: "Blocked",
      contract_date: "2026-02-01",
    })
...
describe("PUT /contracts/:id", () => {
  ...
  test("returns 403 for a member", async () => {
    const response = await request("/contracts/1", await tokenFor(5, "member"), "PUT", {
      title: "Hijacked",
      contract_date: "2026-01-15",
    })
