...
describe("GET /attendance/overtime-summary", () => {
  ...
  test("member requesting scope=reports is forbidden", async () => {
    const response = await requestWithContext({
      db: await createScopeTestDb(),
      jwtSecret,
      path: "/attendance/overtime-summary?scope=reports",
  ...
  test("member requesting scope=all is forbidden", async () => {
    const response = await requestWithContext({
      db: await createScopeTestDb(),
      jwtSecret,
      path: "/attendance/overtime-summary?scope=all",
      token: await tokenFor(20, "member"),
      now: "2026-06-15T00:00:00.000Z",
    })
  ...
  test("scope unspecified aggregates only the caller", async () => {
    const response = await requestWithContext({
      db: await createScopeTestDb(),
      jwtSecret,
      path: "/attendance/overtime-summary?month=2026-06",
  ...
  test("invalid month returns 400", async () => {
    const response = await requestWithContext({
      db: await createScopeTestDb(),
      jwtSecret,
      path: "/attendance/overtime-summary?month=2026-6",
      token: await tokenFor(21, "member"),
      now: "2026-06-15T00:00:00.000Z",
    })
