...
describe("POST /applications/:id/approve", () => {
  ...
  test("returns 404 for an unknown id", async () => {
    const response = await request("/applications/9999/approve", await tokenFor(2, "manager"), {
      method: "POST",
      body: { comment: null },
    })
  ...
  test("returns 401 without a bearer token", async () => {
    const response = await request("/applications/1/approve", null, {
      method: "POST",
      body: { comment: null },
    })
  ...
  test("returns 403 for a non-privileged role", async () => {
    const response = await request("/applications/1/approve", await tokenFor(5, "member"), {
      method: "POST",
      body: { comment: null },
    })
