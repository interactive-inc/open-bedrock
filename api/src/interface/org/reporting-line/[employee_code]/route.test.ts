...
describe("GET /org/reporting-line/:employee_code", () => {
  test("returns 200 with the chain ordered from subject to top", async () => {
    const response = await request("/org/reporting-line/E005", await memberToken())
  ...
  test("returns 404 when the employee has no membership", async () => {
    const response = await request("/org/reporting-line/E999", await memberToken())

    expect(response.status).toBe(404)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/org/reporting-line/E005", null)
