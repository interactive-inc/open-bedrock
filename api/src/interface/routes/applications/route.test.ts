...
describe("GET /applications", () => {
  test("returns 200 with the token employee's own applications", async () => {
  ...
  test("filters by status", async () => {
    const response = await request("/applications?status=approved", await tokenFor(10, "member"))
...
describe("GET /applications?scope=department", () => {
  ...
  test("department reader outside the department is forbidden", async () => {
    ...
    const response = await requestWithContext({
      db,
      jwtSecret,
      path: "/applications?scope=department&department_code=D002",
      token: await tokenFor(20, "member"),
    })
