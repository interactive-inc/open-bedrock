...
describe("workflow repair routes", () => {
  test("lists a missing current-step snapshot only for a global workflow administrator", async () => {
    ...
    const response = await request(db, "/applications/workflow-repairs", 1)
    ...
    expect((await request(db, "/applications/workflow-repairs", 2)).status).toBe(403)
    expect((await request(db, "/applications/workflow-repairs", null)).status).toBe(401)
  })
  ...
  test("paginates repair candidates while preserving the unfiltered total", async () => {
    ...
    const response = await request(db, "/applications/workflow-repairs?limit=1&offset=1", 1)
  ...
  test("rejects reassignment when the first post-deadline request activates a reachable escalation", async () => {
    ...
    const response = await request(
      db,
      `/applications/${applicationId}/reassign-workflow-step`,
      1,
      "POST",
  ...
  test("requires an explicit audited quorum when an all-approval snapshot is missing", async () => {
    ...
    await db
      ...
      .run()
    const path = `/applications/${applicationId}/reassign-workflow-step`
