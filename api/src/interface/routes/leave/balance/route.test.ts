...
describe("GET /leave/balance", () => {
  test("manager reads a report's balance via employee_id", async () => {
    const response = await getRequest("/leave/balance?employee_id=20", await tokenFor(2, "manager"))
  ...
  test("member requesting another employee's balance is forbidden", async () => {
    const response = await getRequest("/leave/balance?employee_id=23", await tokenFor(20, "member"))

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await getRequest("/leave/balance?employee_id=20", null)
