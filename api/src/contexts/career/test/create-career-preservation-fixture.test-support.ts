...
const secret = "career-integration-test-secret"

/** 実System・Companyとキャリア公募・応募・シートの原記録を接続する保全fixture。 */
export async function createCareerPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "career-preservation",
    revision: 1,
    title: "Preserve career records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "career-pagination-test-pepper",
