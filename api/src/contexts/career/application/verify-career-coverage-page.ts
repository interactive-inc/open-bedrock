...
import {
  CareerCoverageForbiddenError,
  CareerCoverageConflictError,
} from "@/contexts/career/application/errors"
import { careerCoveragePageCommandSchema } from "@/contexts/career/domain/schemas/career-coverage-page-command.schema"
...
export class VerifyCareerCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = careerCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CareerCoverageForbiddenError("coverage authentication required")
    const proof = await new CareerRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new CareerCoverageForbiddenError("coverage authorization denied")
    const generation = await new CareerRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new CareerRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new CareerRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      return new CareerCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new CareerCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenCareerRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new CareerCoverageConflictError("coverage mapping or purpose conflicts")
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new CareerCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new CareerCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: CareerRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new CareerCoverageConflictError("coverage terminal page changed")
    return page
  }
}
