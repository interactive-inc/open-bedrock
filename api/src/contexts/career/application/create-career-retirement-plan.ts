import { PrepareCareerRetirementPlanAdapter } from "@/contexts/career/infrastructure/adapters/prepare-career-retirement-plan.adapter"
...
import { CareerRetirementConflictError } from "@/contexts/career/application/errors"

type Context = ConstructorParameters<typeof PrepareCareerRetirementPlanAdapter>[0]

/** career記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateCareerRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareCareerRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new CareerRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CareerRetirementConflictError("retirement plan creation conflicts")
  }
}
