import { zAppCareerRetirementExecution } from "@/contexts/career/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { careerFactory } from "@/contexts/career/interface/request-environment/career-factory"
import { ExecuteCareerRetirementAdapter } from "@/contexts/career/infrastructure/adapters/execute-career-retirement.adapter"
import {
...
export const POST = careerFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteCareerRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppCareerRetirementExecution.parse(submitted), 200)
  },
)
