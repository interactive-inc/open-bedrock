import { zAppCareerRetirementRequest } from "@/contexts/career/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { careerFactory } from "@/contexts/career/interface/request-environment/career-factory"
import { SubmitCareerRetirementRequestAdapter } from "@/contexts/career/infrastructure/adapters/submit-career-retirement-request.adapter"
import {
...
export const POST = careerFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCareerRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof CareerRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CareerRetirementConflictError)
    ...
    return c.json(
      zAppCareerRetirementRequest.parse(submitted.request),
