...
import { CreateCareerRetirementPlan } from "@/contexts/career/application/create-career-retirement-plan"
import { careerRetirementPlanCommandSchema } from "@/contexts/career/domain/schemas/career-retirement-plan-command.schema"
import { zAppCareerRetirementPlan } from "@/contexts/career/interface/http/response-schemas"
...
export const POST = careerFactory.createHandlers(
  ...
  zValidator("json", careerRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = careerRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateCareerRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppCareerRetirementPlan.parse({
