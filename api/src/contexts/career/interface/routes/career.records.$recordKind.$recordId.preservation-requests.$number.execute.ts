...
import { RevalidateCareerRecordSourceAdapter } from "@/contexts/career/infrastructure/adapters/revalidate-career-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateCareerRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
