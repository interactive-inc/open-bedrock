...
import { VerifyCareerRetirementPage } from "@/contexts/career/application/verify-career-retirement-page"
...
import { zAppCareerRetirementVerificationReceipt } from "@/contexts/career/interface/http/response-schemas"
...
export const POST = careerFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = careerRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyCareerRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof CareerRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof CareerRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppCareerRetirementVerificationReceipt.parse({
