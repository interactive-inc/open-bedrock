...
export const careerSourceFreezeResponseSchema = z.strictObject({ freeze: recordSourceFreezeSnapshotSchema })
...
export const zAppCareerRetirementPlan = z.strictObject({
  ...
  totalPages: z.number().int().positive().safe(), recordKinds: z.array(careerRecordKindSchema).length(3),
...
export const zAppCareerRetirementExecution = z.strictObject({ retirement_id: z.uuid(), finalized_at: z.iso.datetime() })
