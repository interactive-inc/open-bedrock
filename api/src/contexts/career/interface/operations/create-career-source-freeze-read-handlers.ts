...
import { z } from "zod"
import { careerSourceFreezeResponseSchema } from "@/contexts/career/interface/http/response-schemas"
...
/** 現在の管理権限でcareer記録の停止世代を読む。 */
export function createCareerSourceFreezeReadHandlers() {
  return careerFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "career" ||
      ...
      return c.json(careerSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
