...
/** career記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareCareerRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = careerRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new CareerActorReadAdapter(this.c).prepare()
    if (reader instanceof Error) return reader
    const baseAssertions = [...authority.assertions, ...reader.assertions]
    const chains = []
    for (const recordKind of careerRecordKinds) {
    ...
    const currentReader = await new CareerActorReadAdapter(this.c).prepare()
