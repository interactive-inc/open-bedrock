...
import { careerRetirementSubmissionCommandSchema } from "@/contexts/career/domain/schemas/career-retirement-submission-command.schema"
import { PrepareCareerRetirementCurrentStateAdapter } from "@/contexts/career/infrastructure/adapters/prepare-career-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareCareerRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitCareerRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = careerRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CareerRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareCareerRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new CareerRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "career.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new CareerRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new CareerRetirementForbiddenError("retirement applicant differs")
      if (
        ...
        return new CareerRetirementConflictError("retirement previous version is not resubmittable")
      ...
      if (
        ...
        return new CareerRetirementConflictError(
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new CareerRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareCareerRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new CareerRetirementForbiddenError("retirement decision qualification required")
    ...
    if (started instanceof Error) {
      ...
      return new CareerRetirementConflictError("retirement submission changed before persistence", {
        cause: started,
      })
    }
    return {
