...
export class OnboardingActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (authorization instanceof Error)
      return new OnboardingError("onboarding_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new OnboardingError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new OnboardingError("onboarding_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors[0]?.employee === undefined)
      return new OnboardingError("forbidden", "active employee is required")
    return {
