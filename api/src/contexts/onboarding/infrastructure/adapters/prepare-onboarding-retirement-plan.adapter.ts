...
/** onboarding記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareOnboardingRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = onboardingRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new OnboardingActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new OnboardingActorReadAdapter(this.c).prepare()
