...
import { CreateOnboardingRetirementPlan } from "@/contexts/onboarding/application/create-onboarding-retirement-plan"
...
import { zAppOnboardingRetirementPlan } from "@/contexts/onboarding/interface/http/response-schemas"
...
export const POST = onboardingFactory.createHandlers(
  ...
  zValidator("json", onboardingRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = onboardingRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateOnboardingRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppOnboardingRetirementPlan.parse({
