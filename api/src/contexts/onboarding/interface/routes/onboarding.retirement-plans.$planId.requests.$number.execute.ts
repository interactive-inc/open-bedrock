import { zAppOnboardingRetirementExecution } from "@/contexts/onboarding/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { onboardingFactory } from "@/contexts/onboarding/interface/request-environment/onboarding-factory"
import { ExecuteOnboardingRetirementAdapter } from "@/contexts/onboarding/infrastructure/adapters/execute-onboarding-retirement.adapter"
import {
...
export const POST = onboardingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteOnboardingRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppOnboardingRetirementExecution.parse(submitted), 200)
  },
)
