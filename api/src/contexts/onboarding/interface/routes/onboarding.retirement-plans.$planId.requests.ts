import { zAppOnboardingRetirementRequest } from "@/contexts/onboarding/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { onboardingFactory } from "@/contexts/onboarding/interface/request-environment/onboarding-factory"
import { SubmitOnboardingRetirementRequestAdapter } from "@/contexts/onboarding/infrastructure/adapters/submit-onboarding-retirement-request.adapter"
import {
...
export const POST = onboardingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitOnboardingRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof OnboardingRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof OnboardingRetirementConflictError)
    ...
    return c.json(
      zAppOnboardingRetirementRequest.parse(submitted.request),
