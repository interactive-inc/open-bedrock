import { PrepareOnboardingRetirementPageAdapter } from "@/contexts/onboarding/infrastructure/adapters/prepare-onboarding-retirement-page.adapter"
...
/** onboarding記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyOnboardingRetirementPage {
  ...
  async execute(
    ...
    const parsed = onboardingRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new OnboardingRetirementForbiddenError("retirement authentication required")
    const authority = await new OnboardingRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new OnboardingRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new OnboardingRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new OnboardingRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new OnboardingRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new OnboardingRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareOnboardingRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new OnboardingRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new OnboardingRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new OnboardingRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new OnboardingRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new OnboardingRetirementConflictError("retirement verification position already written")
  }
}
