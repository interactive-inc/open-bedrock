import { PrepareOnboardingRetirementPlanAdapter } from "@/contexts/onboarding/infrastructure/adapters/prepare-onboarding-retirement-plan.adapter"
...
import { OnboardingRetirementConflictError } from "@/contexts/onboarding/application/errors"

type Context = ConstructorParameters<typeof PrepareOnboardingRetirementPlanAdapter>[0]

/** onboarding記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateOnboardingRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareOnboardingRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new OnboardingRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new OnboardingRetirementConflictError("retirement plan creation conflicts")
  }
}
