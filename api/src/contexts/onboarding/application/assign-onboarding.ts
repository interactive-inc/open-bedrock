...
type Context = Readonly<{
  ...
  templateRepository: Pick<OnboardingTemplateRepository, "findByCode">
  assignmentRepository: Pick<
    OnboardingAssignmentRepository,
    "findActiveByEmployeeAndTemplate" | "create"
  >
}>
...
export class AssignOnboarding {
  ...
  async run(command: Command): Promise<AssignOnboardingResult | ApplicationError> {
    ...
    const employee = await this.c.employeeDirectory.findByCode(command.employeeCode)
    ...
    const template = await this.c.templateRepository.findByCode(command.templateCode)
    ...
    const existing = await this.c.assignmentRepository.findActiveByEmployeeAndTemplate(
      employee.id,
      template.code,
    )
    ...
    const created = await this.c.assignmentRepository.create(assignment)
