...
test("入社手続きテンプレートを停止中に人の承認で保全し、業務コードを外してもSystemから読める", async () => {
  const { database, creator, reviewer, definition, bindings, tokenFor, request } =
    await createOnboardingPreservationFixture()
  await database.exec(`INSERT INTO onboarding_templates
  ...
  const frozen = await app.request(
    "/onboarding/record-source-freezes",
    {
      ...
      body: JSON.stringify({ reason: "Retain onboarding evidence" }),
    },
    bindings,
  )
  ...
  for (const [recordKind, recordId] of sources) {
    ...
    const submitted = await request(path, {
      ...
      body: {
        ...
        conditions: {
          reason: "Retain original onboarding record",
