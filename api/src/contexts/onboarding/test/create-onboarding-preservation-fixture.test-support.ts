...
const secret = "onboarding-integration-test-secret"

/** 実System・Companyと入退社手続き5台帳の原記録を接続する保全fixture。 */
export async function createOnboardingPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "onboarding-preservation",
    revision: 1,
    title: "Preserve onboarding records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "onboarding-pagination-test-pepper",
