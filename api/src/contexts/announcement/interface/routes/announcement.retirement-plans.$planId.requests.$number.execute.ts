import { zAppAnnouncementRetirementExecution } from "@/contexts/announcement/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { announcementFactory } from "@/contexts/announcement/interface/request-environment/announcement-factory"
import { ExecuteAnnouncementRetirementAdapter } from "@/contexts/announcement/infrastructure/adapters/execute-announcement-retirement.adapter"
import {
...
export const POST = announcementFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteAnnouncementRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppAnnouncementRetirementExecution.parse(submitted), 200)
  },
)
