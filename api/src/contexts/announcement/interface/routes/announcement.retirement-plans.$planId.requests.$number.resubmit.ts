import { zAppAnnouncementRetirementRequest } from "@/contexts/announcement/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { announcementFactory } from "@/contexts/announcement/interface/request-environment/announcement-factory"
import { SubmitAnnouncementRetirementRequestAdapter } from "@/contexts/announcement/infrastructure/adapters/submit-announcement-retirement-request.adapter"
import {
...
export const POST = announcementFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitAnnouncementRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof AnnouncementRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof AnnouncementRetirementConflictError)
    ...
    return c.json(
      zAppAnnouncementRetirementRequest.parse(submitted.request),
