...
import { VerifyAnnouncementCoveragePage } from "@/contexts/announcement/application/verify-announcement-coverage-page"
import {
  AnnouncementCoverageForbiddenError,
  AnnouncementCoverageConflictError,
} from "@/contexts/announcement/application/errors"
import { zAppAnnouncementCoveragePageReceipt } from "@/contexts/announcement/interface/http/response-schemas"
...
export const POST = announcementFactory.createHandlers(
  ...
  zValidator("json", announcementCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = announcementCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyAnnouncementCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof AnnouncementCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof AnnouncementCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppAnnouncementCoveragePageReceipt.parse({
