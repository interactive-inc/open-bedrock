...
import { z } from "zod"
import { announcementSourceFreezeResponseSchema } from "@/contexts/announcement/interface/http/response-schemas"
...
/** 現在の管理権限でアナウンスの停止世代を読む。 */
export function createAnnouncementSourceFreezeReadHandlers() {
  return announcementFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "announcement" ||
      ...
      return c.json(announcementSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
