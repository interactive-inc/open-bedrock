...
/** 未解除の停止世代からアナウンスIDを分割取得し、後続の保存でも同じ世代を検査する。 */
export class ListFrozenAnnouncementRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    try {
      const page =
        request.afterId === null
          ? this.c.env.DB.prepare(
              "SELECT id FROM announcements ORDER BY id COLLATE BINARY LIMIT ?1",
            ).bind(request.limit + 1)
          : this.c.env.DB.prepare(
              "SELECT id FROM announcements WHERE id COLLATE BINARY>?1 ORDER BY id COLLATE BINARY LIMIT ?2",
      ...
      if (reads.length !== statements.length || reads.some((read) => !read.success))
        return new Error("frozen announcement inventory unavailable")
      ...
      return new Error("frozen announcement inventory unavailable", { cause })
    }
  }
}
