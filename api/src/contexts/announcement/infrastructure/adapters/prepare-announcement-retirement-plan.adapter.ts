...
/** アナウンスの照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareAnnouncementRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = announcementRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new AnnouncementActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new AnnouncementActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "announcement",
        capability: { revision: 1, recordKinds: ["announcement-record"] },
        coverage: [
          {
