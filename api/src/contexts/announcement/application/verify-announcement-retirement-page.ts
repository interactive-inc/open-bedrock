import { PrepareAnnouncementRetirementPageAdapter } from "@/contexts/announcement/infrastructure/adapters/prepare-announcement-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareAnnouncementRetirementPageAdapter>[0]

/** アナウンスの保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyAnnouncementRetirementPage {
  ...
  async execute(
    ...
    const parsed = announcementRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new AnnouncementRetirementForbiddenError("retirement authentication required")
    const authority = await new AnnouncementRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new AnnouncementRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new AnnouncementRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new AnnouncementRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new AnnouncementRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new AnnouncementRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareAnnouncementRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new AnnouncementRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new AnnouncementRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new AnnouncementRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new AnnouncementRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AnnouncementRetirementConflictError("retirement verification position already written")
  }
}
