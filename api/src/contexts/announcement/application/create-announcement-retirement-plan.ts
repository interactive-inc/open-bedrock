import { PrepareAnnouncementRetirementPlanAdapter } from "@/contexts/announcement/infrastructure/adapters/prepare-announcement-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareAnnouncementRetirementPlanAdapter>[0]

/** 打刻の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateAnnouncementRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareAnnouncementRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new AnnouncementRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AnnouncementRetirementConflictError("retirement plan creation conflicts")
  }
}
