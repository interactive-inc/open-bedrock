...
const secret = "announcement-integration-test-secret"

/** 実System・Companyとアナウンス原記録を接続する保全fixture。 */
export async function createAnnouncementPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "announcement-preservation",
    revision: 1,
    title: "Preserve announcements",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "announcement-pagination-test-pepper",
