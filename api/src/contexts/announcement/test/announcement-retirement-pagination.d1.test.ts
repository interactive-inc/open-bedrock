...
test("11件のアナウンスを全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createAnnouncementPreservationFixture(await pool.next())
  ...
  expect(
    (
      await apiRequest("/announcement/announcements", {
        method: "POST",
        body: { title: "Late announcement", body_md: "Must not be written" },
      })
    ).status,
  ).toBe(409)
  expect(
  ...
  const sourcePath = `/announcement/record-source-freezes/${freezeId}`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["announcement-record"] })
  const retirementPath = `/announcement/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "announcement-retirement",
    revision: 1,
    title: "Retire preserved announcement",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/announcement/retirement-plans/${planId}/verification-receipts`
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved announcement",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_announcement_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_announcement_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('b30b4f0d-3de3-4851-80e5-d678e8f09789','system:record:export'); DROP TABLE announcements;",
  )
