...
import { RevalidateAssetRecordSourceAdapter } from "@/contexts/asset/infrastructure/adapters/revalidate-asset-record-source.adapter"
    ...
    const result = await executeSystemRecordPreservation(
      {
        ...
        source: {
          ...
          sourceNamespace,
          revalidate: (source) =>
            new RevalidateAssetRecordSourceAdapter({
              env: c.env,
              var: c.var,
              sourceNamespace,
            }).prepare(source),
