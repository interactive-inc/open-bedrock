...
import { VerifyAssetRetirementPage } from "@/contexts/asset/application/verify-asset-retirement-page"
...
import { zAppAssetRetirementVerificationReceipt } from "@/contexts/asset/interface/http/response-schemas"
...
export const POST = assetFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = assetRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyAssetRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof AssetRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppAssetRetirementVerificationReceipt.parse({
