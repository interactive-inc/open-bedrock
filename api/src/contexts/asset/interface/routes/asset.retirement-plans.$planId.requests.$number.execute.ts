import { zAppAssetRetirementExecution } from "@/contexts/asset/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { assetFactory } from "@/contexts/asset/interface/request-environment/asset-factory"
import { ExecuteAssetRetirementAdapter } from "@/contexts/asset/infrastructure/adapters/execute-asset-retirement.adapter"
import {
...
export const POST = assetFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteAssetRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppAssetRetirementExecution.parse(submitted), 200)
  },
)
