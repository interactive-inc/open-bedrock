...
import { VerifyAssetCoveragePage } from "@/contexts/asset/application/verify-asset-coverage-page"
import {
  AssetCoverageForbiddenError,
  AssetCoverageConflictError,
} from "@/contexts/asset/application/errors"
import { zAppAssetCoveragePageReceipt } from "@/contexts/asset/interface/http/response-schemas"
...
export const POST = assetFactory.createHandlers(
  ...
  zValidator("json", assetCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = assetCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyAssetCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof AssetCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof AssetCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppAssetCoveragePageReceipt.parse({
