import { zAppAssetRetirementRequest } from "@/contexts/asset/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { assetFactory } from "@/contexts/asset/interface/request-environment/asset-factory"
import { SubmitAssetRetirementRequestAdapter } from "@/contexts/asset/infrastructure/adapters/submit-asset-retirement-request.adapter"
import {
...
export const POST = assetFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitAssetRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof AssetRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof AssetRetirementConflictError)
    ...
    return c.json(
      zAppAssetRetirementRequest.parse(submitted.request),
