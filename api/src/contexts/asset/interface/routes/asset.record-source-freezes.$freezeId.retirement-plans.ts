...
import { AssetRetirementConflictError } from "@/contexts/asset/application/errors"
...
import { CreateAssetRetirementPlan } from "@/contexts/asset/application/create-asset-retirement-plan"
import { assetRetirementPlanCommandSchema } from "@/contexts/asset/domain/schemas/asset-retirement-plan-command.schema"
import { zAppAssetRetirementPlan } from "@/contexts/asset/interface/http/response-schemas"
...
export const POST = assetFactory.createHandlers(
  ...
  zValidator("json", assetRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = assetRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateAssetRetirementPlan(c).execute(command.data, stepUpToken)
    if (receipt instanceof ForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof AssetRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
