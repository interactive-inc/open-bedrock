...
import { assetRecordKindSchema, decodeStocktakeItemRecordId, encodeStocktakeItemRecordId } from "@/contexts/asset/domain/asset-record-kind"
import { RecordSourceFreezeRepository } from "@system/infrastructure/repositories/records/record-source-freeze.repository"

type Context = AssetContext
const inputSchema = z.strictObject({
  freezeId: z.uuid(), sourceNamespace: z.string().min(1).max(255).regex(/^\S+$/),
  recordKind: assetRecordKindSchema, afterCursor: z.string().nullable(),
...
export class ListFrozenAssetRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const generation = await new RecordSourceFreezeRepository({ env: this.c.env, assertions: actor.assertions })
      .prepareActiveGeneration({ id: request.freezeId, sourceNamespace: request.sourceNamespace, ownerContext: "asset" })
    ...
    try {
      ...
      const reads = await this.c.env.DB.batch<{ record_id: string | number; stocktake_id?: string; asset_code?: string }>(statements)
      ...
      const ids = rows.map((row) => request.recordKind === "stocktake-item-record"
  ...
  private query(kind: z.infer<typeof assetRecordKindSchema>, cursor: string | null, limit: number) {
    if (kind === "asset-record") return { sql: "SELECT code AS record_id FROM assets WHERE code>?1 ORDER BY code LIMIT ?2", values: [cursor ?? "", limit] }
    if (kind === "asset-lending-record") {
      const after = cursor === null ? 0 : Number(cursor)
      if (!Number.isSafeInteger(after) || after < 0 || (cursor !== null && String(after) !== cursor)) return new Error("invalid lending cursor")
      return { sql: "SELECT id AS record_id FROM asset_lendings WHERE id>?1 ORDER BY id LIMIT ?2", values: [after, limit] }
    }
    if (kind === "stocktake-record") return { sql: "SELECT id AS record_id FROM stocktakes WHERE id>?1 ORDER BY id LIMIT ?2", values: [cursor ?? "", limit] }
