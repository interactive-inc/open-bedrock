...
/** asset記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareAssetRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = assetRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new AssetActorReadAdapter(this.c).prepare()
    if (reader instanceof Error) return reader
    const baseAssertions = [...authority.assertions, ...reader.assertions]
    const chains = []
    for (const recordKind of assetRecordKinds) {
    ...
    const currentReader = await new AssetActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "asset",
        capability: { revision: 1, recordKinds: assetRecordKinds },
