...
export class AssetActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new AssetError("forbidden", "invalid asset actor")
    ...
    if (authorization instanceof Error)
      return new AssetError("asset_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new AssetError("asset_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new AssetError("forbidden", "active employee is required")
    return {
