...
type Context = AssetContext & SystemAttachmentStorageContext & SystemDatabaseContext
...
/** 計画に固定したasset記録ページの原文・保全本文・保持条件を再検証し、保存条件を渡す。 */
export class PrepareAssetRetirementPageAdapter {
  ...
  async prepare(input: unknown, stepUpToken: string) {
    ...
    const captured = await new CaptureFrozenAssetRecordPageAdapter(this.c).prepare({
      freezeId: request.freezeId,
      sourceNamespace: request.sourceNamespace,
      recordKind: request.recordKind,
      afterCursor: cursor,
    ...
    const current = await new AssetActorReadAdapter(this.c).prepare()
