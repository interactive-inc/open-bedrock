import { PrepareAssetRetirementPlanAdapter } from "@/contexts/asset/infrastructure/adapters/prepare-asset-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareAssetRetirementPlanAdapter>[0]

/** asset記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateAssetRetirementPlan {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    const repository = new AssetRecordSystemAdapter({
      env: this.c.env,
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new AssetRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AssetRetirementConflictError("retirement plan creation conflicts")
  }
}
