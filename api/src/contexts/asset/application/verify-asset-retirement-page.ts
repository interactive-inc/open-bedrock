import { PrepareAssetRetirementPageAdapter } from "@/contexts/asset/infrastructure/adapters/prepare-asset-retirement-page.adapter"
import { AssetRecordSystemAdapter } from "@/contexts/asset/infrastructure/adapters/asset-record-system.adapter"
import { assetRetirementVerificationCommandSchema } from "@/contexts/asset/domain/schemas/asset-retirement-verification-command.schema"
import {
...
import { assetRecordKinds } from "@/contexts/asset/domain/definitions/asset-record-kind.definition"

type Context = ConstructorParameters<typeof PrepareAssetRetirementPageAdapter>[0]

/** asset記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyAssetRetirementPage {
  ...
  async execute(
    ...
    const parsed = assetRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new AssetRetirementForbiddenError("retirement authentication required")
    const authority = await new AssetRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (authority === "forbidden")
      return new AssetRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new AssetRecordSystemAdapter(context).retirementPlans().find(command.planId)
    if (plan instanceof Error) return plan
    if (
      plan === null ||
      plan.snapshot.sourceNamespace !== command.sourceNamespace ||
      plan.snapshot.ownerContext !== "asset" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(assetRecordKinds)
    )
      return new AssetRetirementConflictError("retirement plan unavailable or capability changed")
    const repository = new AssetRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new AssetRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new AssetRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareAssetRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new AssetRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new AssetRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new AssetRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new AssetRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AssetRetirementConflictError("retirement verification position already written")
  }
}
