...
import { z } from "zod"
import { healthCheckupSourceFreezeResponseSchema } from "@/contexts/health-checkup/interface/http/response-schemas"
...
/** 現在の管理権限で健康診断実施記録の停止世代を読む。 */
export function createHealthCheckupSourceFreezeReadHandlers() {
  return healthCheckupFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "health-checkup" ||
      ...
      return c.json(healthCheckupSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
