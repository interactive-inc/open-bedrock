...
import { VerifyHealthCheckupRetirementPage } from "@/contexts/health-checkup/application/verify-health-checkup-retirement-page"
...
import { zAppHealthCheckupRetirementVerificationReceipt } from "@/contexts/health-checkup/interface/http/response-schemas"
...
export const POST = healthCheckupFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = healthCheckupRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    return c.json(
      zAppHealthCheckupRetirementVerificationReceipt.parse({
