import { zAppHealthCheckupRetirementRequest } from "@/contexts/health-checkup/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { healthCheckupFactory } from "@/contexts/health-checkup/interface/request-environment/health-checkup-factory"
import { SubmitHealthCheckupRetirementRequestAdapter } from "@/contexts/health-checkup/infrastructure/adapters/submit-health-checkup-retirement-request.adapter"
import {
...
export const POST = healthCheckupFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitHealthCheckupRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof HealthCheckupRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof HealthCheckupRetirementConflictError)
    ...
    return c.json(
      zAppHealthCheckupRetirementRequest.parse(submitted.request),
