...
export class VerifyHealthCheckupCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    ...
    const proof = await new HealthCheckupRecordSystemAdapter(
      this.c,
    ...
    const generation = await new HealthCheckupRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new HealthCheckupRecordSystemAdapter({
      env: this.c.env,
  ...
  private async confirmTerminal(
    ...
    completion: HealthCheckupRecordSystemAdapter,
  ) {
