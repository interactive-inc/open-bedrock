...
type Context = ConstructorParameters<typeof PrepareHealthCheckupRetirementPlanAdapter>[0]

/** 健康診断記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateHealthCheckupRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareHealthCheckupRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new HealthCheckupRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new HealthCheckupRetirementConflictError("retirement plan creation conflicts")
  }
}
