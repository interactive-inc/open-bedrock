import { PrepareCompensationChangeRetirementPageAdapter } from "@/contexts/compensation-change/infrastructure/adapters/prepare-compensation-change-retirement-page.adapter"
...
/** compensation-change記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyCompensationChangeRetirementPage {
  ...
  async execute(
    ...
    const parsed = compensationChangeRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CompensationChangeRetirementForbiddenError("retirement authentication required")
    const authority = await new CompensationChangeRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new CompensationChangeRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new CompensationChangeRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new CompensationChangeRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new CompensationChangeRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new CompensationChangeRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareCompensationChangeRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new CompensationChangeRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new CompensationChangeRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new CompensationChangeRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new CompensationChangeRecordSystemAdapter(guardedContext)
      .coveragePages()
