...
type Context = ConstructorParameters<typeof PrepareCompensationChangeRetirementPlanAdapter>[0]

/** compensation-change記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateCompensationChangeRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareCompensationChangeRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new CompensationChangeRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CompensationChangeRetirementConflictError("retirement plan creation conflicts")
  }
}
