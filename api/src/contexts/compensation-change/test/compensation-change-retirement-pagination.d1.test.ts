...
import { createCompensationChangePreservationFixture } from "@/contexts/compensation-change/test/create-compensation-change-preservation-fixture.test-support"
...
test("給与改定記録を全件保全し、人の承認を経て1台帳を撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createCompensationChangePreservationFixture(await pool.next())
  ...
  for (const sourceRecord of sourceRecords) {
    const path = `/compensation-change/records/${sourceRecord.recordKind}/${encodeURIComponent(sourceRecord.recordId)}/preservation-requests`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: compensationChangeRecordKinds })
  const retirementPath = `/compensation-change/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "compensation-change-retirement",
    revision: 1,
    title: "Retire preserved compensation-change",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/compensation-change/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM salary_revisions").first<number>("n"),
  ).toBe(11)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved compensation-change",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_compensation_change_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_compensation_change_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('86a34a82-3294-4dbd-8a77-e808623ac2bf','system:record:export'); DROP TABLE salary_revisions;",
  )
