...
test("給与改定記録を全件保全し、人の承認を経て1台帳を撤去確定する", async () => {
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('compensation-change-test-manager','system:record:export'); DROP TABLE salary_revisions;",
  )
