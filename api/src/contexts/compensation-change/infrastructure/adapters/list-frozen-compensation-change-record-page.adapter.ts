...
const inputSchema = z.strictObject({ freezeId: z.uuid(), sourceNamespace: z.string().min(1).max(255).regex(/^\S+$/),
  recordKind: compensationChangeRecordKindSchema, afterCursor: z.string().nullable(), limit: z.number().int().min(1).max(100) })
...
export class ListFrozenCompensationChangeRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const actor = await new CompensationChangeActorReadAdapter(this.c).prepare(); if (actor instanceof Error) return actor
    const generation = await new RecordSourceFreezeRepository({ env: this.c.env, assertions: actor.assertions })
      .prepareActiveGeneration({ id: request.freezeId, sourceNamespace: request.sourceNamespace, ownerContext: "compensation-change" })
    ...
    } catch (cause) { return new Error("frozen compensation change inventory unavailable", { cause }) }
  }
}
