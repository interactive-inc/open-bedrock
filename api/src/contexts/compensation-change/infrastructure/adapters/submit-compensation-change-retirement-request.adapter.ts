...
type Context = ConstructorParameters<
  typeof PrepareCompensationChangeRetirementCurrentStateAdapter
>[0]
...
export class SubmitCompensationChangeRetirementRequestAdapter {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    const current = await new PrepareCompensationChangeRetirementCurrentStateAdapter(
      this.c,
    ).prepare(
      {
    ...
    const replay = async () => {
      ...
      const rechecked = await new PrepareCompensationChangeRetirementCurrentStateAdapter(
        this.c,
      ).prepare(
        {
          planId: command.planId,
    ...
    if (task.resolved.guards.length === 0)
      return new CompensationChangeRetirementForbiddenError(
