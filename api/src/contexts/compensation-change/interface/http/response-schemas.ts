...
export const compensationChangeSourceFreezeResponseSchema = z.strictObject({ freeze: recordSourceFreezeSnapshotSchema })
...
export const zAppCompensationChangeRetirementPlan = z.strictObject({
  ...
  totalPages: z.number().int().positive().safe(), recordKinds: z.array(compensationChangeRecordKindSchema).length(1),
...
export const zAppCompensationChangeRetirementExecution = z.strictObject({ retirement_id: z.uuid(), finalized_at: z.iso.datetime() })
