import { zAppCompensationChangeRetirementExecution } from "@/contexts/compensation-change/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecuteCompensationChangeRetirementAdapter } from "@/contexts/compensation-change/infrastructure/adapters/execute-compensation-change-retirement.adapter"
import {
...
export const POST = compensationChangeFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteCompensationChangeRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppCompensationChangeRetirementExecution.parse(submitted), 200)
  },
)
