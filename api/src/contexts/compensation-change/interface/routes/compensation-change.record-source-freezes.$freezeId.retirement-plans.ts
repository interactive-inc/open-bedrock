...
import { CreateCompensationChangeRetirementPlan } from "@/contexts/compensation-change/application/create-compensation-change-retirement-plan"
...
import { zAppCompensationChangeRetirementPlan } from "@/contexts/compensation-change/interface/http/response-schemas"
...
export const POST = compensationChangeFactory.createHandlers(
  ...
  zValidator("json", compensationChangeRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = compensationChangeRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateCompensationChangeRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppCompensationChangeRetirementPlan.parse({
