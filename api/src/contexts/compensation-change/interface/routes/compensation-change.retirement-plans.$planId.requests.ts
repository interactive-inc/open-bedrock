import { zAppCompensationChangeRetirementRequest } from "@/contexts/compensation-change/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitCompensationChangeRetirementRequestAdapter } from "@/contexts/compensation-change/infrastructure/adapters/submit-compensation-change-retirement-request.adapter"
import {
...
export const POST = compensationChangeFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCompensationChangeRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof CompensationChangeRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CompensationChangeRetirementConflictError)
    ...
    return c.json(
      zAppCompensationChangeRetirementRequest.parse(submitted.request),
