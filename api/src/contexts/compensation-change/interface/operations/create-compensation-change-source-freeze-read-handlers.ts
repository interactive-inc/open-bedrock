...
import { z } from "zod"
import { compensationChangeSourceFreezeResponseSchema } from "@/contexts/compensation-change/interface/http/response-schemas"
...
/** 現在の管理権限でcompensation-change記録の停止世代を読む。 */
export function createCompensationChangeSourceFreezeReadHandlers() {
  return compensationChangeFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "compensation-change" ||
      ...
      return c.json(compensationChangeSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
