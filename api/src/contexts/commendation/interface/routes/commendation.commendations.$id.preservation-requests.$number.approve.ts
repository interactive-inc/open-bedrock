import { createCommendationPreservationDecisionHandlers } from "@/contexts/commendation/interface/operations/create-commendation-preservation-decision-handlers"
import { commendationFactory } from "@/contexts/commendation/interface/request-environment/commendation-factory"
import { authenticateSystemAccessToken } from "@system/interface/middlewares/authenticate-system-access-token"

// @authorization service - 現在の人間の認証とCompanyの承認資格でSystemの判断を記録する
export const POST = commendationFactory.createHandlers(
  authenticateSystemAccessToken,
  ...createCommendationPreservationDecisionHandlers("approve"),
)
