...
import { VerifyCommendationCoveragePage } from "@/contexts/commendation/application/verify-commendation-coverage-page"
import {
  CommendationCoverageForbiddenError,
  CommendationCoverageConflictError,
} from "@/contexts/commendation/application/errors"
import { zAppCommendationCoveragePageReceipt } from "@/contexts/commendation/interface/http/response-schemas"
...
export const POST = commendationFactory.createHandlers(
  ...
  zValidator("json", commendationCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = commendationCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyCommendationCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof CommendationCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof CommendationCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppCommendationCoveragePageReceipt.parse({
