...
import { CommendationRetirementConflictError } from "@/contexts/commendation/application/errors"
...
import { CreateCommendationRetirementPlan } from "@/contexts/commendation/application/create-commendation-retirement-plan"
import { commendationRetirementPlanCommandSchema } from "@/contexts/commendation/domain/schemas/commendation-retirement-plan-command.schema"
import { zAppCommendationRetirementPlan } from "@/contexts/commendation/interface/http/response-schemas"
...
export const POST = commendationFactory.createHandlers(
  ...
  zValidator("json", commendationRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = commendationRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateCommendationRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppCommendationRetirementPlan.parse({
