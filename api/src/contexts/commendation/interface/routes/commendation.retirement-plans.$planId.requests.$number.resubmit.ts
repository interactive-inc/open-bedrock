import { zAppCommendationRetirementRequest } from "@/contexts/commendation/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { commendationFactory } from "@/contexts/commendation/interface/request-environment/commendation-factory"
import { SubmitCommendationRetirementRequestAdapter } from "@/contexts/commendation/infrastructure/adapters/submit-commendation-retirement-request.adapter"
import {
...
export const POST = commendationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCommendationRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof CommendationRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CommendationRetirementConflictError)
    ...
    return c.json(
      zAppCommendationRetirementRequest.parse(submitted.request),
