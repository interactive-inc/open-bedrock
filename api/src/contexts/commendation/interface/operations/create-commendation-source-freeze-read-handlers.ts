...
import { z } from "zod"
import { commendationSourceFreezeResponseSchema } from "@/contexts/commendation/interface/http/response-schemas"
...
/** 現在の管理権限で表彰記録の停止世代を読む。 */
export function createCommendationSourceFreezeReadHandlers() {
  return commendationFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "commendation" ||
      ...
      return c.json(commendationSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
