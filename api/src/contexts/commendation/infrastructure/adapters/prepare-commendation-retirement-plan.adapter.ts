...
/** 表彰記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareCommendationRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = commendationRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new CommendationActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new CommendationActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "commendation",
        capability: { revision: 1, recordKinds: ["commendation-record"] },
        coverage: [
          {
