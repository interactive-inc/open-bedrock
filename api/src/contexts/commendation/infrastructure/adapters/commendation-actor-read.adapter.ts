...
export class CommendationActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new CommendationError("forbidden", "invalid commendation actor")
    const authorization = await new SystemHumanOperationAuthorizationAdapter(this.c).prepare({
      ...
      permissions: ["commendation:manage"],
      now,
    })
    if (authorization instanceof Error)
      return new CommendationError("commendation_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new CommendationError("commendation_unavailable", "Company directory is unavailable")
    return {
