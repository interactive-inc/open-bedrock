  ...
  async findById(id: number): Promise<Commendation | null | Error> {
    try {
      const rows = await this.c.var.database
        ...
        .limit(1)
      const row = rows.at(0)
      return row === undefined ? null : Commendation.fromRow(row)
    } catch (error) {
      return error instanceof Error ? error : new Error("failed to load commendation")
    }
  }
  ...
  async delete(commendation: Commendation): Promise<boolean | Error> {
    ...
    try {
      const rows = await this.c.var.database
        ...
        .where(eq(commendations.id, commendation.id))
        .returning()
