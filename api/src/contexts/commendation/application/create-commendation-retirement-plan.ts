import { PrepareCommendationRetirementPlanAdapter } from "@/contexts/commendation/infrastructure/adapters/prepare-commendation-retirement-plan.adapter"
...
export class CreateCommendationRetirementPlan {
  ...
  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareCommendationRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new CommendationRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CommendationRetirementConflictError("retirement plan creation conflicts")
  }
}
