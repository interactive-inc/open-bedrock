...
import {
  CommendationCoverageForbiddenError,
  CommendationCoverageConflictError,
} from "@/contexts/commendation/application/errors"
import { commendationCoveragePageCommandSchema } from "@/contexts/commendation/domain/schemas/commendation-coverage-page-command.schema"
...
export class VerifyCommendationCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = commendationCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CommendationCoverageForbiddenError("coverage authentication required")
    const proof = await new CommendationRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new CommendationCoverageForbiddenError("coverage authorization denied")
    const generation = await new CommendationRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new CommendationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new CommendationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "commendation-record")
    )
      return new CommendationCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new CommendationCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenCommendationRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
    ...
    if (
      ...
      return new CommendationCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareCommendationCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new CommendationCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new CommendationCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: CommendationRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new CommendationCoverageConflictError("coverage terminal page changed")
    return page
  }
}
