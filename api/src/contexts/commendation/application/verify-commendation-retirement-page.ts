import { PrepareCommendationRetirementPageAdapter } from "@/contexts/commendation/infrastructure/adapters/prepare-commendation-retirement-page.adapter"
import { CommendationRecordSystemAdapter } from "@/contexts/commendation/infrastructure/adapters/commendation-record-system.adapter"
import { commendationRetirementVerificationCommandSchema } from "@/contexts/commendation/domain/schemas/commendation-retirement-verification-command.schema"
import {
  CommendationRetirementForbiddenError,
  CommendationRetirementConflictError,
} from "@/contexts/commendation/application/errors"
...
type Context = ConstructorParameters<typeof PrepareCommendationRetirementPageAdapter>[0]

/** 表彰記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyCommendationRetirementPage {
  ...
  async execute(
    ...
    const parsed = commendationRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CommendationRetirementForbiddenError("retirement authentication required")
    const authority = await new CommendationRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new CommendationRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new CommendationRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "commendation" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !==
        JSON.stringify(["commendation-record"])
    )
      return new CommendationRetirementConflictError(
        "retirement plan unavailable or capability changed",
      )
    const repository = new CommendationRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new CommendationRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new CommendationRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareCommendationRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new CommendationRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new CommendationRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new CommendationRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new CommendationRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CommendationRetirementConflictError("retirement verification position already written")
  }
}
