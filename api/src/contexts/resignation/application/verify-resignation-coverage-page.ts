...
import {
  ResignationCoverageForbiddenError,
  ResignationCoverageConflictError,
} from "@/contexts/resignation/application/errors"
import { resignationCoveragePageCommandSchema } from "@/contexts/resignation/domain/schemas/resignation-coverage-page-command.schema"
...
export class VerifyResignationCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = resignationCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new ResignationCoverageForbiddenError("coverage authentication required")
    const proof = await new ResignationRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization(
      {
        authentication,
        now: this.c.var.now(),
        stepUpToken,
    ...
    if (proof === "forbidden")
      return new ResignationCoverageForbiddenError("coverage authorization denied")
    const generation = await new ResignationRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new ResignationRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "resignation-record")
    )
      return new ResignationCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new ResignationCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenResignationRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      afterId: cursor,
      limit: 10,
    ...
    if (
      ...
      return new ResignationCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareResignationCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new ResignationCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new ResignationCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: ResignationRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new ResignationCoverageConflictError("coverage terminal page changed")
    return page
  }
}
