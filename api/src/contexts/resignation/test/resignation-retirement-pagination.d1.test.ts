...
import { createResignationPreservationFixture } from "@/contexts/resignation/test/create-resignation-preservation-fixture.test-support"
...
test("11件のresignation記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    clock,
    database,
    governance,
    ...
    tokenFor,
    request: apiRequest,
  } = await createResignationPreservationFixture(await pool.next())
  ...
  for (const id of resignationIds) {
    const index = resignationIds.indexOf(id) + 1
    await database
      .prepare(`INSERT INTO resignations
      (id,employee_id,resignation_date,last_working_date,reason,status,created_at)
  ...
  expect(
    (await post("/resignation/record-source-freezes", freezeId, { reason: "Preserve resignation" }))
      .status,
  ).toBe(201)
  ...
  const frozenUpdate = await apiRequest(frozenResignationPath, {
    method: "PUT",
    body: {
      resignation_date: "2031-01-01",
      last_working_date: null,
      reason: "Must not be updated",
    },
  })
  ...
  const mappings = []
  for (const id of resignationIds) {
    const path = `/resignation/resignations/${id}/preservation-requests`
  ...
  const sourcePath = `/resignation/record-source-freezes/${freezeId}`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["resignation-record"] })
  const retirementPath = `/resignation/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "resignation-retirement",
    revision: 1,
    title: "Retire preserved resignation",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/resignation/retirement-plans/${planId}/verification-receipts`
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved resignation",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_resignation_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_resignation_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('f1822927-1e0c-4285-8ee8-54facf47dadb','system:record:export'); DROP TABLE resignations;",
  )
