...
const secret = "resignation-integration-test-secret"

/** 実System・Companyとresignation原記録を接続する保全fixture。 */
export async function createResignationPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "resignation-preservation",
    revision: 1,
    title: "Preserve resignations",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "resignation-pagination-test-pepper",
