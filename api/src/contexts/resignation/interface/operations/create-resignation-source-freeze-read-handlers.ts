...
import { z } from "zod"
import { resignationSourceFreezeResponseSchema } from "@/contexts/resignation/interface/http/response-schemas"
...
/** 現在の管理権限でresignation記録の停止世代を読む。 */
export function createResignationSourceFreezeReadHandlers() {
  return resignationFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "resignation" ||
      ...
      return c.json(resignationSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
