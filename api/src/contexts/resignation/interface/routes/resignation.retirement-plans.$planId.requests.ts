import { zAppResignationRetirementRequest } from "@/contexts/resignation/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { resignationFactory } from "@/contexts/resignation/interface/request-environment/resignation-factory"
import { SubmitResignationRetirementRequestAdapter } from "@/contexts/resignation/infrastructure/adapters/submit-resignation-retirement-request.adapter"
import {
...
export const POST = resignationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitResignationRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof ResignationRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof ResignationRetirementConflictError)
    ...
    return c.json(
      zAppResignationRetirementRequest.parse(submitted.request),
