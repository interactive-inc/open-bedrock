...
import { RevalidateResignationRecordSourceAdapter } from "@/contexts/resignation/infrastructure/adapters/revalidate-resignation-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateResignationRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
