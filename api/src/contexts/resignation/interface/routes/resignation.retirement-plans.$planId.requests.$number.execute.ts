import { zAppResignationRetirementExecution } from "@/contexts/resignation/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { resignationFactory } from "@/contexts/resignation/interface/request-environment/resignation-factory"
import { ExecuteResignationRetirementAdapter } from "@/contexts/resignation/infrastructure/adapters/execute-resignation-retirement.adapter"
import {
...
export const POST = resignationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteResignationRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppResignationRetirementExecution.parse(submitted), 200)
  },
)
