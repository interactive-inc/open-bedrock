...
import { CreateResignationRetirementPlan } from "@/contexts/resignation/application/create-resignation-retirement-plan"
...
import { zAppResignationRetirementPlan } from "@/contexts/resignation/interface/http/response-schemas"
...
export const POST = resignationFactory.createHandlers(
  ...
  zValidator("json", resignationRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = resignationRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateResignationRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppResignationRetirementPlan.parse({
