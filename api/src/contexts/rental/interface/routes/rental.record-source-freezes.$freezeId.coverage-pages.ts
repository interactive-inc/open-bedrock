...
import { VerifyRentalReservationCoveragePage } from "@/contexts/rental/application/verify-rental-reservation-coverage-page"
import {
  RentalReservationCoverageForbiddenError,
  RentalReservationCoverageConflictError,
} from "@/contexts/rental/application/errors"
import { zAppRentalReservationCoveragePageReceipt } from "@/contexts/rental/interface/http/response-schemas"
...
export const POST = rentalReservationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = rentalReservationCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    return c.json(
      zAppRentalReservationCoveragePageReceipt.parse({
