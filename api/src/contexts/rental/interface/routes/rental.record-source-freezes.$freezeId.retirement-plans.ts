...
import { RentalReservationRetirementConflictError } from "@/contexts/rental/application/errors"
...
import { CreateRentalReservationRetirementPlan } from "@/contexts/rental/application/create-rental-reservation-retirement-plan"
import { rentalReservationRetirementPlanCommandSchema } from "@/contexts/rental/domain/schemas/rental-retirement-plan-command.schema"
import { zAppRentalReservationRetirementPlan } from "@/contexts/rental/interface/http/response-schemas"
...
export const POST = rentalReservationFactory.createHandlers(
  ...
  zValidator("json", rentalReservationRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = rentalReservationRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateRentalReservationRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppRentalReservationRetirementPlan.parse({
