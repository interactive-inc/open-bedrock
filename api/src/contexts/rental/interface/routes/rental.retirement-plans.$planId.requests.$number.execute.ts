import { zAppRentalReservationRetirementExecution } from "@/contexts/rental/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { rentalReservationFactory } from "@/contexts/rental/interface/request-environment/rental-factory"
import { ExecuteRentalReservationRetirementAdapter } from "@/contexts/rental/infrastructure/adapters/execute-rental-reservation-retirement.adapter"
import {
...
export const POST = rentalReservationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteRentalReservationRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppRentalReservationRetirementExecution.parse(submitted), 200)
  },
)
