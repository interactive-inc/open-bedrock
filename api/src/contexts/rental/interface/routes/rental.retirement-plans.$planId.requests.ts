import { zAppRentalReservationRetirementRequest } from "@/contexts/rental/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { rentalReservationFactory } from "@/contexts/rental/interface/request-environment/rental-factory"
import { SubmitRentalReservationRetirementRequestAdapter } from "@/contexts/rental/infrastructure/adapters/submit-rental-reservation-retirement-request.adapter"
import {
...
export const POST = rentalReservationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitRentalReservationRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof RentalReservationRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof RentalReservationRetirementConflictError)
    ...
    return c.json(
      zAppRentalReservationRetirementRequest.parse(submitted.request),
