...
import { seedRentalReservations } from "@/contexts/rental/infrastructure/seed/seed-rental-reservations.repository"
...
describe("GET /rental-reservations/admin", () => {
  test("returns 200 with all rental reservations for admin", async () => {
    const response = await request("/rental-reservations/admin", await tokenFor(1))
  ...
  test("returns 403 for a member", async () => {
    const response = await request("/rental-reservations/admin", await tokenFor(5))

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/rental-reservations/admin", null)

    expect(response.status).toBe(401)
  })

  test("filters by employee_id", async () => {
    const response = await request("/rental-reservations/admin?employee_id=2", await tokenFor(1))
