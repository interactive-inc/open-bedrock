...
describe("GET /rental-reservations/admin", () => {
  test("returns 200 with all rental reservations for admin", async () => {
    const response = await request("/rental/rental-reservations/admin", await tokenFor(1))
  ...
  test("returns 403 for a member", async () => {
    const response = await request("/rental/rental-reservations/admin", await tokenFor(5))

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/rental/rental-reservations/admin", null)
  ...
  test("filters by employee_id", async () => {
    const response = await request(
      `/rental/rental-reservations/admin?employee_id=${testEmployeeId(2)}`,
      await tokenFor(1),
    )
    ...
    if (parsed.success) {
      expect(
        parsed.data.data.every(
          (item) => item.requester_id === toWorkforceEmployeeId(testEmployeeId(2)),
        ),
      ).toBe(true)
