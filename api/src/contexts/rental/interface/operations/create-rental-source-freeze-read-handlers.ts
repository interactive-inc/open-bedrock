...
import { z } from "zod"
import { rentalReservationSourceFreezeResponseSchema } from "@/contexts/rental/interface/http/response-schemas"
...
/** 現在の管理権限でrental reservation記録の停止世代を読む。 */
export function createRentalReservationSourceFreezeReadHandlers() {
  return rentalReservationFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "rental" ||
      ...
      return c.json(rentalReservationSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
