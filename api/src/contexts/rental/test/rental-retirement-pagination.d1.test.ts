...
test("11件のrental reservation記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createRentalReservationPreservationFixture(await pool.next())
  const creator = zAccountId.parse(creatorPerson.accountId)
  const rentalReservationIds = Array.from(
    { length: 11 },
  ...
  expect(
    await database
      .prepare("SELECT count(*) AS total FROM rental_reservations")
      .first<number>("total"),
  ).toBe(11)
  const at = clock()
  ...
  expect(
    (await post("/rental/record-source-freezes", freezeId, { reason: "Preserve rental" })).status,
  ).toBe(201)
  ...
  expect(
    (
      await apiRequest(`/rental/rental-reservations/${rentalReservationIds[1]}/lend`, {
        method: "POST",
      })
    ).status,
  ).toBe(409)
  expect(
    (
      await apiRequest(`/rental/rental-reservations/${rentalReservationIds[2]}/return`, {
        method: "POST",
      })
    ).status,
  ).toBe(409)
  ...
  await execSql(database, "DROP TRIGGER fail_rental_reservation_retirement")
