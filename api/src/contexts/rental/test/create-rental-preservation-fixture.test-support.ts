...
const secret = "rental-integration-test-secret"

/** 実System・Companyとrental reservation原記録を接続する保全fixture。 */
export async function createRentalReservationPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "rental-preservation",
    revision: 1,
    title: "Preserve rentals",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "rental-pagination-test-pepper",
