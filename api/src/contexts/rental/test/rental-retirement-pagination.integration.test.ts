...
import { createRentalReservationPreservationFixture } from "@/contexts/rental/test/create-rental-preservation-fixture.test-support"
...
test("11件のrental reservation記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    database,
    governance,
    ...
    tokenFor,
    request: apiRequest,
  } = await createRentalReservationPreservationFixture()
  const creator = zAccountId.parse(creatorPerson.accountId)
  const rentalReservationIds = Array.from({ length: 11 }, (_, index) =>
  ...
  for (const id of rentalReservationIds) {
    const index = rentalReservationIds.indexOf(id) + 1
    await database
      .prepare(`INSERT INTO rental_reservations
      (id,requester_id,item_name,start_date,end_date,purpose,status,created_at)
      VALUES (?1,?2,?3,?4,?5,?6,?7,?8)`)
      .bind(
        id,
        creatorPerson.employeeId,
        `Rental item ${index}`,
        `2026-${String(index).padStart(2, "0")}-10`,
        `2026-${String(index).padStart(2, "0")}-20`,
        `Purpose ${index}`,
        index === 3 ? "lent" : "requested",
        `2026-${String(index).padStart(2, "0")}-01T01:00:00Z`,
      )
      .run()
  }
  expect(await database.prepare("SELECT count(*) AS total FROM rental_reservations").first<number>("total")).toBe(11)
  const at = new Date()
  ...
  expect(
    (await post("/rental/record-source-freezes", freezeId, { reason: "Preserve rental" }))
      .status,
  ).toBe(201)
  const frozenInsert = await database
    .prepare(`INSERT INTO rental_reservations
      (id,requester_id,item_name,start_date,end_date,purpose,status,created_at)
      VALUES (?1,?2,'Rental item','2030-01-01','2030-01-02',NULL,'requested','2030-01-01T00:00:00Z')`)
    .bind(crypto.randomUUID(), creatorPerson.employeeId)
    .run()
    .then(() => null, (error: unknown) => error)
  expect(frozenInsert).toBeInstanceOf(Error)
  const frozenRentalReservationPath = `/rental/rental-reservations/${rentalReservationIds[0]}`
  const frozenUpdate = await apiRequest(frozenRentalReservationPath, {
    method: "PUT",
    body: {
      item_name: "Must not be updated",
  ...
  expect((await apiRequest(frozenRentalReservationPath, { method: "DELETE" })).status).toBe(409)
  expect(
    (
      await apiRequest(
        `/rental/rental-reservations/${rentalReservationIds[1]}/lend`,
        { method: "POST" },
      )
    ).status,
  ).toBe(409)
  expect(
    (
      await apiRequest(
        `/rental/rental-reservations/${rentalReservationIds[2]}/return`,
        { method: "POST" },
      )
    ).status,
  ).toBe(409)
  const mappings = []
  for (const id of rentalReservationIds) {
    const path = `/rental/rental-reservations/${id}/preservation-requests`
  ...
  const sourcePath = `/rental/record-source-freezes/${freezeId}`
  ...
  expect(await firstCoverage.json()).toMatchObject({
    sequence: 1,
    nextCursor: rentalReservationIds[9],
    recordCount: 10,
  })
  expect(
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["rental-reservation-record"] })
  const retirementPath = `/rental/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "rental-retirement",
    revision: 1,
    title: "Retire preserved rental",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/rental/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM rental_reservations").first<number>("n"),
  ).toBe(11)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved rental",
  }
  expect(
  ...
  bindings.ATTACHMENT_KEKS = originalKeys
  await database.exec(
    "CREATE TRIGGER fail_rental_reservation_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await database.exec("DROP TRIGGER fail_rental_reservation_retirement")
