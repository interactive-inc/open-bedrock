import { PrepareRentalReservationRetirementPlanAdapter } from "@/contexts/rental/infrastructure/adapters/prepare-rental-reservation-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareRentalReservationRetirementPlanAdapter>[0]

/** rental reservation記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateRentalReservationRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareRentalReservationRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new RentalReservationRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RentalReservationRetirementConflictError("retirement plan creation conflicts")
  }
}
