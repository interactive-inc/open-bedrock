import { PrepareRentalReservationRetirementPageAdapter } from "@/contexts/rental/infrastructure/adapters/prepare-rental-reservation-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareRentalReservationRetirementPageAdapter>[0]

/** rental reservation記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyRentalReservationRetirementPage {
  ...
  async execute(
    ...
    const parsed = rentalReservationRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RentalReservationRetirementForbiddenError("retirement authentication required")
    const authority = await new RentalReservationRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new RentalReservationRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new RentalReservationRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new RentalReservationRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new RentalReservationRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new RentalReservationRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareRentalReservationRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new RentalReservationRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new RentalReservationRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new RentalReservationRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new RentalReservationRecordSystemAdapter(guardedContext)
      .coveragePages()
