...
import { seedResignations } from "@/contexts/company/infrastructure/seed/seed-resignations"
...
const jwtSecret = "resignation-accept-route-test-secret"
...
describe("POST /resignations/:id/accept", () => {
  test("returns 200 and accepts the resignation for hr", async () => {
    const response = await requestWithContext({
      db: await createTestDb(),
      jwtSecret,
      path: `/resignations/${seedId}/accept`,
  ...
  test("returns 409 when accepting an already-accepted resignation", async () => {
