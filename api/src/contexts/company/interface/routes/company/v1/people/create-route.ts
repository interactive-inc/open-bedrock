import { writePeople } from "@/contexts/company/application/people/write-people"
...
import { peopleCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/people-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", peopleCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writePeople(runtime.actor, change, (resourceChange) =>
