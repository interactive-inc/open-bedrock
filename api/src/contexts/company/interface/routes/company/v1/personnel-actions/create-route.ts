import { writePersonnelActions } from "@/contexts/company/application/personnel-actions/write-personnel-actions"
...
import { personnelActionsCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/personnel-actions-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", personnelActionsCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writePersonnelActions(runtime.actor, change, (resourceChange) =>
