import { writeCompanyDefinitions } from "@/contexts/company/application/definitions/write-company-definitions"
...
import { definitionsCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/definitions-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", definitionsCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writeCompanyDefinitions(runtime.actor, change, (resourceChange) =>
