import { writeAccountEmployeeLinks } from "@/contexts/company/application/account-employee-links/write-account-employee-links"
...
import { accountEmployeeLinksCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/account-employee-links-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", accountEmployeeLinksCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writeAccountEmployeeLinks(runtime.actor, change, (resourceChange) =>
