import { writeEmployees } from "@/contexts/company/application/employees/write-employees"
...
import { employeesCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/employees-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", employeesCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writeEmployees(runtime.actor, change, (resourceChange) =>
