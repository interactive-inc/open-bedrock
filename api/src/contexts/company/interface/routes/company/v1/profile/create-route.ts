import { writeCompanyProfile } from "@/contexts/company/application/profile/write-company-profile"
...
import { profileCompanyWriteRequestSchema } from "@/contexts/company/interface/http/requests/profile-company-write-request-schema"
...
export const POST = factory.createHandlers(
  ...
  zValidator("json", profileCompanyWriteRequestSchema, (result, context) => {
    if (!result.success) {
      return createCompanyProblem(
        context,
        422,
  ...
  async (context) => {
    ...
    const result = await writeCompanyProfile(runtime.actor, change, (resourceChange) =>
