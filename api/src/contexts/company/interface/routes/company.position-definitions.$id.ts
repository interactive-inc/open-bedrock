/** /company/position-definitions/:id */
import { DeletePosition } from "@/contexts/company/application/definitions/delete-position"
import { UpdatePosition } from "@/contexts/company/application/definitions/update-position"
...
// @authorization permission - master:org:writeで役職定義を更新する
export const PUT = factory.createHandlers(
  ...
  async (context) => {
    ...
    const result = await new UpdatePosition({
      actor,
...
// @authorization permission - master:org:writeで未使用の役職定義を削除する
export const DELETE = factory.createHandlers(
  ...
  async (context) => {
    ...
    if (actor === undefined) throw new CompanyAuthenticationRequiredError()
    if (context.env.DB === undefined) throw new CompanyDatabaseUnavailableError()
    const result = await new DeletePosition({
      actor,
      repository: new PositionRepository({
        env: { DB: context.env.DB },
        var: { database: context.var.database, auditContext: context.var.auditContext },
