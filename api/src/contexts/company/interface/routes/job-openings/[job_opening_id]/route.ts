import { UpdatePosition } from "@/contexts/company/application/recruitment/update-position"
...
/** GET /job-openings/:job_opening_id — 募集ポジションの詳細（recruitment:manage）。 */
export const GET = factory.createHandlers(verifyBearer, async (c) => {
  ...
  const id = validateIntParam(c.req.param("job_opening_id"), "job opening")
...
/** PUT /job-openings/:job_opening_id — 募集ポジションの内容・状態を更新（recruitment:manage）。 */
export const PUT = factory.createHandlers(
  ...
  async (c) => {
    ...
    const updated = await new UpdatePosition(c).run({
      session,
      id: validateIntParam(c.req.param("job_opening_id"), "job opening"),
      title: json.title,
