...
describe("POST /application-requests/:id/reject", () => {
  test("returns 200 and flips status to rejected", async () => {
    const response = await request("/application-requests/1/reject", await tokenFor(2, "manager"), {
      method: "POST",
  ...
  test("returns 404 for an unknown id", async () => {
    const response = await request(
      "/application-requests/9999/reject",
  ...
  test("returns 401 without a bearer token", async () => {
    const response = await request("/application-requests/1/reject", null, {
      method: "POST",
      body: { comment: "sending back" },
    })
  ...
  test("returns 403 for a non-privileged role", async () => {
    const response = await request("/application-requests/1/reject", await tokenFor(5, "member"), {
      method: "POST",
      body: { comment: "sending back" },
    })
