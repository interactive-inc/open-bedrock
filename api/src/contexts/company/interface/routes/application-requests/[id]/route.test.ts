...
describe("GET /application-requests/:id", () => {
  ...
  test("returns 403 for a non-owner non-privileged role (no ID-scan leakage)", async () => {
    const response = await request("/application-requests/1", await tokenFor(99, "member"))

    expect(response.status).toBe(403)
  })

  test("returns 200 for a privileged non-owner (approver can view)", async () => {
    const response = await request("/application-requests/1", await tokenFor(4, "manager"))
