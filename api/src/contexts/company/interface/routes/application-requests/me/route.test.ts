...
describe("GET /application-requests/me", () => {
  test("returns only the applicant's own applications", async () => {
    const response = await request("/application-requests/me", await tokenFor(5, "member"))
  ...
  test("returns an empty array when the applicant has no applications", async () => {
    const response = await request("/application-requests/me", await tokenFor(1, "member"))
  ...
  test("returns 401 without a bearer token", async () => {
    const response = await request("/application-requests/me", null)
