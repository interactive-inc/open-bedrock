...
/** 公開責務の全revisionを同じ属性・日付契約で復元する。 */
export class CompanyResponsibilityResourceHistoryAdapter {
  ...
  async read(
    ...
    const rows = await this.c
      ...
      FROM company_resource_revisions WHERE organization_id = ?1 AND resource_type = 'responsibility-assignment' AND resource_id = ?2
      ORDER BY revision`)
      .bind(resource.organizationId, resource.id)
      .all()
    if (!rows.success) return new Error("failed to read responsibility resource history")
    const parsed = z
      .array(
        z.object({
