...
export class RoomActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new RoomError("forbidden", "invalid room actor")
    ...
    if (authorization instanceof Error)
      return new RoomError("room_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new RoomError("room_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new RoomError("forbidden", "active employee is required")
    return {
