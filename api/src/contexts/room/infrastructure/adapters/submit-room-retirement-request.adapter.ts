...
import { roomRetirementSubmissionCommandSchema } from "@/contexts/room/domain/schemas/room-retirement-submission-command.schema"
import { PrepareRoomRetirementCurrentStateAdapter } from "@/contexts/room/infrastructure/adapters/prepare-room-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareRoomRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitRoomRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = roomRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RoomRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareRoomRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new RoomRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "room.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new RoomRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new RoomRetirementForbiddenError("retirement applicant differs")
      if (
        ...
        return new RoomRetirementConflictError("retirement previous version is not resubmittable")
      ...
      if (
        ...
        return new RoomRetirementConflictError(
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new RoomRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareRoomRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new RoomRetirementForbiddenError("retirement decision qualification required")
    ...
    if (started instanceof Error) {
      ...
      return new RoomRetirementConflictError("retirement submission changed before persistence", {
        cause: started,
      })
    }
    return {
