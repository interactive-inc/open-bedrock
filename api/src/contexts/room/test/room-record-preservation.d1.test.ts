...
test("会議室2台帳を停止中に人の承認で保全し、業務コードを外してもSystemから読める", async () => {
  const { database, creator, reviewer, definition, bindings, tokenFor, request } =
    await createRoomPreservationFixture(await pool.next())
  ...
  const frozen = await app.request(
    "/room/record-source-freezes",
    {
      ...
      body: JSON.stringify({ reason: "Retain room evidence" }),
    },
    bindings,
  )
  ...
  for (const [recordKind, recordId] of sources) {
    const path = `/room/records/${recordKind}/${recordId}/preservation-requests`
    const submitted = await request(path, {
      ...
      body: {
        ...
        conditions: {
          reason: "Retain original room record",
