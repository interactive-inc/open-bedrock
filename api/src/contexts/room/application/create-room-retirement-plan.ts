...
type Context = ConstructorParameters<typeof PrepareRoomRetirementPlanAdapter>[0]

/** room記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateRoomRetirementPlan {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new RoomRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RoomRetirementConflictError("retirement plan creation conflicts")
  }
}
