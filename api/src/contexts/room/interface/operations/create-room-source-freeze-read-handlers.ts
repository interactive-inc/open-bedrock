...
import { z } from "zod"
import { roomSourceFreezeResponseSchema } from "@/contexts/room/interface/http/response-schemas"
...
/** 現在の管理権限でroom記録の停止世代を読む。 */
export function createRoomSourceFreezeReadHandlers() {
  return roomFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "room" ||
      ...
      return c.json(roomSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
