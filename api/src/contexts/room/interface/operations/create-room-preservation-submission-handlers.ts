...
export function createRoomPreservationSubmissionHandlers(mode: "create" | "resubmit") {
  ...
  return roomFactory.createHandlers(
    zValidator("param", roomRecordRouteSchema),
    ...
    async (c) => {
      ...
      const adapterContext: Parameters<typeof submitSystemRecordPreservation>[0] = {
        ...
        source: {
          ...
          recordId,
          sourceNamespace,
          authorize: () => new RoomActorReadAdapter(c).prepare(),
          capture: () =>
            new CaptureRoomRecordAdapter(c).prepare({
              recordKind,
              recordId,
              sourceNamespace,
            }),
      ...
      if (submitted instanceof RecordPreservationSubmissionError) {
        switch (submitted.code) {
          case "invalid":
            throw new RoomInputError({ message: submitted.message })
          case "forbidden":
