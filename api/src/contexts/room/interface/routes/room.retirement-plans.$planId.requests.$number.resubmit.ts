import { zAppRoomRetirementRequest } from "@/contexts/room/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { roomFactory } from "@/contexts/room/interface/request-environment/room-factory"
import { SubmitRoomRetirementRequestAdapter } from "@/contexts/room/infrastructure/adapters/submit-room-retirement-request.adapter"
import {
...
export const POST = roomFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitRoomRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof RoomRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof RoomRetirementConflictError)
    ...
    return c.json(zAppRoomRetirementRequest.parse(submitted.request), submitted.created ? 201 : 200)
  },
)
