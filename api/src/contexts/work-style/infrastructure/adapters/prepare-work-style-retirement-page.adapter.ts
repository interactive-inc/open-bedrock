...
type Context = EmployeeWorkStyleContext & SystemAttachmentStorageContext & SystemDatabaseContext
const requestSchema = z.strictObject({
  freezeId: z.uuid(),
  sourceNamespace: z.string().regex(/^\S{1,255}$/),
  purpose: z.string().trim().min(1).max(255),
  recordKind: z.literal("employee-work-style-record"),
  sequence: z.number().int().positive().safe(),
  terminalDigest: z.string().regex(/^[0-9a-f]{64}$/),
})

/** 計画に固定した勤務形態記録ページの原文・保全本文・保持条件を再検証し、保存条件を渡す。 */
export class PrepareEmployeeWorkStyleRetirementPageAdapter {
  ...
  async prepare(input: unknown, stepUpToken: string) {
    ...
    const captured = await new CaptureFrozenEmployeeWorkStyleRecordPageAdapter(this.c).prepare({
      freezeId: request.freezeId,
      sourceNamespace: request.sourceNamespace,
      afterId: cursor,
      limit: 10,
    ...
    const current = await new EmployeeWorkStyleActorReadAdapter(this.c).prepare()
