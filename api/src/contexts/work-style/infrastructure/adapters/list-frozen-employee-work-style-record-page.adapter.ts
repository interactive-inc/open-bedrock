...
/** 未解除の停止世代から勤務形態記録IDを分割取得し、後続の保存でも同じ世代を検査する。 */
export class ListFrozenEmployeeWorkStyleRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    try {
      const page =
        request.afterId === null
          ? this.c.env.DB.prepare(
              "SELECT id FROM employee_work_styles ORDER BY id COLLATE BINARY LIMIT ?1",
            ).bind(request.limit + 1)
          : this.c.env.DB.prepare(
              "SELECT id FROM employee_work_styles WHERE id COLLATE BINARY>?1 ORDER BY id COLLATE BINARY LIMIT ?2",
      ...
      if (reads.length !== statements.length || reads.some((read) => !read.success))
        return new Error("frozen work-style inventory unavailable")
      ...
      return new Error("frozen work-style inventory unavailable", { cause })
    }
  }
}
