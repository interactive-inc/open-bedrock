...
/** 勤務形態記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareEmployeeWorkStyleRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = employeeWorkStyleRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new EmployeeWorkStyleActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new EmployeeWorkStyleActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "work-style",
        capability: { revision: 1, recordKinds: ["employee-work-style-record"] },
        coverage: [
          {
