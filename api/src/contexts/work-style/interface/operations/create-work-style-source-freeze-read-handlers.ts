...
import { z } from "zod"
import { employeeWorkStyleSourceFreezeResponseSchema } from "@/contexts/work-style/interface/http/response-schemas"
...
/** 現在の管理権限で勤務形態記録の停止世代を読む。 */
export function createEmployeeWorkStyleSourceFreezeReadHandlers() {
  return employeeWorkStyleFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "work-style" ||
      ...
      return c.json(employeeWorkStyleSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
