...
import { CreateEmployeeWorkStyleRetirementPlan } from "@/contexts/work-style/application/create-employee-work-style-retirement-plan"
import { employeeWorkStyleRetirementPlanCommandSchema } from "@/contexts/work-style/domain/schemas/work-style-retirement-plan-command.schema"
import { zAppEmployeeWorkStyleRetirementPlan } from "@/contexts/work-style/interface/http/response-schemas"
...
export const POST = employeeWorkStyleFactory.createHandlers(
  ...
  zValidator("json", employeeWorkStyleRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = employeeWorkStyleRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateEmployeeWorkStyleRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppEmployeeWorkStyleRetirementPlan.parse({
