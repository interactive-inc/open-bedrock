...
export class VerifyEmployeeWorkStyleCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = employeeWorkStyleCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new EmployeeWorkStyleCoverageForbiddenError("coverage authentication required")
    const proof = await new EmployeeWorkStyleRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new EmployeeWorkStyleCoverageForbiddenError("coverage authorization denied")
    const generation = await new EmployeeWorkStyleRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new EmployeeWorkStyleRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new EmployeeWorkStyleRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "employee-work-style-record")
    )
      return new EmployeeWorkStyleCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new EmployeeWorkStyleCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenEmployeeWorkStyleRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      afterId: cursor,
      limit: 10,
    ...
    if (
      ...
      return new EmployeeWorkStyleCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareEmployeeWorkStyleCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new EmployeeWorkStyleCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new EmployeeWorkStyleCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: EmployeeWorkStyleRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new EmployeeWorkStyleCoverageConflictError("coverage terminal page changed")
    return page
  }
}
