import { PrepareEmployeeWorkStyleRetirementPageAdapter } from "@/contexts/work-style/infrastructure/adapters/prepare-work-style-retirement-page.adapter"
import { EmployeeWorkStyleRecordSystemAdapter } from "@/contexts/work-style/infrastructure/adapters/employee-work-style-record-system.adapter"
import { employeeWorkStyleRetirementVerificationCommandSchema } from "@/contexts/work-style/domain/schemas/work-style-retirement-verification-command.schema"
import {
...
type Context = ConstructorParameters<typeof PrepareEmployeeWorkStyleRetirementPageAdapter>[0]

/** 勤務形態記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyEmployeeWorkStyleRetirementPage {
  ...
  async execute(
    ...
    const parsed = employeeWorkStyleRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new EmployeeWorkStyleRetirementForbiddenError("retirement authentication required")
    const authority = await new EmployeeWorkStyleRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new EmployeeWorkStyleRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new EmployeeWorkStyleRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new EmployeeWorkStyleRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new EmployeeWorkStyleRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new EmployeeWorkStyleRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareEmployeeWorkStyleRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new EmployeeWorkStyleRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new EmployeeWorkStyleRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new EmployeeWorkStyleRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new EmployeeWorkStyleRecordSystemAdapter(guardedContext)
      .coveragePages()
