import { PrepareEmployeeWorkStyleRetirementPlanAdapter } from "@/contexts/work-style/infrastructure/adapters/prepare-work-style-retirement-plan.adapter"
...
import { EmployeeWorkStyleRetirementConflictError } from "@/contexts/work-style/application/errors"

type Context = ConstructorParameters<typeof PrepareEmployeeWorkStyleRetirementPlanAdapter>[0]

/** 勤務形態記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateEmployeeWorkStyleRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareEmployeeWorkStyleRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new EmployeeWorkStyleRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new EmployeeWorkStyleRetirementConflictError("retirement plan creation conflicts")
  }
}
