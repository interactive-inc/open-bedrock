...
function requireBinding(input: unknown): IdentityBinding {
  const binding = IdentityBinding.create(input)
  expect(binding).toBeInstanceOf(IdentityBinding)
...
describe("IdentityBinding", () => {
  ...
  test("activationとrevocationから状態を導出し、履歴時点の有効性を判定する", () => {
    ...
    expect(active).toBeInstanceOf(IdentityBinding)
    ...
    expect(revoked).toBeInstanceOf(IdentityBinding)
