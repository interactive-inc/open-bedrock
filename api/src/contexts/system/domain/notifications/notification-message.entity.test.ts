...
describe("NotificationMessage", () => {
  test("recipientとread stateを持たないimmutableなplain-text messageを作る", () => {
    ...
    const message = NotificationMessage.create({ ...validProps, source, createdAt })

    expect(message).toBeInstanceOf(NotificationMessage)
    if (!(message instanceof NotificationMessage)) return
  ...
  test("bodyとsourceを持たないmessageも明示的なnullで表す", () => {
    const message = NotificationMessage.create({ ...validProps, body: null, source: null })
  ...
  test.each([
    ...
    const result = NotificationMessage.create(input)
