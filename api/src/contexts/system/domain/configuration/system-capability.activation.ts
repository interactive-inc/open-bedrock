...
} from "@system/domain/configuration/system-capability.catalog"
import {
  InvalidSystemCapabilityActivationError,
  type SystemCapabilityActivationProblem,
} from "@system/domain/configuration/invalid-system-capability-activation.error"
...
export class SystemCapabilityActivation {
  ...
  static create(
    input: unknown,
  ): SystemCapabilityActivation | InvalidSystemCapabilityActivationError {
    if (!isProps(input)) {
    ...
    const implementedCapabilities = collectCapabilities(
      input.implementedCapabilities,
      "implemented",
      problems,
    )
    const enabledCapabilities = collectCapabilities(input.enabledCapabilities, "enabled", problems)
    ...
    return new SystemCapabilityActivation({ implementedCapabilities, enabledCapabilities })
  }
...
function isProps(input: unknown): input is Props {
