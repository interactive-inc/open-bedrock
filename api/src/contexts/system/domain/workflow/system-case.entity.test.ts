...
function requireCase(input: unknown): SystemCase {
  const systemCase = SystemCase.create(input)
  expect(systemCase).toBeInstanceOf(SystemCase)
...
describe("SystemCase", () => {
  ...
  test("差戻しを終端判断として記録し、同じ案件の再決定を拒否する", () => {
    ...
    expect(returned).toBeInstanceOf(SystemCase)
