...
function requireAuthorization(input: unknown): ExecutionAuthorization {
  const authorization = ExecutionAuthorization.create(input)
  expect(authorization).toBeInstanceOf(ExecutionAuthorization)
...
describe("ExecutionAuthorization", () => {
  test("承認済みdigestへ期限内に一度だけ使用できる", () => {
    ...
    expect(used).toBeInstanceOf(ExecutionAuthorization)
