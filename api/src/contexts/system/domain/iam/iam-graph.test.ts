import { IamGraph } from "@system/domain/iam/iam-graph"
...
function requireRole(id: string, permissionKeys: ReadonlyArray<string>): IamRole {
...
function requireGraph(
  ...
  const graph = IamGraph.create(roles, bindings)
  expect(graph).toBeInstanceOf(IamGraph)
  if (graph instanceof Error) throw graph
  return graph
}

describe("IamGraph permission evaluation", () => {
...
describe("IamGraph integrity and last-root", () => {
