import { CancelSystemWorkItem } from "@system/application/work/cancel-system-work-item"
import { cancelSystemWorkItemSchema } from "@system/domain/schemas/work/system-work-item.schema"
...
// @authorization service - 責任者が作業の中止を記録する。
export const POST = systemFactory.createHandlers(
  authenticateSystemAccessToken,
  zValidator("param", z.object({ id: z.uuid() })),
  requireSystemStepUp,
  zValidator("header", z.object({ "x-system-step-up": z.string().regex(/^[0-9a-f]{64}$/) })),
  zValidator("json", cancelSystemWorkItemSchema.omit({ kind: true, id: true })),
  async (context) => {
    ...
    const saved = await new CancelSystemWorkItem(prepared).execute({
