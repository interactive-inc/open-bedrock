import { DeclineSystemWorkHandover } from "@system/application/work/decline-system-work-handover"
import { declineSystemWorkHandoverSchema } from "@system/domain/schemas/work/system-work-item.schema"
...
// @authorization service - 引き継ぎ先が受領を辞退する。
export const POST = systemFactory.createHandlers(
  authenticateSystemAccessToken,
  zValidator("param", z.object({ id: z.uuid() })),
  requireSystemStepUp,
  zValidator("header", z.object({ "x-system-step-up": z.string().regex(/^[0-9a-f]{64}$/) })),
  zValidator("json", declineSystemWorkHandoverSchema.omit({ kind: true, id: true })),
  async (context) => {
    ...
    const saved = await new DeclineSystemWorkHandover(prepared).execute({
      ...context.req.valid("json"),
      kind: "decline_handover",
