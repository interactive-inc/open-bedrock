import { ReturnSystemWorkResult } from "@system/application/work/return-system-work-result"
import { returnSystemWorkResultSchema } from "@system/domain/schemas/work/system-work-item.schema"
...
// @authorization service - 責任者が成果の修正を求める。
export const POST = systemFactory.createHandlers(
  authenticateSystemAccessToken,
  zValidator("param", z.object({ id: z.uuid() })),
  requireSystemStepUp,
  zValidator("header", z.object({ "x-system-step-up": z.string().regex(/^[0-9a-f]{64}$/) })),
  zValidator("json", returnSystemWorkResultSchema.omit({ kind: true, id: true })),
  async (context) => {
    ...
    const saved = await new ReturnSystemWorkResult(prepared).execute({
