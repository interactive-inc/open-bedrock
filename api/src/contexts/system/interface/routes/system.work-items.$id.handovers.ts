import { RequestSystemWorkHandover } from "@system/application/work/request-system-work-handover"
...
// @authorization service - 受領が必要な責任の引き継ぎを依頼する。
export const POST = systemFactory.createHandlers(
  authenticateSystemAccessToken,
  zValidator("param", z.object({ id: z.uuid() })),
  requireSystemStepUp,
  zValidator("header", z.object({ "x-system-step-up": z.string().regex(/^[0-9a-f]{64}$/) })),
  zValidator("json", requestSystemWorkHandoverSchema.omit({ kind: true, id: true })),
  async (context) => {
    ...
    const saved = await new RequestSystemWorkHandover(prepared).execute({
