...
describe("findOidcAccessToken", () => {
  test("issuerと有効期限が一致するtokenだけを返す", async () => {
    ...
    const issued = await createOidcAccessToken(
    ...
    expect(
      await findOidcAccessToken(
        { var: { database, now: () => new Date("2026-01-01T00:04:59.999Z") } },
        { issuer: "https://secondary.identity.example.test", accessToken: issued.accessToken },
      ),
    ).toBeNull()
    expect(
      await findOidcAccessToken(
        { var: { database, now: () => new Date("2026-01-01T00:04:59.999Z") } },
        { issuer: "https://identity.example.test", accessToken: issued.accessToken },
      ),
