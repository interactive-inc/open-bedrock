...
export class SystemRoleAdministrationRepository {
  ...
  async create(
    ...
    try {
      const statements = [
        ...
        ).bind(role.id, role.key, role.name, role.description, role.createdAt.getTime()),
  ...
  async update(
    ...
    try {
      const statements = [
        ...
        this.context.env.DB.prepare(
          "DELETE FROM system_iam_role_permissions WHERE role_id = ?1",
        ).bind(previous.id),
