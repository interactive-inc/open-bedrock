...
function kekOf(version: number) {
  const registry = AttachmentKekRegistry.fromEnv(testKekEnv(version))
...
describe("AttachmentKekRegistry", () => {
  ...
  test("最大 version を現行鍵にし、旧 version も解決できる", () => {
    const registry = AttachmentKekRegistry.fromEnv(
      JSON.stringify({
        1: JSON.parse(testKekEnv(1))["1"],
        2: JSON.parse(testKekEnv(2))["2"],
      }),
    )
  ...
  test("設定に無い version は fail-closed で拒否する", () => {
    const registry = AttachmentKekRegistry.fromEnv(testKekEnv(1))
  ...
  test("base64 の鍵を 32 バイトへ復元する", () => {
    ...
    expect(fromBase64(JSON.parse(testKekEnv(1))["1"]).byteLength).toBe(32)
  })
})
