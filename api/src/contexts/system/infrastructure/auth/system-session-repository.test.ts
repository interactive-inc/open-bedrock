...
function createRotation(): SessionRotation {
  ...
  const rotation = SessionRotation.create(previous, successor, rotatedAt)
...
function createRotationAudits(rotation: SessionRotation): SessionRotationAuditEvents {
...
function insertSession(fixture: SystemSessionTestContext, session: Session): void {
  fixture.sqlite
    .query(
...
describe("SystemSessionRepository", () => {
  test("active AccountへhashだけのSessionと監査を同じtransactionで発行する", async () => {
    ...
    expect(stored).toBeInstanceOf(Session)
    expect(stored instanceof Session ? stored.id : null).toBe(session.id)
    expect(stored instanceof Session ? stored.tokenHash : null).toBe(session.tokenHash)
  ...
  test.each([
    ...
    "Account security stateが一致しない発行を監査ごとrollbackする",
  ...
  test("監査insertが黙殺された発行をSessionごとrollbackする", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
    fixture.sqlite.exec(`
  ...
  test("一度だけ旧Sessionを消費し後継Sessionと成功監査を原子的に作る", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
  ...
  test("同じ旧Sessionの並行後発をreuseとしてfamily全体とともに失効する", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
    const rotation = createRotation()
    insertSession(fixture, createSession({ id: "session-1", tokenHash, createdAt, expiresAt }))
  ...
  test("Account token version変更後のrotationをinvalidとしてfamily全体ごと失効する", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
    const rotation = createRotation()
    insertSession(fixture, createSession({ id: "session-1", tokenHash, createdAt, expiresAt }))
  ...
  test("複数の未消費Sessionがある壊れたfamilyをinvalidとしてfail closedに失効する", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
    const rotation = createRotation()
    insertSession(fixture, createSession({ id: "session-1", tokenHash, createdAt, expiresAt }))
  ...
  test("rotation監査insertが黙殺された場合はSession mutationも残さない", async () => {
    const fixture = new SystemSessionTestContext()
    insertAccount(fixture)
    const rotation = createRotation()
    insertSession(fixture, createSession({ id: "session-1", tokenHash, createdAt, expiresAt }))
