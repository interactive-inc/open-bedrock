import { zAppSurveyRetirementExecution } from "@/contexts/survey/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { surveyFactory } from "@/contexts/survey/interface/request-environment/survey-factory"
import { ExecuteSurveyRetirementAdapter } from "@/contexts/survey/infrastructure/adapters/execute-survey-retirement.adapter"
import {
...
export const POST = surveyFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteSurveyRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppSurveyRetirementExecution.parse(submitted), 200)
  },
)
