import { zAppSurveyRetirementRequest } from "@/contexts/survey/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { surveyFactory } from "@/contexts/survey/interface/request-environment/survey-factory"
import { SubmitSurveyRetirementRequestAdapter } from "@/contexts/survey/infrastructure/adapters/submit-survey-retirement-request.adapter"
import {
...
export const POST = surveyFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitSurveyRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof SurveyRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof SurveyRetirementConflictError)
    ...
    return c.json(
      zAppSurveyRetirementRequest.parse(submitted.request),
