...
const secret = "survey-integration-test-secret"

/** 実System・Companyとアンケート本体・回答の原記録を接続する保全fixture。 */
export async function createSurveyPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "survey-preservation",
    revision: 1,
    title: "Preserve survey records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "survey-pagination-test-pepper",
