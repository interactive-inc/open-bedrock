...
export class SurveyRepository {
  ...
  async deleteWithResponses(survey: Survey): Promise<true | SurveyDeleteConflict | Error> {
    ...
    try {
      ...
      if (isAbortedByGuard(error)) {
        const current = await this.findById(survey.id)
        if (current instanceof Error) return current
        if (current === null) return { reason: "not_found" }
        return current.isOpen() ? { reason: "not_deletable" } : { reason: "not_found" }
      }
      return error instanceof Error ? error : new Error("failed to delete survey")
    }
  }
