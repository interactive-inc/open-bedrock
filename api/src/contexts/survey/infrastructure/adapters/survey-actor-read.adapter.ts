...
export class SurveyActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new SurveyError("forbidden", "invalid survey actor")
    ...
    if (authorization instanceof Error)
      return new SurveyError("survey_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new SurveyError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new SurveyError("survey_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new SurveyError("survey_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new SurveyError("forbidden", "active employee is required")
    return {
