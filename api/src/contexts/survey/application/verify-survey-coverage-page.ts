...
export class VerifySurveyCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = surveyCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new SurveyCoverageForbiddenError("coverage authentication required")
    const proof = await new SurveyRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new SurveyCoverageForbiddenError("coverage authorization denied")
    const generation = await new SurveyRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new SurveyRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      return new SurveyCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new SurveyCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenSurveyRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new SurveyCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareSurveyCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new SurveyCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new SurveyCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: SurveyRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new SurveyCoverageConflictError("coverage terminal page changed")
    return page
  }
}
