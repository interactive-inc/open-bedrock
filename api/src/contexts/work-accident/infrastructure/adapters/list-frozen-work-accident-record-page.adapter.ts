      ...
      const page =
        request.afterId === null
          ? this.c.env.DB.prepare(
              "SELECT id FROM work_accidents ORDER BY id COLLATE BINARY LIMIT ?1",
            ).bind(request.limit + 1)
          : this.c.env.DB.prepare(
              "SELECT id FROM work_accidents WHERE id COLLATE BINARY>?1 ORDER BY id COLLATE BINARY LIMIT ?2",
