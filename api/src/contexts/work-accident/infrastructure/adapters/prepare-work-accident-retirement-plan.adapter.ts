...
/** 労災・事故記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareWorkAccidentRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = workAccidentRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new WorkAccidentActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new WorkAccidentActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "work-accident",
        capability: { revision: 1, recordKinds: ["work-accident-record"] },
        coverage: [
          {
