...
export class WorkAccidentActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new WorkAccidentError("forbidden", "invalid work-accident actor")
    const authorization = await new SystemHumanOperationAuthorizationAdapter(this.c).prepare({
      ...
      permissions: ["work_accident:manage"],
      now,
    })
    if (authorization instanceof Error)
      return new WorkAccidentError("work_accident_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new WorkAccidentError("forbidden", "human work-accident manager is required")
    ...
    if (guard instanceof Error)
      return new WorkAccidentError("work_accident_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new WorkAccidentError("work_accident_unavailable", "Company directory is unavailable")
    return {
