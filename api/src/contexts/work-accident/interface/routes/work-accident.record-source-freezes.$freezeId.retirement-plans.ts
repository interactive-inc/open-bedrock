...
import { CreateWorkAccidentRetirementPlan } from "@/contexts/work-accident/application/create-work-accident-retirement-plan"
import { workAccidentRetirementPlanCommandSchema } from "@/contexts/work-accident/domain/schemas/work-accident-retirement-plan-command.schema"
import { zAppWorkAccidentRetirementPlan } from "@/contexts/work-accident/interface/http/response-schemas"
...
export const POST = workAccidentFactory.createHandlers(
  ...
  zValidator("json", workAccidentRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = workAccidentRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateWorkAccidentRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppWorkAccidentRetirementPlan.parse({
