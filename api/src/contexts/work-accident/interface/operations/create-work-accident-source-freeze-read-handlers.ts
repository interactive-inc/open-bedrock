...
import { z } from "zod"
import { workAccidentSourceFreezeResponseSchema } from "@/contexts/work-accident/interface/http/response-schemas"
...
/** 現在の管理権限で労災・事故記録の停止世代を読む。 */
export function createWorkAccidentSourceFreezeReadHandlers() {
  return workAccidentFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "work-accident" ||
      ...
      return c.json(workAccidentSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
