...
import { itIncidentRetirementSubmissionCommandSchema } from "@/contexts/it-incident/domain/schemas/it-incident-retirement-submission-command.schema"
import { PrepareItIncidentRetirementCurrentStateAdapter } from "@/contexts/it-incident/infrastructure/adapters/prepare-it-incident-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareItIncidentRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitItIncidentRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = itIncidentRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new ItIncidentRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareItIncidentRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new ItIncidentRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "it-incident.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new ItIncidentRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new ItIncidentRetirementForbiddenError("retirement applicant differs")
      if (
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new ItIncidentRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareItIncidentRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
