...
import { VerifyItIncidentCoveragePage } from "@/contexts/it-incident/application/verify-it-incident-coverage-page"
import {
  ItIncidentCoverageForbiddenError,
  ItIncidentCoverageConflictError,
} from "@/contexts/it-incident/application/errors"
import { zAppItIncidentCoveragePageReceipt } from "@/contexts/it-incident/interface/http/response-schemas"
...
export const POST = itIncidentFactory.createHandlers(
  ...
  zValidator("json", itIncidentCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = itIncidentCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyItIncidentCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof ItIncidentCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof ItIncidentCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppItIncidentCoveragePageReceipt.parse({
