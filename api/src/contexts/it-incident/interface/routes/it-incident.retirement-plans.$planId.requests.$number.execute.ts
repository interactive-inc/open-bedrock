import { zAppItIncidentRetirementExecution } from "@/contexts/it-incident/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { itIncidentFactory } from "@/contexts/it-incident/interface/request-environment/it-incident-factory"
import { ExecuteItIncidentRetirementAdapter } from "@/contexts/it-incident/infrastructure/adapters/execute-it-incident-retirement.adapter"
import {
...
export const POST = itIncidentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteItIncidentRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppItIncidentRetirementExecution.parse(submitted), 200)
  },
)
