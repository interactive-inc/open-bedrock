...
/** partner記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyPartnerRetirementPage {
  ...
  async execute(
    ...
    const parsed = partnerRetirementVerificationCommandSchema.safeParse(input)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "partner" ||
    ...
    if (
      ...
      return new PartnerRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new PartnerRetirementConflictError("all plan pages already verified")
    const checked = await new PreparePartnerRetirementPageAdapter(this.c).prepare(
      {
    ...
    return raced !== null && matches(raced)
      ? raced
      : new PartnerRetirementConflictError("retirement verification position already written")
  }
}
