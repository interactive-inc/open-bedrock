...
import {
  PartnerCoverageForbiddenError,
  PartnerCoverageConflictError,
} from "@/contexts/partner/application/errors"
import { partnerCoveragePageCommandSchema } from "@/contexts/partner/domain/schemas/partner-coverage-page-command.schema"
...
import { CaptureFrozenPartnerRecordPageAdapter } from "@/contexts/partner/infrastructure/adapters/capture-frozen-partner-record-page.adapter"
import { PreparePartnerCoverageRecordsAdapter } from "@/contexts/partner/infrastructure/adapters/prepare-partner-coverage-records.adapter"

type Context = PartnerContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからpartner記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyPartnerCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = partnerCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new PartnerCoverageForbiddenError("coverage authentication required")
    const proof = await new PartnerRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new PartnerCoverageForbiddenError("coverage authorization denied")
    const generation = await new PartnerRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new PartnerRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      return new PartnerCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new PartnerCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenPartnerRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new PartnerCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PreparePartnerCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new PartnerCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new PartnerCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: PartnerRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new PartnerCoverageConflictError("coverage terminal page changed")
    return page
  }
}
