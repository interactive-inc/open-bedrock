...
type Context = PartnerContext & SystemAttachmentStorageContext & SystemDatabaseContext
...
/** 計画に固定したpartner記録ページの原文・保全本文・保持条件を再検証し、保存条件を渡す。 */
export class PreparePartnerRetirementPageAdapter {
  ...
  async prepare(input: unknown, stepUpToken: string) {
    ...
    const captured = await new CaptureFrozenPartnerRecordPageAdapter(this.c).prepare({
      freezeId: request.freezeId,
      sourceNamespace: request.sourceNamespace,
      recordKind: request.recordKind,
      afterCursor: cursor,
    ...
    const current = await new PartnerActorReadAdapter(this.c).prepare()
