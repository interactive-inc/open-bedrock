...
const tableByKind = {
  ...
  "partner-contract-record": "partner_contracts",
} as const

/** 未解除の停止世代から指定した取引先台帳のIDを分割取得し、同じ停止世代を検査する。 */
export class ListFrozenPartnerRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    if (after !== null && (!Number.isSafeInteger(after) || String(after) !== request.afterCursor))
      return new Error("invalid partner cursor")
    try {
      ...
      return new Error("frozen partner inventory unavailable", { cause })
    }
  }
}
