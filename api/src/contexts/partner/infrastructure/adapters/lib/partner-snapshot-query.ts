...
export function partnerSnapshotQuery(
  ...
  if (recordKind === "partner-record")
    return {
      sql: `SELECT json_object('format','partner-record','version',1,'partner',json_object(
        'id',id,'code',code,'name',name,'category',category,'corporate_number',corporate_number,
        'note',note,'status',status,
        'created_at',created_at)) AS snapshot_json FROM partners WHERE id=?1`,
      values: [id],
    }
  return {
    sql: `SELECT json_object('format','partner-contract-record','version',1,'contract',json_object(
      'id',id,'partner_id',partner_id,'title',title,'contract_date',contract_date,
