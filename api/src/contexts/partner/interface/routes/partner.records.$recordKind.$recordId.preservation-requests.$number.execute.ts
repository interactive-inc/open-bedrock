...
import { RevalidatePartnerRecordSourceAdapter } from "@/contexts/partner/infrastructure/adapters/revalidate-partner-record-source.adapter"
...
export const POST = partnerFactory.createHandlers(
  ...
  zValidator(
    "param",
    partnerRecordRouteSchema.extend({ number: z.coerce.number().int().positive().safe() }),
  ),
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidatePartnerRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
    ...
    if (result instanceof RecordPreservationExecutionError) {
      switch (result.code) {
        case "invalid":
          throw new PartnerInputError({ message: result.message })
        case "forbidden":
