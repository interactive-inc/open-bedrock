import { zAppPartnerRetirementRequest } from "@/contexts/partner/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { partnerFactory } from "@/contexts/partner/interface/request-environment/partner-factory"
import { SubmitPartnerRetirementRequestAdapter } from "@/contexts/partner/infrastructure/adapters/submit-partner-retirement-request.adapter"
import {
...
export const POST = partnerFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitPartnerRetirementRequestAdapter(c).execute(
      {
    ...
    return c.json(
      zAppPartnerRetirementRequest.parse(submitted.request),
