...
import { VerifyPartnerCoveragePage } from "@/contexts/partner/application/verify-partner-coverage-page"
import {
  PartnerCoverageForbiddenError,
  PartnerCoverageConflictError,
} from "@/contexts/partner/application/errors"
import { zAppPartnerCoveragePageReceipt } from "@/contexts/partner/interface/http/response-schemas"
...
export const POST = partnerFactory.createHandlers(
  ...
  zValidator(
    "json",
    partnerCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = partnerCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyPartnerCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof PartnerCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof PartnerCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppPartnerCoveragePageReceipt.parse({
