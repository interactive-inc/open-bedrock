...
import { CreatePartnerRetirementPlan } from "@/contexts/partner/application/create-partner-retirement-plan"
import { partnerRetirementPlanCommandSchema } from "@/contexts/partner/domain/schemas/partner-retirement-plan-command.schema"
import { zAppPartnerRetirementPlan } from "@/contexts/partner/interface/http/response-schemas"
...
export const POST = partnerFactory.createHandlers(
  ...
  zValidator("json", partnerRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = partnerRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreatePartnerRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppPartnerRetirementPlan.parse({
