...
import { z } from "zod"
import { partnerSourceFreezeResponseSchema } from "@/contexts/partner/interface/http/response-schemas"
...
/** 現在の管理権限でpartner記録の停止世代を読む。 */
export function createPartnerSourceFreezeReadHandlers() {
  return partnerFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "partner" ||
      ...
      return c.json(partnerSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
