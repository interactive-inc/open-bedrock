import { zAppCertificateRequestRetirementRequest } from "@/contexts/certificate-request/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitCertificateRequestRetirementRequestAdapter } from "@/contexts/certificate-request/infrastructure/adapters/submit-certificate-request-retirement-request.adapter"
import {
...
export const POST = certificateRequestFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCertificateRequestRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof CertificateRequestRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CertificateRequestRetirementConflictError)
    ...
    return c.json(
      zAppCertificateRequestRetirementRequest.parse(submitted.request),
