...
const SORT_OPTIONS = {
  created_at_desc: desc(certificateRequests.createdAt),
  created_at_asc: asc(certificateRequests.createdAt),
} as const
...
export const GET = factory.createHandlers(
  ...
  async (c) => {
    ...
    if (query.status !== undefined && query.status !== "") {
      conditions.push(eq(certificateRequests.status, query.status))
    }

    if (query.employee_id !== undefined) {
      conditions.push(eq(certificateRequests.requesterId, query.employee_id))
    }
