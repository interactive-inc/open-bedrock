import { zAppCertificateRequestRetirementExecution } from "@/contexts/certificate-request/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecuteCertificateRequestRetirementAdapter } from "@/contexts/certificate-request/infrastructure/adapters/execute-certificate-request-retirement.adapter"
import {
...
export const POST = certificateRequestFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteCertificateRequestRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppCertificateRequestRetirementExecution.parse(submitted), 200)
  },
)
