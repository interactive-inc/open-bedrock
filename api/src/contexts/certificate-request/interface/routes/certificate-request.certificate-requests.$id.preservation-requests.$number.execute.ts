...
import { RevalidateCertificateRequestRecordSourceAdapter } from "@/contexts/certificate-request/infrastructure/adapters/revalidate-certificate-request-record-source.adapter"
    ...
    const result = await executeSystemRecordPreservation(
      {
        ...
        source: {
          ...
          sourceNamespace,
          revalidate: (source) =>
            new RevalidateCertificateRequestRecordSourceAdapter({
              env: c.env,
              var: c.var,
              sourceNamespace,
            }).prepare(source),
