...
import { VerifyCertificateRequestCoveragePage } from "@/contexts/certificate-request/application/verify-certificate-request-coverage-page"
import {
...
import { zAppCertificateRequestCoveragePageReceipt } from "@/contexts/certificate-request/interface/http/response-schemas"
...
export const POST = certificateRequestFactory.createHandlers(
  ...
  zValidator("json", certificateRequestCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = certificateRequestCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyCertificateRequestCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof CertificateRequestCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof CertificateRequestCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppCertificateRequestCoveragePageReceipt.parse({
