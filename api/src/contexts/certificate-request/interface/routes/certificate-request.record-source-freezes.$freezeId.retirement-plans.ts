...
import { CreateCertificateRequestRetirementPlan } from "@/contexts/certificate-request/application/create-certificate-request-retirement-plan"
...
import { zAppCertificateRequestRetirementPlan } from "@/contexts/certificate-request/interface/http/response-schemas"
...
export const POST = certificateRequestFactory.createHandlers(
  ...
  zValidator("json", certificateRequestRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = certificateRequestRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateCertificateRequestRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppCertificateRequestRetirementPlan.parse({
