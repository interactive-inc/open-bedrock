...
import { z } from "zod"
import { certificateRequestSourceFreezeResponseSchema } from "@/contexts/certificate-request/interface/http/response-schemas"
...
/** 現在の管理権限でcertificate request記録の停止世代を読む。 */
export function createCertificateRequestSourceFreezeReadHandlers() {
  return certificateRequestFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "certificate-request" ||
      ...
      return c.json(certificateRequestSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
