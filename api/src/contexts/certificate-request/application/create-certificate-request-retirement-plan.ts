...
type Context = ConstructorParameters<typeof PrepareCertificateRequestRetirementPlanAdapter>[0]

/** certificate request記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateCertificateRequestRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareCertificateRequestRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new CertificateRequestRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CertificateRequestRetirementConflictError("retirement plan creation conflicts")
  }
}
