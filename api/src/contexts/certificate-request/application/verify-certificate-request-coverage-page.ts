...
export class VerifyCertificateRequestCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    ...
    const proof = await new CertificateRequestRecordSystemAdapter(
      this.c,
    ...
    const generation = await new CertificateRequestRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new CertificateRequestRecordSystemAdapter({
      env: this.c.env,
  ...
  private async confirmTerminal(
    ...
    completion: CertificateRequestRecordSystemAdapter,
  ) {
