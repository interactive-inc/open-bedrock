import { PrepareCertificateRequestRetirementPageAdapter } from "@/contexts/certificate-request/infrastructure/adapters/prepare-certificate-request-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareCertificateRequestRetirementPageAdapter>[0]

/** certificate request記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyCertificateRequestRetirementPage {
  ...
  async execute(
    ...
    const parsed = certificateRequestRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CertificateRequestRetirementForbiddenError("retirement authentication required")
    const authority = await new CertificateRequestRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new CertificateRequestRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new CertificateRequestRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new CertificateRequestRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new CertificateRequestRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new CertificateRequestRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareCertificateRequestRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new CertificateRequestRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new CertificateRequestRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new CertificateRequestRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new CertificateRequestRecordSystemAdapter(guardedContext)
      .coveragePages()
