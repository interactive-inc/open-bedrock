import { certificateRequestRetirementSubmissionCommandSchema } from "@/contexts/certificate-request/domain/schemas/certificate-request-retirement-submission-command.schema"
import { PrepareCertificateRequestRetirementCurrentStateAdapter } from "@/contexts/certificate-request/infrastructure/adapters/prepare-certificate-request-retirement-current-state.adapter"
...
export class SubmitCertificateRequestRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = certificateRequestRetirementSubmissionCommandSchema.safeParse(input)
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new CertificateRequestRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "certificate-request.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new CertificateRequestRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new CertificateRequestRetirementForbiddenError("retirement applicant differs")
      if (
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new CertificateRequestRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareCertificateRequestRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new CertificateRequestRetirementForbiddenError("retirement decision qualification required")
