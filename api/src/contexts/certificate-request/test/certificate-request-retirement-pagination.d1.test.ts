...
import { createCertificateRequestPreservationFixture } from "@/contexts/certificate-request/test/create-certificate-request-preservation-fixture.test-support"
...
test("11件のcertificate request記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createCertificateRequestPreservationFixture(await pool.next())
  ...
  for (const id of certificateRequestIds) {
    const index = certificateRequestIds.indexOf(id) + 1
    await database
      .prepare(`INSERT INTO certificate_requests
      (id,requester_id,certificate_type,submit_to,needed_by,note,status,created_at)
      VALUES (?1,?2,?3,?4,?5,?6,'requested',?7)`)
      .bind(
        id,
        creatorPerson.employeeId,
        `Employment certificate ${index}`,
        `Recipient ${index}`,
  ...
  const frozenUpdate = await apiRequest(frozenCertificateRequestPath, {
    method: "PUT",
    body: {
      certificate_type: "Must not be updated",
      submit_to: null,
      needed_by: null,
      note: null,
    },
  ...
  expect(
    (
      await apiRequest(
        `/certificate-request/certificate-requests/${certificateRequestIds[1]}/issue`,
        { method: "POST" },
      )
    ).status,
  ).toBe(403)
  expect(
    (
      await apiRequest(
        `/certificate-request/certificate-requests/${certificateRequestIds[2]}/reject`,
        { method: "POST" },
      )
    ).status,
  ).toBe(403)
  ...
  const sourcePath = `/certificate-request/record-source-freezes/${freezeId}`
  ...
  expect(await firstCoverage.json()).toMatchObject({
    sequence: 1,
    nextCursor: certificateRequestIds[9],
    recordCount: 10,
  })
  expect(
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["certificate-request-record"] })
  const retirementPath = `/certificate-request/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "certificate-request-retirement",
    revision: 1,
    title: "Retire preserved certificate-request",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/certificate-request/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM certificate_requests").first<number>("n"),
  ).toBe(11)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved certificate-request",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_certificate_request_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_certificate_request_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('cb159749-e8f4-4f7d-807f-ad76a3095a82','system:record:export'); DROP TABLE certificate_requests;",
  )
