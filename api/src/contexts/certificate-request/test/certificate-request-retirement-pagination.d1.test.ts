...
test("11件のcertificate request記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('certificate-request-test-manager','system:record:export'); DROP TABLE certificate_requests;",
  )
