...
describe("GET /application-requests/inbox", () => {
  test("returns 200 with the inbox columns and joined names", async () => {
    const response = await request("/application-requests/inbox", await tokenFor(1, "root"))
  ...
  test("returns 401 without a bearer token", async () => {
    const response = await request("/application-requests/inbox", null)

    expect(response.status).toBe(401)
  })

  test("returns 200 with empty data for a role not listed in any approverRoles", async () => {
    const response = await request("/application-requests/inbox", await tokenFor(99, "member"))
  ...
  test("does not list legacy applications outside a manager's organization scope", async () => {
    const response = await request("/application-requests/inbox", await tokenFor(2, "manager"))
  ...
  test("paginates a large workflow inbox with a constant number of database queries", async () => {
    ...
    const response = await requestWithContext({
      db,
      jwtSecret,
      path: "/application-requests/inbox?limit=10",
