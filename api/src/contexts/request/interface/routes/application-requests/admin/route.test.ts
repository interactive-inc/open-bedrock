...
import { seedApplicationTemplates } from "@/contexts/company/infrastructure/seed/seed-application-templates"
...
describe("GET /application-requests/admin", () => {
  test("returns 200 with all applications for admin", async () => {
    const response = await request("/application-requests/admin", await tokenFor(1, "root"))
  ...
  test("returns 200 with all applications for hr", async () => {
    ...
    const response = await requestWithContext({
      db,
      jwtSecret,
      path: "/application-requests/admin",
  ...
  test("returns 403 for manager", async () => {
    const response = await request("/application-requests/admin", await tokenFor(4, "manager"))

    expect(response.status).toBe(403)
  })

  test("returns 403 for member", async () => {
    const response = await request("/application-requests/admin", await tokenFor(5, "member"))

    expect(response.status).toBe(403)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request("/application-requests/admin", null)
  ...
  test("filters by status", async () => {
    const response = await request(
      "/application-requests/admin?status=approved",
  ...
  test("filters by applicant_id", async () => {
    const response = await request(
      "/application-requests/admin?applicant_id=5",
      await tokenFor(1, "root"),
    )
  ...
  test("filters by template_code", async () => {
    const response = await request(
      "/application-requests/admin?template_code=paid_leave",
      await tokenFor(1, "root"),
    )
  ...
  test("filters by created_at range", async () => {
    const response = await request(
      "/application-requests/admin?from=2026-05-20T00:00:00Z&to=2026-05-23T00:00:00Z",
      await tokenFor(1, "root"),
    )
  ...
  test("sorts by created_at ascending", async () => {
    const response = await request(
      "/application-requests/admin?sort=created_at_asc",
      await tokenFor(1, "root"),
    )
  ...
  test("respects limit and offset", async () => {
    const response = await request(
      "/application-requests/admin?limit=2&offset=1",
      await tokenFor(1, "root"),
    )
  ...
  test("silently ignores non-numeric applicant_id", async () => {
    const response = await request(
      "/application-requests/admin?applicant_id=abc",
      await tokenFor(1, "root"),
    )
  ...
  test("returns empty when template_code does not match any template", async () => {
    const response = await request(
      "/application-requests/admin?template_code=does_not_exist",
      await tokenFor(1, "root"),
    )
