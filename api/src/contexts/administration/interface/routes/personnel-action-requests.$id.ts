...
import { WithdrawPersonnelActionRequest } from "@/contexts/administration/application/employee-lifecycle/procedure/withdraw-personnel-action-request"
...
export const GET = factory.createHandlers(verifyBearer, async (c) => {
  ...
  if (request instanceof CompanyOperationError) {
    throw new InternalError("failed to load personnel action request")
  }
  if (request === null) {
  ...
  const action: SystemJsonValue = request.action
  return c.json(
    {
      id: request.id,
