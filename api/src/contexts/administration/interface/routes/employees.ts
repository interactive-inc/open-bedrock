import { RegisterEmployee } from "@/contexts/administration/application/employee/register-employee"
...
function toEmployeeStatus(state: WorkforceStateAt): "active" | "leave" | "retired" | null {
...
export const GET = factory.createHandlers(
  ...
  async (c) => {
    ...
    const filtered = candidates.filter((row) => {
      ...
      const status = state === undefined ? null : toEmployeeStatus(state)
    ...
    if (emailByEmployeeId instanceof Error) {
      throw toHttpException(
        new UnexpectedError("failed to resolve emails", {
          cause: emailByEmployeeId,
        }),
      )
    }
