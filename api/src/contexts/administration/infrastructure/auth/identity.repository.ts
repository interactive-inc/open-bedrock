...
export class IdentityRepository {
  ...
  async findAccountIdByEmail(email: string): Promise<AccountId | null | Error> {
    const identity = await new FindSystemIdentityByEmail({
      env: { DB: this.c.env.DB },
    }).execute(email)
  ...
  async updateProvisionedIdentity(
    ...
    try {
      const statements: D1PreparedStatement[] = [
        new PrepareSystemIdentityProfileUpdate({
          env: { DB: this.c.env.DB },
        }).prepare(identityId, email, new Date(this.c.env.NOW ?? Date.now())),
    ...
    try {
      ...
      const identities = await Promise.all(
        links.results.map((link) =>
          new SystemIdentityAdministrationRepository({
            env: { DB: this.c.env.DB },
          }).listForAccount(link.account_id),
        ),
      )
