...
export class OrgDepartmentRepository {
  ...
  async create(department: OrgDepartment): Promise<OrgDepartment | ParentNotFound | Error> {
    ...
    try {
      ...
      if (isUniqueConstraintError(error)) {
        return new UniqueConstraintError("department code already exists", {
          cause: error,
        })
      }
   ...
   */
  async delete(department: OrgDepartment): Promise<true | null | Error> {
    try {
  ...
  /** 現在・未来の組織factから参照されていない部署だけをアーカイブする。 */
  async archive(
    department: OrgDepartment,
    input: { archivedAt: number; archivedByAccountId: string; businessDate: string },
  ): Promise<"archived" | "in_use" | Error> {
    try {
      await this.c.env.DB.batch([
        this.c.env.DB.prepare(
          `UPDATE org_departments
           ...
           RETURNING code`,
        ).bind(department.code, input.archivedAt, input.archivedByAccountId, input.businessDate),
        abortWhenPreviousStatementChangedNoRows(this.c.env.DB),
      ])
      return "archived"
    } catch (cause) {
      return isAbortedByGuard(cause)
        ...
        : cause instanceof Error
          ? cause
          : new Error("failed to archive org_department")
    }
  }
