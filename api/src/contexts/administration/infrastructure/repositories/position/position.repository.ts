import { Position } from "@/contexts/administration/domain/entities/position.entity"
...
export class PositionRepository {
  ...
  async create(position: Position): Promise<Position | Error> {
    try {
      ...
      if (isUniqueConstraintError(error)) {
        return new UniqueConstraintError("position code already exists", {
          cause: error,
        })
      }
  ...
  async delete(position: Position): Promise<null | Error> {
    try {
      if (position.id === null) return new Error("cannot delete unsaved position")
      await this.c.var.database.delete(positions).where(eq(positions.id, position.id))
