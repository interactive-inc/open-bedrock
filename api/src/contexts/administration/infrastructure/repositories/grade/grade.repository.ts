...
export class GradeRepository {
  ...
  async delete(grade: Grade): Promise<null | Error> {
    try {
      if (grade.id === null) return new Error("cannot delete unsaved grade")
      await this.c.var.database.delete(grades).where(eq(grades.id, grade.id))
