import { zAppRegulationRetirementRequest } from "@/contexts/regulation/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { regulationFactory } from "@/contexts/regulation/interface/request-environment/regulation-factory"
import { SubmitRegulationRetirementRequestAdapter } from "@/contexts/regulation/infrastructure/adapters/submit-regulation-retirement-request.adapter"
import {
...
export const POST = regulationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitRegulationRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof RegulationRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof RegulationRetirementConflictError)
    ...
    return c.json(
      zAppRegulationRetirementRequest.parse(submitted.request),
