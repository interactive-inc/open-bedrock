...
import { VerifyRegulationCoveragePage } from "@/contexts/regulation/application/verify-regulation-coverage-page"
import {
  RegulationCoverageForbiddenError,
  RegulationCoverageConflictError,
} from "@/contexts/regulation/application/errors"
import { zAppRegulationCoveragePageReceipt } from "@/contexts/regulation/interface/http/response-schemas"
...
export const POST = regulationFactory.createHandlers(
  ...
  zValidator(
    "json",
    regulationCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = regulationCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyRegulationCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof RegulationCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof RegulationCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppRegulationCoveragePageReceipt.parse({
