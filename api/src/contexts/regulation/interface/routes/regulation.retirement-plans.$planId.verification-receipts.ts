...
import { VerifyRegulationRetirementPage } from "@/contexts/regulation/application/verify-regulation-retirement-page"
...
import { zAppRegulationRetirementVerificationReceipt } from "@/contexts/regulation/interface/http/response-schemas"
...
export const POST = regulationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = regulationRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyRegulationRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof RegulationRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof RegulationRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppRegulationRetirementVerificationReceipt.parse({
