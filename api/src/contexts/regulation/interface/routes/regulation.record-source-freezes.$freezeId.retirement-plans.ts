...
import { CreateRegulationRetirementPlan } from "@/contexts/regulation/application/create-regulation-retirement-plan"
...
import { zAppRegulationRetirementPlan } from "@/contexts/regulation/interface/http/response-schemas"
...
export const POST = regulationFactory.createHandlers(
  ...
  zValidator("json", regulationRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = regulationRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateRegulationRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppRegulationRetirementPlan.parse({
