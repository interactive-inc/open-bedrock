...
import { z } from "zod"
import { regulationSourceFreezeResponseSchema } from "@/contexts/regulation/interface/http/response-schemas"
...
/** 現在の管理権限でregulation記録の停止世代を読む。 */
export function createRegulationSourceFreezeReadHandlers() {
  return regulationFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "regulation" ||
      ...
      return c.json(regulationSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
