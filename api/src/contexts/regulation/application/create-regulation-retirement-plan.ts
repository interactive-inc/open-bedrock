import { PrepareRegulationRetirementPlanAdapter } from "@/contexts/regulation/infrastructure/adapters/prepare-regulation-retirement-plan.adapter"
...
import { RegulationRetirementConflictError } from "@/contexts/regulation/application/errors"

type Context = ConstructorParameters<typeof PrepareRegulationRetirementPlanAdapter>[0]

/** regulation記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateRegulationRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareRegulationRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new RegulationRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RegulationRetirementConflictError("retirement plan creation conflicts")
  }
}
