import { PrepareRegulationRetirementPageAdapter } from "@/contexts/regulation/infrastructure/adapters/prepare-regulation-retirement-page.adapter"
import { RegulationRecordSystemAdapter } from "@/contexts/regulation/infrastructure/adapters/regulation-record-system.adapter"
import { regulationRetirementVerificationCommandSchema } from "@/contexts/regulation/domain/schemas/regulation-retirement-verification-command.schema"
import {
  RegulationRetirementForbiddenError,
  RegulationRetirementConflictError,
} from "@/contexts/regulation/application/errors"
...
type Context = ConstructorParameters<typeof PrepareRegulationRetirementPageAdapter>[0]

/** regulation記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyRegulationRetirementPage {
  ...
  async execute(
    ...
    const parsed = regulationRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RegulationRetirementForbiddenError("retirement authentication required")
    const authority = await new RegulationRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new RegulationRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new RegulationRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "regulation" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(regulationRecordKinds)
    )
      return new RegulationRetirementConflictError(
        "retirement plan unavailable or capability changed",
      )
    const repository = new RegulationRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new RegulationRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new RegulationRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareRegulationRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new RegulationRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new RegulationRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new RegulationRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new RegulationRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RegulationRetirementConflictError("retirement verification position already written")
  }
}
