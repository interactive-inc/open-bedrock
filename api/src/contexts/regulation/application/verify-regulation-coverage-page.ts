...
import {
  ...
  RegulationCoverageConflictError,
} from "@/contexts/regulation/application/errors"
import { regulationCoveragePageCommandSchema } from "@/contexts/regulation/domain/schemas/regulation-coverage-page-command.schema"
...
import { CaptureFrozenRegulationRecordPageAdapter } from "@/contexts/regulation/infrastructure/adapters/capture-frozen-regulation-record-page.adapter"
import { PrepareRegulationCoverageRecordsAdapter } from "@/contexts/regulation/infrastructure/adapters/prepare-regulation-coverage-records.adapter"

type Context = RegulationContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからregulation記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyRegulationCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = regulationCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RegulationCoverageForbiddenError("coverage authentication required")
    const proof = await new RegulationRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new RegulationCoverageForbiddenError("coverage authorization denied")
    const generation = await new RegulationRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new RegulationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new RegulationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      return new RegulationCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new RegulationCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenRegulationRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new RegulationCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareRegulationCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new RegulationCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new RegulationCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: RegulationRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new RegulationCoverageConflictError("coverage terminal page changed")
    return page
  }
}
