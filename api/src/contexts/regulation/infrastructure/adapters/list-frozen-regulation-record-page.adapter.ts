...
const tableByKind = {
  "regulation-record": "regulations",
  "regulation-version-record": "regulation_versions",
} as const

/** 未解除の停止世代から指定した規程台帳のIDを分割取得し、同じ停止世代を検査する。 */
export class ListFrozenRegulationRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    if (
      ...
      return new Error("invalid regulation cursor")
    try {
      ...
      return new Error("frozen regulation inventory unavailable", { cause })
    }
  }
}
