...
import { regulationRetirementSubmissionCommandSchema } from "@/contexts/regulation/domain/schemas/regulation-retirement-submission-command.schema"
import { PrepareRegulationRetirementCurrentStateAdapter } from "@/contexts/regulation/infrastructure/adapters/prepare-regulation-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareRegulationRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitRegulationRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = regulationRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RegulationRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareRegulationRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new RegulationRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "regulation.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new RegulationRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new RegulationRetirementForbiddenError("retirement applicant differs")
      if (
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new RegulationRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareRegulationRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new RegulationRetirementForbiddenError("retirement decision qualification required")
