...
export class RegulationActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new RegulationError("forbidden", "invalid regulation actor")
    ...
    if (authorization instanceof Error)
      return new RegulationError("regulation_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new RegulationError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new RegulationError("regulation_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new RegulationError("regulation_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new RegulationError("forbidden", "active employee is required")
    return {
