...
test("規程11件と改定版を分割照合し撤去確定する", async () => {
  ...
  const frozen = await app.request(
    "/regulation/record-source-freezes",
    {
      ...
      body: JSON.stringify({ reason: "Verify regulation archive" }),
    },
    bindings,
  )
  if (frozen.status !== 201) throw new Error(await frozen.text())
  const regulationMappings: Array<{ sourceRecordId: string; preservedRecordId: string }> = []
  const versionMappings: Array<{ sourceRecordId: string; preservedRecordId: string }> = []
  const sources = [
  ...
  for (const source of sources) {
    ...
    const mappings = source.kind === "regulation-record" ? regulationMappings : versionMappings
    mappings.push({ sourceRecordId: source.id, preservedRecordId: record.record_id })
  }
  const coveragePath = `/regulation/record-source-freezes/${freezeId}/coverage-pages`
  const cover = (recordKind: string, records: typeof regulationMappings) =>
  ...
  const version = await cover("regulation-version-record", versionMappings)
  if (version.status !== 200) throw new Error(await version.text())
  expect(await version.json()).toMatchObject({ sequence: 1, nextCursor: null, recordCount: 1 })
  ...
  expect(plan.recordKinds).toEqual(["regulation-record", "regulation-version-record"])
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "regulation-retirement",
    revision: 1,
    title: "Retire preserved regulation",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/regulation/retirement-plans/${plan.id}/verification-receipts`
  ...
  const approved = await app.request(
    `${retirementPath}/${retirementRequest.number}/approve`,
    {
      ...
      body: JSON.stringify({
        ...
        comment: "Reviewed all regulation records",
      }),
    },
    bindings,
  )
  ...
  expect(
    (
      await post(`/regulation/record-source-freezes/${freezeId}/release`, {
  ...
  for (const mapping of [...regulationMappings, ...versionMappings]) {
