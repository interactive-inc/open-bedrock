...
test("規程の停止世代は2台帳の追加・更新・削除をDB確定時に拒否する", () => {
  const database = new Database(":memory:")
  database.exec(`
    CREATE TABLE system_record_source_freezes (owner_context TEXT NOT NULL, revision INTEGER NOT NULL);
    CREATE TABLE regulations (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
    CREATE TABLE regulation_versions (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
  `)
  const migration = readFileSync(
    new URL(
      "../../../../migrations/0304_guard_regulation_record_source_freeze.sql",
      import.meta.url,
    ),
    "utf8",
  )
  database.exec(migration)
  const tables = ["regulations", "regulation_versions"]
  ...
  database.exec("INSERT INTO system_record_source_freezes VALUES ('regulation', 1)")
  for (const table of tables) {
  ...
  database.exec(
    "UPDATE system_record_source_freezes SET revision = 2 WHERE owner_context = 'regulation'",
  )
  for (const table of tables) {
