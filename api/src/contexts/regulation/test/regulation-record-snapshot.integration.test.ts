...
test("規程と改定版の本文・有効日・版を原記録に残す", () => {
  ...
  database.exec(`
    CREATE TABLE regulations (
      id INTEGER PRIMARY KEY, code TEXT, title TEXT, category TEXT, status TEXT, created_at TEXT
    );
    CREATE TABLE regulation_versions (
      id INTEGER PRIMARY KEY, regulation_id INTEGER, version INTEGER, body_md TEXT,
      effective_on TEXT, note TEXT, created_at TEXT
    );
    INSERT INTO regulations VALUES (
      1,'work-rules','Work Rules','personnel','active','2026-09-01T00:00:00.000Z'
    );
    INSERT INTO regulation_versions VALUES (
      2,1,3,'# Work Rules','2026-10-01','Approved revision','2026-09-15T12:00:00.000Z'
    );
  `)
  ...
  expect(regulationSnapshotQuery("regulation-record", "01")).toBeInstanceOf(Error)
  database.close()
})
