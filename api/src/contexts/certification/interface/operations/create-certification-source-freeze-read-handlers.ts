...
import { z } from "zod"
import { certificationSourceFreezeResponseSchema } from "@/contexts/certification/interface/http/response-schemas"
...
/** 現在の管理権限でcertification記録の停止世代を読む。 */
export function createCertificationSourceFreezeReadHandlers() {
  return certificationFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "certification" ||
      ...
      return c.json(certificationSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
