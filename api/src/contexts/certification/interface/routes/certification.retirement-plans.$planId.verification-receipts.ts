...
import { VerifyCertificationRetirementPage } from "@/contexts/certification/application/verify-certification-retirement-page"
...
import { zAppCertificationRetirementVerificationReceipt } from "@/contexts/certification/interface/http/response-schemas"
...
export const POST = certificationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = certificationRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    return c.json(
      zAppCertificationRetirementVerificationReceipt.parse({
