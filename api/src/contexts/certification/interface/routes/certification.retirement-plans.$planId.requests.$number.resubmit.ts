import { zAppCertificationRetirementRequest } from "@/contexts/certification/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { certificationFactory } from "@/contexts/certification/interface/request-environment/certification-factory"
import { SubmitCertificationRetirementRequestAdapter } from "@/contexts/certification/infrastructure/adapters/submit-certification-retirement-request.adapter"
import {
...
export const POST = certificationFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCertificationRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof CertificationRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CertificationRetirementConflictError)
    ...
    return c.json(
      zAppCertificationRetirementRequest.parse(submitted.request),
