import { PrepareCertificationRetirementPlanAdapter } from "@/contexts/certification/infrastructure/adapters/prepare-certification-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareCertificationRetirementPlanAdapter>[0]

/** certification記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateCertificationRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareCertificationRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new CertificationRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new CertificationRetirementConflictError("retirement plan creation conflicts")
  }
}
