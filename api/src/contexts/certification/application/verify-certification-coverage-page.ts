import { z } from "zod"
import { CertificationRecordSystemAdapter } from "@/contexts/certification/infrastructure/adapters/certification-record-system.adapter"
import {
...
import { CaptureFrozenCertificationRecordPageAdapter } from "@/contexts/certification/infrastructure/adapters/capture-frozen-certification-record-page.adapter"
import { PrepareCertificationCoverageRecordsAdapter } from "@/contexts/certification/infrastructure/adapters/prepare-certification-coverage-records.adapter"

type Context = CertificationContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからcertification記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyCertificationCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = certificationCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CertificationCoverageForbiddenError("coverage authentication required")
    const proof = await new CertificationRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new CertificationCoverageForbiddenError("coverage authorization denied")
    const generation = await new CertificationRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new CertificationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new CertificationRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      return new CertificationCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new CertificationCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenCertificationRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new CertificationCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareCertificationCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new CertificationCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new CertificationCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: CertificationRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new CertificationCoverageConflictError("coverage terminal page changed")
    return page
  }
}
