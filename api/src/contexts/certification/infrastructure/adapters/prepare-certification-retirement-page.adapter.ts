...
type Context = CertificationContext & SystemAttachmentStorageContext & SystemDatabaseContext
...
/** 計画に固定したcertification記録ページの原文・保全本文・保持条件を再検証し、保存条件を渡す。 */
export class PrepareCertificationRetirementPageAdapter {
  ...
  async prepare(input: unknown, stepUpToken: string) {
    ...
    const captured = await new CaptureFrozenCertificationRecordPageAdapter(this.c).prepare({
      freezeId: request.freezeId,
      sourceNamespace: request.sourceNamespace,
      recordKind: request.recordKind,
      afterCursor: cursor,
    ...
    const current = await new CertificationActorReadAdapter(this.c).prepare()
