...
export class CertificationActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new CertificationError("forbidden", "invalid certification actor")
    ...
    if (authorization instanceof Error)
      return new CertificationError("certification_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new CertificationError("certification_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new CertificationError("forbidden", "active employee is required")
    return {
