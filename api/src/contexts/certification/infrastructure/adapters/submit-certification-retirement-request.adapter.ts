...
import { certificationRetirementSubmissionCommandSchema } from "@/contexts/certification/domain/schemas/certification-retirement-submission-command.schema"
...
type Context = ConstructorParameters<typeof PrepareCertificationRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitCertificationRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = certificationRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new CertificationRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareCertificationRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new CertificationRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "certification.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new CertificationRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new CertificationRetirementForbiddenError("retirement applicant differs")
      if (
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new CertificationRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareCertificationRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new CertificationRetirementForbiddenError("retirement decision qualification required")
