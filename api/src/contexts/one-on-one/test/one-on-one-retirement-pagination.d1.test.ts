...
import { createOneOnOnePreservationFixture } from "@/contexts/one-on-one/test/create-one-on-one-preservation-fixture.test-support"
...
test("11件の1on1記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    clock,
    database,
    governance,
    ...
    tokenFor,
    request: apiRequest,
  } = await createOneOnOnePreservationFixture(await pool.next())
  ...
  expect(
    (await post("/one-on-one/record-source-freezes", freezeId, { reason: "Preserve one-on-one" }))
      .status,
  ).toBe(201)
  ...
  const mappings = []
  for (const id of oneOnOneIds) {
    const path = `/one-on-one/one-on-ones/${id}/preservation-requests`
  ...
  const sourcePath = `/one-on-one/record-source-freezes/${freezeId}`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["one-on-one-record"] })
  const retirementPath = `/one-on-one/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "one-on-one-retirement",
    revision: 1,
    title: "Retire preserved one-on-one",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/one-on-one/retirement-plans/${planId}/verification-receipts`
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved one-on-one",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_one_on_one_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_one_on_one_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('97095975-7e77-44b8-8da9-625e6bcbdc01','system:record:export'); DROP TABLE one_on_ones;",
  )
