import { zAppOneOnOneRetirementRequest } from "@/contexts/one-on-one/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { oneOnOneFactory } from "@/contexts/one-on-one/interface/request-environment/one-on-one-factory"
import { SubmitOneOnOneRetirementRequestAdapter } from "@/contexts/one-on-one/infrastructure/adapters/submit-one-on-one-retirement-request.adapter"
import {
...
export const POST = oneOnOneFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitOneOnOneRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof OneOnOneRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof OneOnOneRetirementConflictError)
    ...
    return c.json(
      zAppOneOnOneRetirementRequest.parse(submitted.request),
