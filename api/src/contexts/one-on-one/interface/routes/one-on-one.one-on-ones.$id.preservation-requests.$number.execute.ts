...
import { RevalidateOneOnOneRecordSourceAdapter } from "@/contexts/one-on-one/infrastructure/adapters/revalidate-one-on-one-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateOneOnOneRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
