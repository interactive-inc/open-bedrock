import { zAppOneOnOneRetirementExecution } from "@/contexts/one-on-one/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { oneOnOneFactory } from "@/contexts/one-on-one/interface/request-environment/one-on-one-factory"
import { ExecuteOneOnOneRetirementAdapter } from "@/contexts/one-on-one/infrastructure/adapters/execute-one-on-one-retirement.adapter"
import {
...
export const POST = oneOnOneFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteOneOnOneRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppOneOnOneRetirementExecution.parse(submitted), 200)
  },
)
