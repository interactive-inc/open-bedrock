...
import { z } from "zod"
import { oneOnOneSourceFreezeResponseSchema } from "@/contexts/one-on-one/interface/http/response-schemas"
...
/** 現在の管理権限で1on1記録の停止世代を読む。 */
export function createOneOnOneSourceFreezeReadHandlers() {
  return oneOnOneFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "one-on-one" ||
      ...
      return c.json(oneOnOneSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
