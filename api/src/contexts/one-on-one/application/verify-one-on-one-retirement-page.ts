import { PrepareOneOnOneRetirementPageAdapter } from "@/contexts/one-on-one/infrastructure/adapters/prepare-one-on-one-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareOneOnOneRetirementPageAdapter>[0]

/** 1on1記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyOneOnOneRetirementPage {
  ...
  async execute(
    ...
    const parsed = oneOnOneRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new OneOnOneRetirementForbiddenError("retirement authentication required")
    const authority = await new OneOnOneRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new OneOnOneRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new OneOnOneRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "one-on-one" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(["one-on-one-record"])
    )
      return new OneOnOneRetirementConflictError(
        "retirement plan unavailable or capability changed",
      )
    const repository = new OneOnOneRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new OneOnOneRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new OneOnOneRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareOneOnOneRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new OneOnOneRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new OneOnOneRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new OneOnOneRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new OneOnOneRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new OneOnOneRetirementConflictError("retirement verification position already written")
  }
}
