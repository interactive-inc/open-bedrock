...
import { z } from "zod"
import { documentSourceFreezeResponseSchema } from "@/contexts/document/interface/http/response-schemas"
...
/** 現在の管理権限でdocument記録の停止世代を読む。 */
export function createDocumentSourceFreezeReadHandlers() {
  return documentFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "document" ||
      ...
      return c.json(documentSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
