import { zAppDocumentRetirementRequest } from "@/contexts/document/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { documentFactory } from "@/contexts/document/interface/request-environment/document-factory"
import { SubmitDocumentRetirementRequestAdapter } from "@/contexts/document/infrastructure/adapters/submit-document-retirement-request.adapter"
import {
...
export const POST = documentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitDocumentRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof DocumentRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof DocumentRetirementConflictError)
    ...
    return c.json(
      zAppDocumentRetirementRequest.parse(submitted.request),
