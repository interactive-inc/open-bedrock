import { zAppDocumentRetirementExecution } from "@/contexts/document/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { documentFactory } from "@/contexts/document/interface/request-environment/document-factory"
import { ExecuteDocumentRetirementAdapter } from "@/contexts/document/infrastructure/adapters/execute-document-retirement.adapter"
import {
...
export const POST = documentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteDocumentRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppDocumentRetirementExecution.parse(submitted), 200)
  },
)
