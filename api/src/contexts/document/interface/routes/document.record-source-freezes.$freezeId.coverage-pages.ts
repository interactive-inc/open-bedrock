...
import { VerifyDocumentCoveragePage } from "@/contexts/document/application/verify-document-coverage-page"
import {
  DocumentCoverageForbiddenError,
  DocumentCoverageConflictError,
} from "@/contexts/document/application/errors"
import { zAppDocumentCoveragePageReceipt } from "@/contexts/document/interface/http/response-schemas"
...
export const POST = documentFactory.createHandlers(
  ...
  zValidator("json", documentCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = documentCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyDocumentCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof DocumentCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof DocumentCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppDocumentCoveragePageReceipt.parse({
