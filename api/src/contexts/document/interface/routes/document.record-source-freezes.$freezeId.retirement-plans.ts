...
import { CreateDocumentRetirementPlan } from "@/contexts/document/application/create-document-retirement-plan"
import { documentRetirementPlanCommandSchema } from "@/contexts/document/domain/schemas/document-retirement-plan-command.schema"
import { zAppDocumentRetirementPlan } from "@/contexts/document/interface/http/response-schemas"
...
export const POST = documentFactory.createHandlers(
  ...
  zValidator("json", documentRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = documentRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateDocumentRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppDocumentRetirementPlan.parse({
