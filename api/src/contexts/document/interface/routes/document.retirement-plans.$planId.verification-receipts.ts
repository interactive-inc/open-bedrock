...
import { VerifyDocumentRetirementPage } from "@/contexts/document/application/verify-document-retirement-page"
...
import { zAppDocumentRetirementVerificationReceipt } from "@/contexts/document/interface/http/response-schemas"
...
export const POST = documentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = documentRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyDocumentRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof DocumentRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof DocumentRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppDocumentRetirementVerificationReceipt.parse({
