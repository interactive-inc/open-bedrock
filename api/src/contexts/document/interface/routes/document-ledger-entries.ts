  ...
  async (c) => {
    ...
    const responseBody = zAppDocumentList.parse({
      data: rows.map((row) => ({
        ...
        counterparty_reference: row.counterpartyReference,
...
export const POST = factory.createHandlers(
  ...
  zValidator(
    ...
    z.object({
      ...
      counterparty_reference: z.string().max(200).nullable().optional(),
  ...
  async (c) => {
    ...
    const created = await new RegisterDocument(c).run({
      ...
      counterpartyReference: json.counterparty_reference ?? null,
    ...
    const responseBody = zAppDocument.parse({
      ...
      counterparty_reference: created.counterpartyReference,
