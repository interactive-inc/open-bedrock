...
const secret = "document-integration-test-secret"

/** 実System・Companyと文書台帳の原記録を接続する保全fixture。 */
export async function createDocumentPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "document-preservation",
    revision: 1,
    title: "Preserve document-ledger-entries",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "document-pagination-test-pepper",
