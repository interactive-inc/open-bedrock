...
test("11件の文書台帳記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createDocumentPreservationFixture()
  ...
  await database.exec("DROP TRIGGER fail_document_retirement")
  ...
  await database.exec(
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('document-test-manager','system:record:export'); DROP TABLE document_ledger_entries;",
  )
