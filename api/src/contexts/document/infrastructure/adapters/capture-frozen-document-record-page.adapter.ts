...
import { ListFrozenDocumentRecordPageAdapter } from "@/contexts/document/infrastructure/adapters/list-frozen-document-record-page.adapter"
...
/** 停止したdocument記録の原文を分割取得し、全取得後と後続保存時にも同じ停止世代を検査する。 */
export class CaptureFrozenDocumentRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const page = await new ListFrozenDocumentRecordPageAdapter(this.c).prepare(request)
    if (page instanceof Error) return page
    const records = []
    for (const recordId of page.recordIds) {
      const record = await new CaptureDocumentRecordAdapter(this.c).prepare({
        documentId: recordId,
    ...
    try {
      ...
      if (checks.length !== page.assertions.length || checks.some((check) => !check.success))
        return new Error("frozen document capture changed")
    } catch (cause) {
      return new Error("frozen document capture changed", { cause })
    }
