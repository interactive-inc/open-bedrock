...
/** 出張申請記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareBusinessTripRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = businessTripRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new BusinessTripActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new BusinessTripActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "business-trip",
        capability: { revision: 1, recordKinds: ["business-trip-record"] },
        coverage: [
          {
