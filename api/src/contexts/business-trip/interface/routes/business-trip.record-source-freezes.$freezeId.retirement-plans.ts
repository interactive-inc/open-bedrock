...
import { CreateBusinessTripRetirementPlan } from "@/contexts/business-trip/application/create-business-trip-retirement-plan"
import { businessTripRetirementPlanCommandSchema } from "@/contexts/business-trip/domain/schemas/business-trip-retirement-plan-command.schema"
import { zAppBusinessTripRetirementPlan } from "@/contexts/business-trip/interface/http/response-schemas"
...
export const POST = businessTripFactory.createHandlers(
  ...
  zValidator("json", businessTripRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = businessTripRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateBusinessTripRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppBusinessTripRetirementPlan.parse({
