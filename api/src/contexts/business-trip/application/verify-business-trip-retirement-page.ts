import { PrepareBusinessTripRetirementPageAdapter } from "@/contexts/business-trip/infrastructure/adapters/prepare-business-trip-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareBusinessTripRetirementPageAdapter>[0]

/** 出張申請記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyBusinessTripRetirementPage {
  ...
  async execute(
    ...
    const parsed = businessTripRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new BusinessTripRetirementForbiddenError("retirement authentication required")
    const authority = await new BusinessTripRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new BusinessTripRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new BusinessTripRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new BusinessTripRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new BusinessTripRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new BusinessTripRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareBusinessTripRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new BusinessTripRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new BusinessTripRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new BusinessTripRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new BusinessTripRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new BusinessTripRetirementConflictError("retirement verification position already written")
  }
}
