...
export class VerifyBusinessTripCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = businessTripCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new BusinessTripCoverageForbiddenError("coverage authentication required")
    const proof = await new BusinessTripRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new BusinessTripCoverageForbiddenError("coverage authorization denied")
    const generation = await new BusinessTripRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new BusinessTripRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "business-trip-record")
    )
      return new BusinessTripCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new BusinessTripCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenBusinessTripRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      afterId: cursor,
      limit: 10,
    ...
    if (
      ...
      return new BusinessTripCoverageConflictError("coverage mapping or purpose conflicts")
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new BusinessTripCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new BusinessTripCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: BusinessTripRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new BusinessTripCoverageConflictError("coverage terminal page changed")
    return page
  }
}
