...
import { createBusinessTripPreservationFixture } from "@/contexts/business-trip/test/create-business-trip-preservation-fixture.test-support"
...
test("11件の出張申請記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createBusinessTripPreservationFixture(await pool.next())
  ...
  for (const id of tripIds) {
    await database
      .prepare(`INSERT INTO business_trips
      ...
      .bind(
        ...
        `Destination ${id}`,
        `2026-${String(tripIds.indexOf(id) + 1).padStart(2, "0")}-01`,
        `2026-${String(tripIds.indexOf(id) + 1).padStart(2, "0")}-02`,
        `Purpose ${id}`,
        1000 + tripIds.indexOf(id),
        `2026-01-${String(tripIds.indexOf(id) + 1).padStart(2, "0")}T00:00:00Z`,
      )
      .run()
  }
  const at = clock()
  ...
  expect(
    (
      await apiRequest("/business-trip/business-trips", {
        method: "POST",
        body: {
          destination: "Late destination",
          start_date: "2030-01-01",
          end_date: "2030-01-02",
          purpose: "Must not be written",
          estimated_cost: 5000,
        },
      })
    ).status,
  ).toBe(409)
  const frozenTripPath = `/business-trip/business-trips/${tripIds[0]}`
  expect(
    (
      await apiRequest(frozenTripPath, {
        method: "PUT",
        body: {
          destination: "Changed destination",
          start_date: "2026-01-01",
          end_date: "2026-01-02",
          purpose: "Must not be updated",
          estimated_cost: 9000,
        },
      })
    ).status,
  ...
  const sourcePath = `/business-trip/record-source-freezes/${freezeId}`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["business-trip-record"] })
  const retirementPath = `/business-trip/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "business-trip-retirement",
    revision: 1,
    title: "Retire preserved business-trip",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/business-trip/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM business_trips").first<number>("n"),
  ).toBe(11)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved business-trip",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_business_trip_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_business_trip_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('c410ad7b-18bd-475d-8d17-a71413b8290f','system:record:export'); DROP TABLE business_trips;",
  )
