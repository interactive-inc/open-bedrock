import { zAppLifeEventRetirementRequest } from "@/contexts/life-event/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { lifeEventFactory } from "@/contexts/life-event/interface/request-environment/life-event-factory"
import { SubmitLifeEventRetirementRequestAdapter } from "@/contexts/life-event/infrastructure/adapters/submit-life-event-retirement-request.adapter"
import {
...
export const POST = lifeEventFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitLifeEventRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof LifeEventRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof LifeEventRetirementConflictError)
    ...
    return c.json(
      zAppLifeEventRetirementRequest.parse(submitted.request),
