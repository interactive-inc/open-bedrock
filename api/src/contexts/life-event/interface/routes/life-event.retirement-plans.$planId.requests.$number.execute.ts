import { zAppLifeEventRetirementExecution } from "@/contexts/life-event/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { lifeEventFactory } from "@/contexts/life-event/interface/request-environment/life-event-factory"
import { ExecuteLifeEventRetirementAdapter } from "@/contexts/life-event/infrastructure/adapters/execute-life-event-retirement.adapter"
import {
...
export const POST = lifeEventFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteLifeEventRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppLifeEventRetirementExecution.parse(submitted), 200)
  },
)
