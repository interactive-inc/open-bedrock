...
export function createLifeEventSourceFreezeReadHandlers() {
  return lifeEventFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "life-event" ||
      ...
      return c.json(lifeEventSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
