...
import { ListFrozenLifeEventRecordPageAdapter } from "@/contexts/life-event/infrastructure/adapters/list-frozen-life-event-record-page.adapter"
...
/** 停止したlife event記録の原文を分割取得し、全取得後と後続保存時にも同じ停止世代を検査する。 */
export class CaptureFrozenLifeEventRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const page = await new ListFrozenLifeEventRecordPageAdapter(this.c).prepare(request)
    if (page instanceof Error) return page
    const records = []
    for (const recordId of page.recordIds) {
      const record = await new CaptureLifeEventRecordAdapter(this.c).prepare({
        lifeEventId: recordId,
    ...
    try {
      ...
      if (checks.length !== page.assertions.length || checks.some((check) => !check.success))
        return new Error("frozen life-event capture changed")
    } catch (cause) {
      return new Error("frozen life-event capture changed", { cause })
    }
