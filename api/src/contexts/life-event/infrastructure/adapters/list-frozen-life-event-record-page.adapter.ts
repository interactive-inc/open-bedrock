...
/** 未解除の停止世代からlife event記録IDを分割取得し、後続の保存でも同じ世代を検査する。 */
export class ListFrozenLifeEventRecordPageAdapter {
    ...
    try {
      const statements = [
        ...
        this.c.env.DB.prepare(
          "SELECT id FROM life_events WHERE (?1 IS NULL OR id COLLATE BINARY>?1) ORDER BY id COLLATE BINARY LIMIT ?2",
      ...
      if (reads.length !== statements.length || reads.some((read) => !read.success))
        return new Error("frozen life-event inventory unavailable")
      ...
      return new Error("frozen life-event inventory unavailable", { cause })
    }
  }
}
