...
/** life event記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareLifeEventRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = lifeEventRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new LifeEventActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new LifeEventActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "life-event",
        capability: { revision: 1, recordKinds: ["life-event-record"] },
        coverage: [
          {
