...
/** life event記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyLifeEventRetirementPage {
  ...
  async execute(
    ...
    const parsed = lifeEventRetirementVerificationCommandSchema.safeParse(input)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "life-event" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(["life-event-record"])
    )
    ...
    if (
      ...
      return new LifeEventRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new LifeEventRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareLifeEventRetirementPageAdapter(this.c).prepare(
      {
    ...
    return raced !== null && matches(raced)
      ? raced
      : new LifeEventRetirementConflictError("retirement verification position already written")
  }
}
