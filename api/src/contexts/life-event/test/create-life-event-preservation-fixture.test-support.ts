...
const secret = "life-event-integration-test-secret"

/** 実System・Companyとlife event原記録を接続する保全fixture。 */
export async function createLifeEventPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "life-event-preservation",
    revision: 1,
    title: "Preserve life-events",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "life-event-pagination-test-pepper",
