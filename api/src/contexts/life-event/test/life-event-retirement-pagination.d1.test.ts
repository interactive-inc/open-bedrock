...
import { createLifeEventPreservationFixture } from "@/contexts/life-event/test/create-life-event-preservation-fixture.test-support"
...
test("11件のlife event記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    clock,
    database,
    governance,
    ...
    tokenFor,
    request: apiRequest,
  } = await createLifeEventPreservationFixture(await pool.next())
  ...
  for (const id of lifeEventIds) {
    const index = lifeEventIds.indexOf(id) + 1
    await database
      .prepare(`INSERT INTO life_events
      (id,employee_id,event_type,event_date,detail,status,created_at)
      VALUES (?1,?2,'relocation',?3,?4,'submitted',?5)`)
      .bind(
        id,
  ...
  expect(
    (await post("/life-event/record-source-freezes", freezeId, { reason: "Preserve life-event" }))
      .status,
  ).toBe(201)
  ...
  const mappings = []
  for (const id of lifeEventIds) {
    const path = `/life-event/life-events/${id}/preservation-requests`
  ...
  const sourcePath = `/life-event/record-source-freezes/${freezeId}`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["life-event-record"] })
  const retirementPath = `/life-event/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "life-event-retirement",
    revision: 1,
    title: "Retire preserved life-event",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/life-event/retirement-plans/${planId}/verification-receipts`
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved life-event",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_life_event_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_life_event_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('faeb314d-0ae4-49be-8ffa-ea57504bdc1e','system:record:export'); DROP TABLE life_events;",
  )
