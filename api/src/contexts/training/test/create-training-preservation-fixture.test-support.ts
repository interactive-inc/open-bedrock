...
const secret = "training-integration-test-secret"

/** 実System・Companyと研修2台帳の原記録を接続する保全fixture。 */
export async function createTrainingPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "training-preservation",
    revision: 1,
    title: "Preserve training records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "training-pagination-test-pepper",
