...
import { createTrainingPreservationFixture } from "@/contexts/training/test/create-training-preservation-fixture.test-support"
...
import { trainingRecordKinds, type TrainingRecordKind } from "@/contexts/training/domain/training-record-kind"
...
test("研修コース・受講記録を全件保全し、人の承認を経て2台帳を撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createTrainingPreservationFixture()
  ...
  for (let id = 1; id <= 11; id++) {
    ...
    await database.prepare(`INSERT INTO training_courses
      (id,code,title,description,duration_minutes,category,is_required,status)
      VALUES (?1,?2,?3,NULL,60,'skill',0,'active')`)
      .bind(id, code, `Training ${id}`).run()
  }
  await database.prepare(`INSERT INTO training_enrollments
  ...
  expect(
    (await post("/training/record-source-freezes", freezeId, { reason: "Preserve training" }))
      .status,
  ).toBe(201)
  expect(
  ...
  await expect(
    database.prepare("UPDATE training_courses SET title='Must not change' WHERE code='A0001'").run(),
  ).rejects.toThrow("training_record_source_frozen")
  await expect(
    database.prepare(`INSERT INTO training_enrollments
      (id,course_id,employee_id,status,completed_at,score,due_date)
  ...
  expect(
    (
      await apiRequest("/training/training-courses", {
        method: "POST",
  ...
  expect(
    (
      await apiRequest("/training/training-courses/A0001", {
        method: "PUT",
        body: {
  ...
  const sourceRecords: ReadonlyArray<Readonly<{ recordKind: TrainingRecordKind; recordId: string }>> = [
  ...
  for (const sourceRecord of sourceRecords) {
    const path = `/training/records/${sourceRecord.recordKind}/${encodeURIComponent(sourceRecord.recordId)}/preservation-requests`
  ...
  const sourcePath = `/training/record-source-freezes/${freezeId}`
  const planId = crypto.randomUUID()
  const trainingMappings = mappings.filter((mapping) => mapping.recordKind === "training-course-record")
  ...
  const firstCoverage = await post(`${sourcePath}/coverage-pages`, crypto.randomUUID(), {
    purpose: "archive", recordKind: "training-course-record", records: coverageRecords(trainingMappings.slice(0, 10)),
  })
  ...
  const lastCoverage = await post(`${sourcePath}/coverage-pages`, crypto.randomUUID(), {
    purpose: "archive", recordKind: "training-course-record", records: coverageRecords(trainingMappings.slice(10)),
  })
  ...
  for (const recordKind of trainingRecordKinds.slice(1)) {
  ...
  expect(publicPlan).toMatchObject({ totalPages: 3, recordKinds: trainingRecordKinds })
  const retirementPath = `/training/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "training-retirement",
    revision: 1,
    title: "Retire preserved training",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/training/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM training_courses").first<number>("n"),
  ).toBe(11)
  expect(await database.prepare("SELECT count(*) AS n FROM training_enrollments").first<number>("n")).toBe(1)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved training",
  }
  expect(
  ...
  bindings.ATTACHMENT_KEKS = originalKeys
  await database.exec(
    "CREATE TRIGGER fail_training_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await database.exec("DROP TRIGGER fail_training_retirement")
  ...
  await database.exec(
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('training-test-manager','system:record:export'); DROP TABLE training_enrollments; DROP TABLE training_courses;",
  )
