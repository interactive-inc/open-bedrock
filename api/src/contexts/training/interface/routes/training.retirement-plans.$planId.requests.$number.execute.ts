import { zAppTrainingRetirementExecution } from "@/contexts/training/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { trainingFactory } from "@/contexts/training/interface/request-environment/training-factory"
import { ExecuteTrainingRetirementAdapter } from "@/contexts/training/infrastructure/adapters/execute-training-retirement.adapter"
import {
...
export const POST = trainingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteTrainingRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppTrainingRetirementExecution.parse(submitted), 200)
  },
)
