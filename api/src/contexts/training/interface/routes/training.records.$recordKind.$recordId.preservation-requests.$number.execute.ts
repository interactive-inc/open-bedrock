...
import { RevalidateTrainingRecordSourceAdapter } from "@/contexts/training/infrastructure/adapters/revalidate-training-record-source.adapter"
    ...
    const result = await executeSystemRecordPreservation(
      {
        ...
        source: {
          ...
          sourceNamespace,
          revalidate: (source) =>
            new RevalidateTrainingRecordSourceAdapter({
              env: c.env,
              var: c.var,
              sourceNamespace,
            }).prepare(source),
