...
import { VerifyTrainingRetirementPage } from "@/contexts/training/application/verify-training-retirement-page"
...
import { zAppTrainingRetirementVerificationReceipt } from "@/contexts/training/interface/http/response-schemas"
...
export const POST = trainingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = trainingRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyTrainingRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof TrainingRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppTrainingRetirementVerificationReceipt.parse({
