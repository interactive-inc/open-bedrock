...
import { VerifyTrainingCoveragePage } from "@/contexts/training/application/verify-training-coverage-page"
import {
  TrainingCoverageForbiddenError,
  TrainingCoverageConflictError,
} from "@/contexts/training/application/errors"
import { zAppTrainingCoveragePageReceipt } from "@/contexts/training/interface/http/response-schemas"
...
export const POST = trainingFactory.createHandlers(
  ...
  zValidator("json", trainingCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = trainingCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyTrainingCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof TrainingCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof TrainingCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppTrainingCoveragePageReceipt.parse({
