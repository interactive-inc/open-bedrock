...
export const trainingSourceFreezeResponseSchema = z.strictObject({ freeze: recordSourceFreezeSnapshotSchema })
...
export const zAppTrainingRetirementPlan = z.strictObject({
  ...
  totalPages: z.number().int().positive().safe(), recordKinds: z.array(trainingRecordKindSchema).length(2),
...
export const zAppTrainingRetirementExecution = z.strictObject({ retirement_id: z.uuid(), finalized_at: z.iso.datetime() })
