...
/** training記録の取得と会社資格をSystemの共通提出処理へ接続する。 */
export function createTrainingPreservationSubmissionHandlers(mode: "create" | "resubmit") {
    ...
    async (c) => {
      ...
      const submit = async () => {
        if (mode === "create") {
          ...
          if (idempotencyKey === undefined)
            throw new TrainingInputError({ message: "invalid preservation request" })
      ...
      if (submitted instanceof RecordPreservationSubmissionError) {
        switch (submitted.code) {
          case "invalid":
            throw new TrainingInputError({ message: submitted.message })
          case "forbidden":
