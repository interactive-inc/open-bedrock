...
import { z } from "zod"
import { trainingSourceFreezeResponseSchema } from "@/contexts/training/interface/http/response-schemas"
...
/** 現在の管理権限でtraining記録の停止世代を読む。 */
export function createTrainingSourceFreezeReadHandlers() {
  return trainingFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "training" ||
      ...
      return c.json(trainingSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
