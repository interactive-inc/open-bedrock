import { PrepareTrainingRetirementPlanAdapter } from "@/contexts/training/infrastructure/adapters/prepare-training-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareTrainingRetirementPlanAdapter>[0]

/** training記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateTrainingRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareTrainingRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new TrainingRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new TrainingRetirementConflictError("retirement plan creation conflicts")
  }
}
