...
/** training記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareTrainingRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = trainingRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new TrainingActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new TrainingActorReadAdapter(this.c).prepare()
