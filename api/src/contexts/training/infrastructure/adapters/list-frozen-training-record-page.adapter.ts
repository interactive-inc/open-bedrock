...
export class ListFrozenTrainingRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const generation = await new RecordSourceFreezeRepository({ env: this.c.env, assertions: actor.assertions })
      .prepareActiveGeneration({ id: request.freezeId, sourceNamespace: request.sourceNamespace, ownerContext: "training" })
    ...
    const table = request.recordKind === "training-course-record" ? "training_courses" : "training_enrollments"
    try {
