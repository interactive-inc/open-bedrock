...
test("再送にも現在のDB権限条件を要求し、監査保存失敗では停止しない", async () => {
  ...
  await f.database
    .exec(`CREATE TRIGGER freeze_test_audit_failure BEFORE INSERT ON system_audit_events
  ...
  await f.database.exec("DROP TRIGGER freeze_test_audit_failure")
  expect(await create.execute(command, new Date())).toMatchObject({ kind: "created" })
  await f.database.exec("UPDATE freeze_test_permission SET allowed=0")
  expect(await create.execute(command, new Date())).toBeInstanceOf(Error)
  expect(
