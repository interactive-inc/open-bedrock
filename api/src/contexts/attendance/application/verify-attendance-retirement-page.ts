import { PrepareAttendanceRetirementPageAdapter } from "@/contexts/attendance/infrastructure/adapters/prepare-attendance-retirement-page.adapter"
import { AttendanceRecordSystemAdapter } from "@/contexts/attendance/infrastructure/adapters/attendance-record-system.adapter"
import { attendanceRetirementVerificationCommandSchema } from "@/contexts/attendance/domain/schemas/attendance-retirement-verification-command.schema"
import {
...
type Context = ConstructorParameters<typeof PrepareAttendanceRetirementPageAdapter>[0]

/** 打刻の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyAttendanceRetirementPage {
  ...
  async execute(
    ...
    const parsed = attendanceRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new AttendanceRetirementForbiddenError("retirement authentication required")
    const authority = await new AttendanceRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new AttendanceRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new AttendanceRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "attendance" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(["attendance-record"])
    )
      return new AttendanceRetirementConflictError(
        "retirement plan unavailable or capability changed",
      )
    const repository = new AttendanceRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new AttendanceRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new AttendanceRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareAttendanceRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new AttendanceRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new AttendanceRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new AttendanceRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new AttendanceRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AttendanceRetirementConflictError("retirement verification position already written")
  }
}
