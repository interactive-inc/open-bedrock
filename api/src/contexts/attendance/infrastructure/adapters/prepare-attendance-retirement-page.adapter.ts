...
const requestSchema = z.strictObject({
  ...
  recordKind: z.literal("attendance-record"),
  sequence: z.number().int().positive().safe(),
  terminalDigest: z.string().regex(/^[0-9a-f]{64}$/),
})

/** 計画に固定した打刻ページの原文・保全本文・保持条件を再検証し、保存条件を渡す。 */
export class PrepareAttendanceRetirementPageAdapter {
  ...
  async prepare(input: unknown, stepUpToken: string) {
    ...
    const current = await new AttendanceRecordSourceAuthorizationAdapter(this.c).prepare()
