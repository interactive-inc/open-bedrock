import { zAppAttendanceRetirementExecution } from "@/contexts/attendance/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { attendanceFactory } from "@/contexts/attendance/interface/request-environment/attendance-factory"
import { ExecuteAttendanceRetirementAdapter } from "@/contexts/attendance/infrastructure/adapters/execute-attendance-retirement.adapter"
import {
...
export const POST = attendanceFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteAttendanceRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppAttendanceRetirementExecution.parse(submitted), 200)
  },
)
