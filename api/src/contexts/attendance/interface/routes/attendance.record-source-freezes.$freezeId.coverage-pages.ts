...
import { VerifyAttendanceCoveragePage } from "@/contexts/attendance/application/verify-attendance-coverage-page"
import {
  AttendanceCoverageForbiddenError,
  AttendanceCoverageConflictError,
} from "@/contexts/attendance/application/errors"
import { zAppAttendanceCoveragePageReceipt } from "@/contexts/attendance/interface/http/response-schemas"
...
export const POST = attendanceFactory.createHandlers(
  ...
  zValidator("json", attendanceCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = attendanceCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyAttendanceCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof AttendanceCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof AttendanceCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppAttendanceCoveragePageReceipt.parse({
