...
import { VerifyAttendanceRetirementPage } from "@/contexts/attendance/application/verify-attendance-retirement-page"
...
import { zAppAttendanceRetirementVerificationReceipt } from "@/contexts/attendance/interface/http/response-schemas"
...
export const POST = attendanceFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = attendanceRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyAttendanceRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof AttendanceRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof AttendanceRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppAttendanceRetirementVerificationReceipt.parse({
