...
import { CreateAttendanceRetirementPlan } from "@/contexts/attendance/application/create-attendance-retirement-plan"
...
import { zAppAttendanceRetirementPlan } from "@/contexts/attendance/interface/http/response-schemas"
...
export const POST = attendanceFactory.createHandlers(
  ...
  zValidator("json", attendanceRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = attendanceRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateAttendanceRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppAttendanceRetirementPlan.parse({
