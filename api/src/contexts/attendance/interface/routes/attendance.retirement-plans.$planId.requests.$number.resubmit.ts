import { zAppAttendanceRetirementRequest } from "@/contexts/attendance/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { attendanceFactory } from "@/contexts/attendance/interface/request-environment/attendance-factory"
import { SubmitAttendanceRetirementRequestAdapter } from "@/contexts/attendance/infrastructure/adapters/submit-attendance-retirement-request.adapter"
import {
...
export const POST = attendanceFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitAttendanceRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof AttendanceRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof AttendanceRetirementConflictError)
    ...
    return c.json(
      zAppAttendanceRetirementRequest.parse(submitted.request),
