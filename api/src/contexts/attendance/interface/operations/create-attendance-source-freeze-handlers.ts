...
/** 打刻の書込み停止と解除を、現在の人の再認証と技術権限へ接続する。 */
export function createAttendanceSourceFreezeHandlers(mode: "create" | "release") {
  return attendanceFactory.createHandlers(
    ...
    async (c) => {
      ...
      const execute = async () => {
        if (mode === "create")
          return new CreateRecordSourceFreeze({ repository }).execute(command.data, now)
        return new ReleaseRecordSourceFreeze({ repository }).execute(command.data, now)
      }
