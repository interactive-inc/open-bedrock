...
import { z } from "zod"
import { companyCalendarDaySourceFreezeResponseSchema } from "@/contexts/company-calendar/interface/http/response-schemas"
...
/** 現在の管理権限で会社カレンダー記録の停止世代を読む。 */
export function createCompanyCalendarDaySourceFreezeReadHandlers() {
  return companyCalendarDayFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "company-calendar" ||
      ...
      return c.json(companyCalendarDaySourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
