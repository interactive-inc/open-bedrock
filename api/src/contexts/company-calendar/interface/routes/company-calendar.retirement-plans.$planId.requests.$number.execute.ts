import { zAppCompanyCalendarDayRetirementExecution } from "@/contexts/company-calendar/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecuteCompanyCalendarDayRetirementAdapter } from "@/contexts/company-calendar/infrastructure/adapters/execute-company-calendar-retirement.adapter"
import {
...
export const POST = companyCalendarDayFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteCompanyCalendarDayRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppCompanyCalendarDayRetirementExecution.parse(submitted), 200)
  },
)
