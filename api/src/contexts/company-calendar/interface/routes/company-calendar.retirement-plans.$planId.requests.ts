import { zAppCompanyCalendarDayRetirementRequest } from "@/contexts/company-calendar/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitCompanyCalendarDayRetirementRequestAdapter } from "@/contexts/company-calendar/infrastructure/adapters/submit-company-calendar-retirement-request.adapter"
import {
...
export const POST = companyCalendarDayFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitCompanyCalendarDayRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof CompanyCalendarDayRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof CompanyCalendarDayRetirementConflictError)
    ...
    return c.json(
      zAppCompanyCalendarDayRetirementRequest.parse(submitted.request),
