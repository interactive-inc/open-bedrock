...
import { VerifyCompanyCalendarDayRetirementPage } from "@/contexts/company-calendar/application/verify-company-calendar-day-retirement-page"
...
import { zAppCompanyCalendarDayRetirementVerificationReceipt } from "@/contexts/company-calendar/interface/http/response-schemas"
...
export const POST = companyCalendarDayFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = companyCalendarDayRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyCompanyCalendarDayRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof CompanyCalendarDayRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppCompanyCalendarDayRetirementVerificationReceipt.parse({
