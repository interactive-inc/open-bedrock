...
import { CreateCompanyCalendarDayRetirementPlan } from "@/contexts/company-calendar/application/create-company-calendar-day-retirement-plan"
import { companyCalendarDayRetirementPlanCommandSchema } from "@/contexts/company-calendar/domain/schemas/company-calendar-retirement-plan-command.schema"
import { zAppCompanyCalendarDayRetirementPlan } from "@/contexts/company-calendar/interface/http/response-schemas"
...
export const POST = companyCalendarDayFactory.createHandlers(
  ...
  zValidator("json", companyCalendarDayRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = companyCalendarDayRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateCompanyCalendarDayRetirementPlan(c).execute(command.data, stepUpToken)
    if (receipt instanceof ForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof CompanyCalendarDayRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppCompanyCalendarDayRetirementPlan.parse({
