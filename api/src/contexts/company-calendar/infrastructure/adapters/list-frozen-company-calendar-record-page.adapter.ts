...
export class ListFrozenCompanyCalendarDayRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    try {
      const page =
        request.afterId === 0
          ? this.c.env.DB.prepare("SELECT id FROM company_calendar_days ORDER BY id LIMIT ?1").bind(
              request.limit + 1,
            )
          : this.c.env.DB.prepare(
              "SELECT id FROM company_calendar_days WHERE id>?1 ORDER BY id LIMIT ?2",
