...
/** 未解除の停止世代から会社カレンダー記録IDを分割取得し、後続の保存でも同じ世代を検査する。 */
export class ListFrozenCompanyCalendarDayRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    try {
      const page =
        request.afterId === null
          ? this.c.env.DB.prepare(
              "SELECT id FROM company_calendar_days ORDER BY id COLLATE BINARY LIMIT ?1",
            ).bind(request.limit + 1)
          : this.c.env.DB.prepare(
              "SELECT id FROM company_calendar_days WHERE id COLLATE BINARY>?1 ORDER BY id COLLATE BINARY LIMIT ?2",
      ...
      if (reads.length !== statements.length || reads.some((read) => !read.success))
        return new Error("frozen company-calendar inventory unavailable")
      ...
      return new Error("frozen company-calendar inventory unavailable", { cause })
    }
  }
}
