...
/** 会社カレンダー記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareCompanyCalendarDayRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = companyCalendarDayRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new CompanyCalendarDayActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new CompanyCalendarDayActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "company-calendar",
        capability: { revision: 1, recordKinds: ["company-calendar-record"] },
        coverage: [
          {
