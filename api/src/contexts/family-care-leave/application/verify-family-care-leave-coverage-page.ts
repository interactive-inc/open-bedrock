...
export class VerifyFamilyCareLeaveCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = familyCareLeaveCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new FamilyCareLeaveCoverageForbiddenError("coverage authentication required")
    const proof = await new FamilyCareLeaveRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new FamilyCareLeaveCoverageForbiddenError("coverage authorization denied")
    const generation = await new FamilyCareLeaveRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new FamilyCareLeaveRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new FamilyCareLeaveRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "family-care-leave-record")
    )
      return new FamilyCareLeaveCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new FamilyCareLeaveCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenFamilyCareLeaveRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      afterId: cursor,
      limit: 10,
    ...
    if (
      ...
      return new FamilyCareLeaveCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareFamilyCareLeaveCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new FamilyCareLeaveCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new FamilyCareLeaveCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: FamilyCareLeaveRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new FamilyCareLeaveCoverageConflictError("coverage terminal page changed")
    return page
  }
}
