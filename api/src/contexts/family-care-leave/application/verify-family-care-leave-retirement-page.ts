import { PrepareFamilyCareLeaveRetirementPageAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/prepare-family-care-leave-retirement-page.adapter"
import { FamilyCareLeaveRecordSystemAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/family-care-leave-record-system.adapter"
import { familyCareLeaveRetirementVerificationCommandSchema } from "@/contexts/family-care-leave/domain/schemas/family-care-leave-retirement-verification-command.schema"
import {
  FamilyCareLeaveRetirementForbiddenError,
  FamilyCareLeaveRetirementConflictError,
...
type Context = ConstructorParameters<typeof PrepareFamilyCareLeaveRetirementPageAdapter>[0]

/** family care leave記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyFamilyCareLeaveRetirementPage {
  ...
  async execute(
    ...
    const parsed = familyCareLeaveRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new FamilyCareLeaveRetirementForbiddenError("retirement authentication required")
    const authority = await new FamilyCareLeaveRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new FamilyCareLeaveRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new FamilyCareLeaveRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new FamilyCareLeaveRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new FamilyCareLeaveRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new FamilyCareLeaveRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareFamilyCareLeaveRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new FamilyCareLeaveRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new FamilyCareLeaveRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new FamilyCareLeaveRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new FamilyCareLeaveRecordSystemAdapter(guardedContext)
      .coveragePages()
