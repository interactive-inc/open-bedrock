import { PrepareFamilyCareLeaveRetirementPlanAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/prepare-family-care-leave-retirement-plan.adapter"
...
import { FamilyCareLeaveRetirementConflictError } from "@/contexts/family-care-leave/application/errors"

type Context = ConstructorParameters<typeof PrepareFamilyCareLeaveRetirementPlanAdapter>[0]

/** family care leave記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateFamilyCareLeaveRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareFamilyCareLeaveRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new FamilyCareLeaveRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new FamilyCareLeaveRetirementConflictError("retirement plan creation conflicts")
  }
}
