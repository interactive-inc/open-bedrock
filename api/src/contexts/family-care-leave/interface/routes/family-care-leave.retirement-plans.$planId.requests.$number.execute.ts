import { zAppFamilyCareLeaveRetirementExecution } from "@/contexts/family-care-leave/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecuteFamilyCareLeaveRetirementAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/execute-family-care-leave-retirement.adapter"
import {
...
export const POST = familyCareLeaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteFamilyCareLeaveRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppFamilyCareLeaveRetirementExecution.parse(submitted), 200)
  },
)
