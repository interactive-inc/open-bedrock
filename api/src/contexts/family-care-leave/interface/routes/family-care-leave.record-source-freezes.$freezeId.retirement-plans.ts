...
import { CreateFamilyCareLeaveRetirementPlan } from "@/contexts/family-care-leave/application/create-family-care-leave-retirement-plan"
...
import { zAppFamilyCareLeaveRetirementPlan } from "@/contexts/family-care-leave/interface/http/response-schemas"
...
export const POST = familyCareLeaveFactory.createHandlers(
  ...
  zValidator("json", familyCareLeaveRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = familyCareLeaveRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateFamilyCareLeaveRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppFamilyCareLeaveRetirementPlan.parse({
