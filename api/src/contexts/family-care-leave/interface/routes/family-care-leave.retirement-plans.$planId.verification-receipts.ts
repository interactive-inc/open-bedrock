...
import { VerifyFamilyCareLeaveRetirementPage } from "@/contexts/family-care-leave/application/verify-family-care-leave-retirement-page"
...
import { zAppFamilyCareLeaveRetirementVerificationReceipt } from "@/contexts/family-care-leave/interface/http/response-schemas"
...
export const POST = familyCareLeaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = familyCareLeaveRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyFamilyCareLeaveRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof FamilyCareLeaveRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof FamilyCareLeaveRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppFamilyCareLeaveRetirementVerificationReceipt.parse({
