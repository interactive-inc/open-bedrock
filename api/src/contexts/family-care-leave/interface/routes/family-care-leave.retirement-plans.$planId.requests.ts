import { zAppFamilyCareLeaveRetirementRequest } from "@/contexts/family-care-leave/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitFamilyCareLeaveRetirementRequestAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/submit-family-care-leave-retirement-request.adapter"
import {
...
export const POST = familyCareLeaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitFamilyCareLeaveRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof FamilyCareLeaveRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof FamilyCareLeaveRetirementConflictError)
    ...
    return c.json(
      zAppFamilyCareLeaveRetirementRequest.parse(submitted.request),
