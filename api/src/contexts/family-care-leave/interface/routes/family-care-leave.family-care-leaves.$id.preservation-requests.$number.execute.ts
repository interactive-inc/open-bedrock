...
import { RevalidateFamilyCareLeaveRecordSourceAdapter } from "@/contexts/family-care-leave/infrastructure/adapters/revalidate-family-care-leave-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateFamilyCareLeaveRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
