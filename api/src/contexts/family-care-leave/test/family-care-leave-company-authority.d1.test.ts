...
const jwtSecret = "family-care-leave-company-authority-test-secret"

// E002 の産休・育休・介護休業の申出。E001 は直属上司、E099 は技術的権限を持つが E002 の管理系列に属さない。
const recordId = "20000000-0000-4000-8000-000000000001"
const manager = 1
const unrelatedManagerWithPermission = 99

const actions = ["approve"] as const
...
async function decide(
  ...
  return requestWithContext({
    db,
    jwtSecret,
    path: `/family-care-leave/family-care-leaves/${recordId}/${action}`,
...
function storedStatus(db: D1Database): Promise<string | null> {
  return db
    .prepare("SELECT status FROM family_care_leaves WHERE id = ?1")
    .bind(recordId)
    .first<string>("status")
}

describe("産休・育休・介護休業の申出の判断は 技術的権限とCompany上の資格を合成する", () => {
  for (const action of actions) {
    ...
    test(`${action}: 資格の参照後に会社の状態が変われば保存しない`, async () => {
      ...
      const save = FamilyCareLeaveRepository.prototype.decideStatus
      const interception = spyOn(
        FamilyCareLeaveRepository.prototype,
        "decideStatus",
      ).mockImplementationOnce(async function (this: FamilyCareLeaveRepository, props) {
        await db
          .prepare(
