...
test("11件のfamily care leave記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createFamilyCareLeavePreservationFixture(await pool.next())
  ...
  await execSql(database, "DROP TRIGGER fail_family_care_leave_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('77788a48-60c9-4465-8fc8-1a786dd22595','system:record:export'); DROP TABLE family_care_leaves;",
  )
