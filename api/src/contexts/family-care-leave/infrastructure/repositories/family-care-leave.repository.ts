  ...
  async decideStatus(props: {
    ...
    try {
      ...
      const results = await database.batch([
        ...
        database
          .prepare(
            "UPDATE family_care_leaves SET status = ?1 WHERE id = ?2 AND status = ?3 RETURNING id",
          )
      ...
      return error instanceof Error
        ? error
        : new Error("failed to update family_care_leaves status")
    }
  }
