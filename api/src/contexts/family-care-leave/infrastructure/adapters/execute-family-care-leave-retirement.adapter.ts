...
type Context = ConstructorParameters<typeof PrepareFamilyCareLeaveRetirementCurrentStateAdapter>[0]

/** 現在の全件保全と人の承認資格を再検査し、原記録を消さずに停止世代の撤去を確定する。 */
export class ExecuteFamilyCareLeaveRetirementAdapter {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    const current = await new PrepareFamilyCareLeaveRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (
      ...
      return new FamilyCareLeaveRetirementConflictError("retirement proposal version differs")
    ...
    if (
      ...
      return new FamilyCareLeaveRetirementForbiddenError("retirement applicant differs")
    if (
      ...
      return new FamilyCareLeaveRetirementConflictError("retirement verified scope differs")
    ...
    if (proposal.status !== "approved")
      return new FamilyCareLeaveRetirementConflictError("retirement is not approved")
    ...
    if (finalized instanceof Error) {
      ...
      return new FamilyCareLeaveRetirementConflictError("retirement changed before finalization", {
        cause: finalized,
      })
    }
