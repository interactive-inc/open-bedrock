...
export class SubmitFamilyCareLeaveRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = familyCareLeaveRetirementSubmissionCommandSchema.safeParse(input)
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new FamilyCareLeaveRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "family-care-leave.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new FamilyCareLeaveRetirementConflictError("retirement previous version is missing")
