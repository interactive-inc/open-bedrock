...
/** family care leave記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareFamilyCareLeaveRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = familyCareLeaveRetirementPlanCommandSchema.safeParse(input)
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "family-care-leave",
        capability: { revision: 1, recordKinds: ["family-care-leave-record"] },
        coverage: [
          {
