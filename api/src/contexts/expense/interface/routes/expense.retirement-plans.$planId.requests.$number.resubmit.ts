...
import { SubmitExpenseRetirementRequestAdapter } from "@/contexts/expense/infrastructure/adapters/submit-expense-retirement-request.adapter"
import {
...
export const POST = expenseFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitExpenseRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
