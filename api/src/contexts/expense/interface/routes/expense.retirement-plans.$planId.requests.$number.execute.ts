...
import { ExecuteExpenseRetirementAdapter } from "@/contexts/expense/infrastructure/adapters/execute-expense-retirement.adapter"
import {
...
export const POST = expenseFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteExpenseRetirementAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof ExpenseRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof ExpenseRetirementConflictError)
    ...
    return c.json(
      z.strictObject({ retirement_id: z.uuid(), finalized_at: z.iso.datetime() }).parse(submitted),
      200,
    )
  },
)
