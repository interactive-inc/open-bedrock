...
describe("GET /expenses/:id", () => {
  test("returns 200 with the detail for the owner", async () => {
    const response = await request({ path: "/expense/expenses/1", token: await tokenFor(5) })
  ...
  test("returns 403 for a non owner member", async () => {
    const response = await request({ path: "/expense/expenses/1", token: await tokenFor(9) })

    expect(response.status).toBe(403)
  })

  test("returns 200 for a manager inside the applicant organization scope", async () => {
    const response = await request({ path: "/expense/expenses/1", token: await tokenFor(4) })

    expect(response.status).toBe(200)
  })

  test("returns 403 for a manager outside the applicant organization scope", async () => {
    const response = await request({ path: "/expense/expenses/1", token: await tokenFor(2) })

    expect(response.status).toBe(403)
  })

  test("returns 404 for a non numeric id", async () => {
    const response = await request({ path: "/expense/expenses/abc", token: await tokenFor(5) })

    expect(response.status).toBe(404)
  })

  test("returns 404 for an unknown id", async () => {
    const response = await request({ path: "/expense/expenses/9999", token: await tokenFor(2) })

    expect(response.status).toBe(404)
  })

  test("returns 401 without a bearer token", async () => {
    const response = await request({ path: "/expense/expenses/1", token: null })
