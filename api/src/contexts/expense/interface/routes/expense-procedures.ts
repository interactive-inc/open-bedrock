...
export const GET = factory.createHandlers(verifyBearer, async (c) => {
  ...
  if (
    !["expense:submit", "expense:approve", "expense:procedure:manage"].some((permission) =>
      session.hasPermission(permission),
    )
  )
    throw new ForbiddenError()
  const definition = await openSystemProcedures(c).find(procedureKeySchema.parse("expense_request"))
...
export const PUT = factory.createHandlers(
  ...
  async (c) => {
    ...
    const saved = await new PublishExpenseProcedure(c).run({
