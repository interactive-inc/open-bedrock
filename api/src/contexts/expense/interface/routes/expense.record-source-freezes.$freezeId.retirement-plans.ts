...
import { CreateExpenseRetirementPlan } from "@/contexts/expense/application/create-expense-retirement-plan"
import { expenseRetirementPlanCommandSchema } from "@/contexts/expense/domain/schemas/expense-retirement-command.schema"
import { zAppExpenseRetirementPlan } from "@/contexts/expense/interface/http/response-schemas"
...
export const POST = expenseFactory.createHandlers(
  ...
  zValidator("json", expenseRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = expenseRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateExpenseRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppExpenseRetirementPlan.parse({
