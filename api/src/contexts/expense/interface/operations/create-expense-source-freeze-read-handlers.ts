...
import { zAppExpenseSourceFreeze } from "@/contexts/expense/interface/http/response-schemas"
...
export function createExpenseSourceFreezeReadHandlers() {
  return expenseFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "expense" ||
      ...
      return c.json(zAppExpenseSourceFreeze.parse({ freeze: freeze.snapshot }))
    },
  )
}
