...
test("経費照合は保存済みの続きから12件を照合し、飛越しと再認証失効を拒否する", async () => {
  ...
  await execSql(f.database, "DROP TRIGGER fail_retirement_withdrawal_audit")
  ...
  await execSql(f.database, "DROP TRIGGER fail_retirement_resubmission")
  ...
  await execSql(f.database, "DROP TRIGGER fail_retirement_final_audit")
  ...
  // ローカルD1は問い合わせごとにworkerdへ往復するため、12件の照合と保全に20秒ほどかかる。
}, 90_000)
