...
test("添付対応の保全は元の全列を残し、対応の変更不能性と資格取消を維持する", async () => {
  ...
  await c.database.exec("CREATE TABLE expense_link_capture_receipts (id TEXT PRIMARY KEY)")
  expect(
    await c.database
      .prepare(
...
test("停止世代の経費全種別をページ分割し、権限取消・停止解除後の確定を拒否する", async () => {
  ...
  await c.database
    ...
    .run()
  await c.database
    .exec(`INSERT INTO expense_budgets (id,organization_unit_id,fiscal_period,period_start,period_end,amount,name,note,created_at) SELECT 902,organization_unit_id,'2027',period_start,period_end,amount,name,note,created_at
    FROM expense_budgets WHERE id=901`)
  await c.database
    .exec(`INSERT INTO expense_budgets (id,organization_unit_id,fiscal_period,period_start,period_end,amount,name,note,created_at)
  ...
  await c.database.exec("UPDATE expense_budgets SET amount=amount+1 WHERE id=901")
...
test("経費の全6種別を実認証で保全し、業務全テーブル撤去後も元の列と領収書原文を取得できる", async () => {
  ...
  await c.database.exec(`DROP TABLE expense_approvals; DROP TABLE expense_attachments;
