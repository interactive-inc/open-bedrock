...
test("添付なしの申請も元行の全列を取得し、案件取消後の古い保全確定を拒否する", async () => {
  ...
  await c.database.exec("CREATE TABLE expense_source_capture_receipts (id TEXT PRIMARY KEY)")
  expect(
    await c.database
      .batch([
...
test("旧承認を元行のまま取得し、別申請への付替えや取得後の内容変更を確定しない", async () => {
  ...
  await c.database.exec("CREATE TABLE legacy_approval_capture_receipts (id TEXT PRIMARY KEY)")
  expect(
    await c.database
      .batch([
