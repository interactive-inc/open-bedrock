...
/** 経費の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyExpenseRetirementPage {
  ...
  async execute(
    ...
    const parsed = expenseRetirementVerificationCommandSchema.safeParse(input)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "expense" ||
    ...
    if (
      ...
      return new ExpenseRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new ExpenseRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareExpenseRetirementPageAdapter(this.c).prepare(
      {
    ...
    return raced !== null && matches(raced)
      ? raced
      : new ExpenseRetirementConflictError("retirement verification position already written")
  }
}
