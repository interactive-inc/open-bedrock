...
import { ExpenseRetirementConflictError } from "@/contexts/expense/application/errors"

type Context = ConstructorParameters<typeof PrepareExpenseRetirementPlanAdapter>[0]

/** 経費の全種別を検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateExpenseRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareExpenseRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new ExpenseRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new ExpenseRetirementConflictError("retirement plan creation conflicts")
  }
}
