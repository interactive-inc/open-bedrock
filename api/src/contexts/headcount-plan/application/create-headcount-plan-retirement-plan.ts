...
type Context = ConstructorParameters<typeof PrepareHeadcountPlanRetirementPlanAdapter>[0]

/** 人員計画記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateHeadcountPlanRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareHeadcountPlanRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new HeadcountPlanRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new HeadcountPlanRetirementConflictError("retirement plan creation conflicts")
  }
}
