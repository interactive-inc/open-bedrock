import { PrepareHeadcountPlanRetirementPageAdapter } from "@/contexts/headcount-plan/infrastructure/adapters/prepare-headcount-plan-retirement-page.adapter"
...
import { headcountPlanRetirementVerificationCommandSchema } from "@/contexts/headcount-plan/domain/schemas/headcount-plan-retirement-verification-command.schema"
import {
  HeadcountPlanRetirementForbiddenError,
  HeadcountPlanRetirementConflictError,
} from "@/contexts/headcount-plan/application/errors"
...
type Context = ConstructorParameters<typeof PrepareHeadcountPlanRetirementPageAdapter>[0]

/** 人員計画記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyHeadcountPlanRetirementPage {
  ...
  async execute(
    ...
    const parsed = headcountPlanRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new HeadcountPlanRetirementForbiddenError("retirement authentication required")
    const authority = await new HeadcountPlanRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new HeadcountPlanRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new HeadcountPlanRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    const repository = new HeadcountPlanRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new HeadcountPlanRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new HeadcountPlanRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareHeadcountPlanRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new HeadcountPlanRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new HeadcountPlanRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new HeadcountPlanRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new HeadcountPlanRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new HeadcountPlanRetirementConflictError("retirement verification position already written")
  }
}
