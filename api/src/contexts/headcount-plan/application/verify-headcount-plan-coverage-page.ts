...
import {
  HeadcountPlanCoverageForbiddenError,
  HeadcountPlanCoverageConflictError,
} from "@/contexts/headcount-plan/application/errors"
import { headcountPlanCoveragePageCommandSchema } from "@/contexts/headcount-plan/domain/schemas/headcount-plan-coverage-page-command.schema"
...
export class VerifyHeadcountPlanCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = headcountPlanCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new HeadcountPlanCoverageForbiddenError("coverage authentication required")
    const proof = await new HeadcountPlanRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new HeadcountPlanCoverageForbiddenError("coverage authorization denied")
    const generation = await new HeadcountPlanRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new HeadcountPlanRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "headcount-plan-record")
    )
      return new HeadcountPlanCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new HeadcountPlanCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenHeadcountPlanRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
    ...
    if (
      ...
      return new HeadcountPlanCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareHeadcountPlanCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new HeadcountPlanCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new HeadcountPlanCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: HeadcountPlanRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new HeadcountPlanCoverageConflictError("coverage terminal page changed")
    return page
  }
}
