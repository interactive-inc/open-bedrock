...
import { z } from "zod"
import { headcountPlanSourceFreezeResponseSchema } from "@/contexts/headcount-plan/interface/http/response-schemas"
...
/** 現在の管理権限で人員計画記録の停止世代を読む。 */
export function createHeadcountPlanSourceFreezeReadHandlers() {
  return headcountPlanFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "headcount-plan" ||
      ...
      return c.json(headcountPlanSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
