...
import { VerifyHeadcountPlanRetirementPage } from "@/contexts/headcount-plan/application/verify-headcount-plan-retirement-page"
...
import { zAppHeadcountPlanRetirementVerificationReceipt } from "@/contexts/headcount-plan/interface/http/response-schemas"
...
export const POST = headcountPlanFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = headcountPlanRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyHeadcountPlanRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof HeadcountPlanRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppHeadcountPlanRetirementVerificationReceipt.parse({
