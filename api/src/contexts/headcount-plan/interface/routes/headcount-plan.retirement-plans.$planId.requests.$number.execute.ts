import { zAppHeadcountPlanRetirementExecution } from "@/contexts/headcount-plan/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { headcountPlanFactory } from "@/contexts/headcount-plan/interface/request-environment/headcount-plan-factory"
import { ExecuteHeadcountPlanRetirementAdapter } from "@/contexts/headcount-plan/infrastructure/adapters/execute-headcount-plan-retirement.adapter"
import {
...
export const POST = headcountPlanFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteHeadcountPlanRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppHeadcountPlanRetirementExecution.parse(submitted), 200)
  },
)
