...
import { CreateHeadcountPlanRetirementPlan } from "@/contexts/headcount-plan/application/create-headcount-plan-retirement-plan"
...
import { zAppHeadcountPlanRetirementPlan } from "@/contexts/headcount-plan/interface/http/response-schemas"
...
export const POST = headcountPlanFactory.createHandlers(
  ...
  zValidator("json", headcountPlanRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = headcountPlanRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    return c.json(
      zAppHeadcountPlanRetirementPlan.parse({
