...
import { VerifyHeadcountPlanCoveragePage } from "@/contexts/headcount-plan/application/verify-headcount-plan-coverage-page"
import {
  HeadcountPlanCoverageForbiddenError,
  HeadcountPlanCoverageConflictError,
} from "@/contexts/headcount-plan/application/errors"
import { zAppHeadcountPlanCoveragePageReceipt } from "@/contexts/headcount-plan/interface/http/response-schemas"
...
export const POST = headcountPlanFactory.createHandlers(
  ...
  zValidator("json", headcountPlanCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = headcountPlanCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyHeadcountPlanCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof HeadcountPlanCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof HeadcountPlanCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppHeadcountPlanCoveragePageReceipt.parse({
