import { zAppHeadcountPlanRetirementRequest } from "@/contexts/headcount-plan/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { headcountPlanFactory } from "@/contexts/headcount-plan/interface/request-environment/headcount-plan-factory"
import { SubmitHeadcountPlanRetirementRequestAdapter } from "@/contexts/headcount-plan/infrastructure/adapters/submit-headcount-plan-retirement-request.adapter"
import {
...
export const POST = headcountPlanFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitHeadcountPlanRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof HeadcountPlanRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof HeadcountPlanRetirementConflictError)
    ...
    return c.json(
      zAppHeadcountPlanRetirementRequest.parse(submitted.request),
