...
type Context = ConstructorParameters<typeof PrepareHeadcountPlanRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitHeadcountPlanRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = headcountPlanRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new HeadcountPlanRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareHeadcountPlanRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new HeadcountPlanRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "headcount-plan.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new HeadcountPlanRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new HeadcountPlanRetirementForbiddenError("retirement applicant differs")
      if (
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new HeadcountPlanRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareHeadcountPlanRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new HeadcountPlanRetirementForbiddenError("retirement decision qualification required")
