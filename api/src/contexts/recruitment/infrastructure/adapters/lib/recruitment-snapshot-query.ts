...
export function recruitmentSnapshotQuery(
  ...
  if (!parsed.success) return new Error("invalid recruitment record id")
  const id = parsed.data
  if (recordKind === "recruitment-position-record")
    return {
      sql: `SELECT json_object('format','recruitment-position-record','version',2,'position',json_object(
        'id',id,'legacy_id',legacy_id,'title',title,'department_code',department_code,'status',status,
        'note',note,'created_at',created_at)) AS snapshot_json FROM job_openings WHERE id=?1`,
      values: [id],
    }
  return {
    sql: `SELECT json_object('format','recruitment-candidate-record','version',2,'candidate',json_object(
      'id',id,'legacy_id',legacy_id,'position_id',position_id,'name',name,'email',email,'source',source,
