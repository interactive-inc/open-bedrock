...
import { VerifyRecruitmentCoveragePage } from "@/contexts/recruitment/application/verify-recruitment-coverage-page"
import {
  RecruitmentCoverageForbiddenError,
  RecruitmentCoverageConflictError,
} from "@/contexts/recruitment/application/errors"
import { zAppRecruitmentCoveragePageReceipt } from "@/contexts/recruitment/interface/http/response-schemas"
...
export const POST = recruitmentFactory.createHandlers(
  ...
  zValidator(
    "json",
    recruitmentCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = recruitmentCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyRecruitmentCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof RecruitmentCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof RecruitmentCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppRecruitmentCoveragePageReceipt.parse({
