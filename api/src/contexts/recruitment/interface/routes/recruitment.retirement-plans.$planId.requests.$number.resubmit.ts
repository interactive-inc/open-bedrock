import { zAppRecruitmentRetirementRequest } from "@/contexts/recruitment/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { recruitmentFactory } from "@/contexts/recruitment/interface/request-environment/recruitment-factory"
import { SubmitRecruitmentRetirementRequestAdapter } from "@/contexts/recruitment/infrastructure/adapters/submit-recruitment-retirement-request.adapter"
import {
...
export const POST = recruitmentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitRecruitmentRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof RecruitmentRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof RecruitmentRetirementConflictError)
    ...
    return c.json(
      zAppRecruitmentRetirementRequest.parse(submitted.request),
