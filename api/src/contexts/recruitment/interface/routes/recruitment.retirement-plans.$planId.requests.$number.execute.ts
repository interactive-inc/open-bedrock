import { zAppRecruitmentRetirementExecution } from "@/contexts/recruitment/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { recruitmentFactory } from "@/contexts/recruitment/interface/request-environment/recruitment-factory"
import { ExecuteRecruitmentRetirementAdapter } from "@/contexts/recruitment/infrastructure/adapters/execute-recruitment-retirement.adapter"
import {
...
export const POST = recruitmentFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteRecruitmentRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppRecruitmentRetirementExecution.parse(submitted), 200)
  },
)
