...
test("採用2台帳を停止中に人の承認で保全し、業務コードを外してもSystemから読める", async () => {
  ...
  const frozen = await app.request(
    "/recruitment/record-source-freezes",
    {
      ...
      body: JSON.stringify({ reason: "Retain recruitment evidence" }),
    },
    bindings,
  )
  ...
  for (const [recordKind, recordId] of sources) {
    const path = `/recruitment/records/${recordKind}/${recordId}/preservation-requests`
    const submitted = await request(path, {
      ...
      body: {
        ...
        conditions: {
          reason: "Retain original recruitment record",
