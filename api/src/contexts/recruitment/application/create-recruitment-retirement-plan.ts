import { PrepareRecruitmentRetirementPlanAdapter } from "@/contexts/recruitment/infrastructure/adapters/prepare-recruitment-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareRecruitmentRetirementPlanAdapter>[0]

/** recruitment記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateRecruitmentRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareRecruitmentRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new RecruitmentRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RecruitmentRetirementConflictError("retirement plan creation conflicts")
  }
}
