...
export class RejectCandidate {
  ...
  async execute(command: Command): Promise<RecruitmentCandidate | ApplicationError> {
    ...
    if (existing.canAdvanceTo("rejected") === false) {
      return new ConflictError(
        `cannot advance candidate from ${existing.stage} to ${"rejected"}`,
        "recruitment_stage_transition_invalid",
      )
    }

    const updated = await repository.updateCandidate(command.id, existing.withStage("rejected"))
