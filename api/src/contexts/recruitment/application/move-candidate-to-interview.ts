...
/** 応募者を面接へ進める。 */
export class MoveCandidateToInterview {
  ...
  async execute(command: Command): Promise<RecruitmentCandidate | ApplicationError> {
    ...
    if (existing.canAdvanceTo("interview") === false) {
      return new ConflictError(
        `cannot advance candidate from ${existing.stage} to ${"interview"}`,
        "recruitment_stage_transition_invalid",
      )
    }

    const updated = await repository.updateCandidate(command.id, existing.withStage("interview"))
