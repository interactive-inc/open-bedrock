...
/** 応募者を内定提示へ進める。 */
export class MoveCandidateToOffer {
  ...
  async execute(command: Command): Promise<RecruitmentCandidate | ApplicationError> {
    ...
    if (existing.canAdvanceTo("offer") === false) {
      return new ConflictError(
        `cannot advance candidate from ${existing.stage} to ${"offer"}`,
        "recruitment_stage_transition_invalid",
      )
    }

    const updated = await repository.updateCandidate(command.id, existing.withStage("offer"))
