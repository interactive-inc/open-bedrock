...
export class HireCandidate {
  ...
  async execute(command: Command): Promise<RecruitmentCandidate | ApplicationError> {
    ...
    if (existing.canAdvanceTo("hired") === false) {
      return new ConflictError(
        `cannot advance candidate from ${existing.stage} to ${"hired"}`,
        "recruitment_stage_transition_invalid",
      )
    }

    const updated = await repository.updateCandidate(command.id, existing.withStage("hired"))
