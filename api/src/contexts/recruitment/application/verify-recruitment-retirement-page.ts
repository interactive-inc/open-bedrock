import { PrepareRecruitmentRetirementPageAdapter } from "@/contexts/recruitment/infrastructure/adapters/prepare-recruitment-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareRecruitmentRetirementPageAdapter>[0]

/** recruitment記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyRecruitmentRetirementPage {
  ...
  async execute(
    ...
    const parsed = recruitmentRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RecruitmentRetirementForbiddenError("retirement authentication required")
    const authority = await new RecruitmentRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new RecruitmentRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new RecruitmentRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      return new RecruitmentRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new RecruitmentRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareRecruitmentRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new RecruitmentRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new RecruitmentRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new RecruitmentRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new RecruitmentRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new RecruitmentRetirementConflictError("retirement verification position already written")
  }
}
