...
test("11件のantisocial check記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createAntisocialCheckPreservationFixture(await pool.next())
  const creator = zAccountId.parse(creatorPerson.accountId)
  const antisocialCheckIds = Array.from(
    { length: 11 },
  ...
  expect(
    (
      await post("/antisocial-check/record-source-freezes", freezeId, {
        reason: "Preserve antisocial-check",
      })
    ).status,
  ).toBe(201)
  ...
  await execSql(database, "DROP TRIGGER fail_antisocial_check_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('c535be9a-af76-4337-8743-65c61e3029e3','system:record:export'); DROP TABLE antisocial_checks;",
  )
