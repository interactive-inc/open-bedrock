...
import { createAntisocialCheckPreservationFixture } from "@/contexts/antisocial-check/test/create-antisocial-check-preservation-fixture.test-support"
...
test("11件のantisocial check記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createAntisocialCheckPreservationFixture()
  const creator = zAccountId.parse(creatorPerson.accountId)
  const antisocialCheckIds = Array.from({ length: 11 }, (_, index) =>
  ...
  await database
    ...
    .run()
  for (const id of antisocialCheckIds) {
    const index = antisocialCheckIds.indexOf(id) + 1
    await database
      .prepare(`INSERT INTO antisocial_checks
      (id,requester_id,partner_name,partner_address,representative_name,result,status,created_at)
  ...
  expect(await database.prepare("SELECT count(*) AS total FROM antisocial_checks").first<number>("total")).toBe(11)
  const at = new Date()
  ...
  expect(
    (await post("/antisocial-check/record-source-freezes", freezeId, { reason: "Preserve antisocial-check" }))
      .status,
  ).toBe(201)
  const frozenInsert = await database
    .prepare(`INSERT INTO antisocial_checks
      (id,requester_id,partner_name,partner_address,representative_name,result,status,created_at)
      VALUES (?1,?2,'Partner',NULL,NULL,NULL,'requested','2030-01-01T00:00:00Z')`)
    .bind(crypto.randomUUID(), creatorPerson.employeeId)
    .run()
    .then(() => null, (error: unknown) => error)
  expect(frozenInsert).toBeInstanceOf(Error)
  const frozenAntisocialCheckPath = `/antisocial-check/antisocial-checks/${antisocialCheckIds[0]}`
  const frozenUpdate = await apiRequest(frozenAntisocialCheckPath, {
    method: "PUT",
    body: {
      partner_name: "Must not be updated",
  ...
  expect((await apiRequest(frozenAntisocialCheckPath, { method: "DELETE" })).status).toBe(409)
  const mappings = []
  for (const id of antisocialCheckIds) {
    const path = `/antisocial-check/antisocial-checks/${id}/preservation-requests`
  ...
  const sourcePath = `/antisocial-check/record-source-freezes/${freezeId}`
  ...
  expect(await firstCoverage.json()).toMatchObject({
    sequence: 1,
    nextCursor: antisocialCheckIds[9],
    recordCount: 10,
  })
  expect(
  ...
  expect(publicPlan).toMatchObject({ totalPages: 2, recordKinds: ["antisocial-check-record"] })
  const retirementPath = `/antisocial-check/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "antisocial-check-retirement",
    revision: 1,
    title: "Retire preserved antisocial-check",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/antisocial-check/retirement-plans/${planId}/verification-receipts`
  ...
  expect(
    await database.prepare("SELECT count(*) AS n FROM antisocial_checks").first<number>("n"),
  ).toBe(11)
  expect(
    await database
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved antisocial-check",
  }
  expect(
  ...
  bindings.ATTACHMENT_KEKS = originalKeys
  await database.exec(
    "CREATE TRIGGER fail_antisocial_check_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await database.exec("DROP TRIGGER fail_antisocial_check_retirement")
  ...
  await database.exec(
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('antisocial-check-test-manager','system:record:export'); DROP TABLE antisocial_checks;",
  )
