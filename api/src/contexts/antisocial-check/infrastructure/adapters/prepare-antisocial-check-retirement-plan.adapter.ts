...
/** antisocial check記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareAntisocialCheckRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = antisocialCheckRetirementPlanCommandSchema.safeParse(input)
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "antisocial-check",
        capability: { revision: 1, recordKinds: ["antisocial-check-record"] },
        coverage: [
          {
