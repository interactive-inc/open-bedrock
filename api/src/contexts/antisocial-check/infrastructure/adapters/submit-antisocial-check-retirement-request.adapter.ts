...
export class SubmitAntisocialCheckRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = antisocialCheckRetirementSubmissionCommandSchema.safeParse(input)
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new AntisocialCheckRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "antisocial-check.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new AntisocialCheckRetirementConflictError("retirement previous version is missing")
