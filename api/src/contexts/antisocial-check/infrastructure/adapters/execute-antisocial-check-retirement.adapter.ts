...
type Context = ConstructorParameters<typeof PrepareAntisocialCheckRetirementCurrentStateAdapter>[0]

/** 現在の全件保全と人の承認資格を再検査し、原記録を消さずに停止世代の撤去を確定する。 */
export class ExecuteAntisocialCheckRetirementAdapter {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    const current = await new PrepareAntisocialCheckRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (
      ...
      return new AntisocialCheckRetirementConflictError("retirement proposal version differs")
    ...
    if (
      ...
      return new AntisocialCheckRetirementForbiddenError("retirement applicant differs")
    if (
      ...
      return new AntisocialCheckRetirementConflictError("retirement verified scope differs")
    ...
    if (proposal.status !== "approved")
      return new AntisocialCheckRetirementConflictError("retirement is not approved")
    ...
    if (finalized instanceof Error) {
      ...
      return new AntisocialCheckRetirementConflictError("retirement changed before finalization", {
        cause: finalized,
      })
    }
