...
export class VerifyAntisocialCheckCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    ...
    const proof = await new AntisocialCheckRecordSystemAdapter(
      this.c,
    ...
    const generation = await new AntisocialCheckRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new AntisocialCheckRecordSystemAdapter({
      env: this.c.env,
  ...
  private async confirmTerminal(
    ...
    completion: AntisocialCheckRecordSystemAdapter,
  ) {
