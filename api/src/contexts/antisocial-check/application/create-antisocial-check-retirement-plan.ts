...
type Context = ConstructorParameters<typeof PrepareAntisocialCheckRetirementPlanAdapter>[0]

/** antisocial check記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateAntisocialCheckRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareAntisocialCheckRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new AntisocialCheckRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new AntisocialCheckRetirementConflictError("retirement plan creation conflicts")
  }
}
