...
import { z } from "zod"
import { antisocialCheckSourceFreezeResponseSchema } from "@/contexts/antisocial-check/interface/http/response-schemas"
...
/** 現在の管理権限でantisocial check記録の停止世代を読む。 */
export function createAntisocialCheckSourceFreezeReadHandlers() {
  return antisocialCheckFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "antisocial-check" ||
      ...
      return c.json(antisocialCheckSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
