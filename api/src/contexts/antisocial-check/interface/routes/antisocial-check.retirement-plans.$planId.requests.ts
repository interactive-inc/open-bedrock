import { zAppAntisocialCheckRetirementRequest } from "@/contexts/antisocial-check/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitAntisocialCheckRetirementRequestAdapter } from "@/contexts/antisocial-check/infrastructure/adapters/submit-antisocial-check-retirement-request.adapter"
import {
...
export const POST = antisocialCheckFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitAntisocialCheckRetirementRequestAdapter(c).execute(
      {
    ...
    return c.json(
      zAppAntisocialCheckRetirementRequest.parse(submitted.request),
