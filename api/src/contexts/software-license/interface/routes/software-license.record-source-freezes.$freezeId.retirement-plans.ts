...
import { CreateSoftwareLicenseRetirementPlan } from "@/contexts/software-license/application/create-software-license-retirement-plan"
...
import { zAppSoftwareLicenseRetirementPlan } from "@/contexts/software-license/interface/http/response-schemas"
...
export const POST = softwareLicenseFactory.createHandlers(
  ...
  zValidator("json", softwareLicenseRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = softwareLicenseRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateSoftwareLicenseRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppSoftwareLicenseRetirementPlan.parse({
