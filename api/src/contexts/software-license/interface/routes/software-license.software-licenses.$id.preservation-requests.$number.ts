...
export const GET = softwareLicenseFactory.createHandlers(
  ...
  async (c) => {
    ...
    if (caseGuard instanceof Error) throw new SoftwareLicenseHTTPException(503)
    ...
    const prepareOriginal = async () => {
      ...
      if (pending instanceof Error) throw new SoftwareLicenseHTTPException(503)
    ...
    if (recorded instanceof Error) throw new SoftwareLicenseHTTPException(409)
    return c.json(
      {
        number: proposal.number,
