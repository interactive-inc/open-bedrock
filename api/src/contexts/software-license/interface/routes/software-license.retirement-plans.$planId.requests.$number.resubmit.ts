import { zAppSoftwareLicenseRetirementRequest } from "@/contexts/software-license/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitSoftwareLicenseRetirementRequestAdapter } from "@/contexts/software-license/infrastructure/adapters/submit-software-license-retirement-request.adapter"
import {
...
export const POST = softwareLicenseFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitSoftwareLicenseRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    return c.json(
      zAppSoftwareLicenseRetirementRequest.parse(submitted.request),
