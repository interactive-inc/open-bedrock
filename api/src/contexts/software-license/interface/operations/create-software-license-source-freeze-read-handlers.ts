...
import { z } from "zod"
import { softwareLicenseSourceFreezeResponseSchema } from "@/contexts/software-license/interface/http/response-schemas"
...
/** 現在の管理権限でサービス利用台帳の停止世代を読む。 */
export function createSoftwareLicenseSourceFreezeReadHandlers() {
  return softwareLicenseFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "software-license" ||
      ...
      return c.json(softwareLicenseSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
