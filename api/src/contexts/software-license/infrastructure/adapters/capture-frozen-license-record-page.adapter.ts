...
import { ListFrozenLicenseRecordPageAdapter } from "@/contexts/software-license/infrastructure/adapters/list-frozen-license-record-page.adapter"
...
/** 停止したサービス利用台帳の原文を分割取得し、全取得後と後続保存時にも同じ停止世代を検査する。 */
export class CaptureFrozenLicenseRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const page = await new ListFrozenLicenseRecordPageAdapter(this.c).prepare(request)
    if (page instanceof Error) return page
    const records = []
    for (const recordId of page.recordIds) {
      const record = await new CaptureLicenseRecordAdapter(this.c).prepare({
        licenseId: recordId,
    ...
    try {
      ...
      if (checks.length !== page.assertions.length || checks.some((check) => !check.success))
        return new Error("frozen software-license capture changed")
    } catch (cause) {
      return new Error("frozen software-license capture changed", { cause })
    }
