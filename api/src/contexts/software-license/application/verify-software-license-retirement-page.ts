...
/** サービス利用台帳の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifySoftwareLicenseRetirementPage {
  ...
  async execute(
    ...
    const parsed = softwareLicenseRetirementVerificationCommandSchema.safeParse(input)
    ...
    if (
      ...
      plan.snapshot.ownerContext !== "software-license" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(["license-record"])
    )
    ...
    if (
      ...
      return new SoftwareLicenseRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new SoftwareLicenseRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareSoftwareLicenseRetirementPageAdapter(this.c).prepare(
      {
