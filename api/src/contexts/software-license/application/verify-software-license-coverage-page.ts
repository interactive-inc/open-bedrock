...
import {
  SoftwareLicenseCoverageForbiddenError,
  SoftwareLicenseCoverageConflictError,
} from "@/contexts/software-license/application/errors"
import { softwareLicenseCoveragePageCommandSchema } from "@/contexts/software-license/domain/schemas/software-license-coverage-page-command.schema"
...
export class VerifySoftwareLicenseCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = softwareLicenseCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new SoftwareLicenseCoverageForbiddenError("coverage authentication required")
    const proof = await new SoftwareLicenseRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new SoftwareLicenseCoverageForbiddenError("coverage authorization denied")
    const generation = await new SoftwareLicenseRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new SoftwareLicenseRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new SoftwareLicenseRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "license-record")
    )
      return new SoftwareLicenseCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new SoftwareLicenseCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenLicenseRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
    ...
    if (
      ...
      return new SoftwareLicenseCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareLicenseCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new SoftwareLicenseCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new SoftwareLicenseCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: SoftwareLicenseRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new SoftwareLicenseCoverageConflictError("coverage terminal page changed")
    return page
  }
}
