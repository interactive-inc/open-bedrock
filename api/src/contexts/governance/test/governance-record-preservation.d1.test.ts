...
test("規程機能を停止中に人の承認で保全し、業務コードを外してもSystemから読める", async () => {
  ...
  await execSql(
    database,
    `INSERT INTO governance_capabilities
  ...
  await database
    .prepare(`INSERT INTO system_step_up_grants
      ...
      VALUES ('296abf0d-2aaa-4d6a-bb9a-c7a8df733558',?1,?2,'external_identity',?3,?4)`)
  ...
  const frozen = await app.request(
    "/governance/record-source-freezes",
    {
      ...
      body: JSON.stringify({ reason: "Retain governance evidence" }),
    },
    bindings,
  )
  if (frozen.status !== 201) throw new Error(await frozen.text())
  await expect(
    database.prepare("UPDATE governance_capabilities SET name='changed' WHERE code='cap:a'").run(),
  ).rejects.toThrow("governance_record_source_frozen")
  const sources = [["governance-capability-record", encodeGovernanceRecordId(["cap:a"])]] as const
  const preservedIds: string[] = []
  for (const [recordKind, recordId] of sources) {
    const path = `/governance/records/${recordKind}/${encodeURIComponent(recordId)}/preservation-requests`
    const submitted = await request(path, {
      ...
      body: {
        ...
        conditions: {
          reason: "Retain original governance record",
