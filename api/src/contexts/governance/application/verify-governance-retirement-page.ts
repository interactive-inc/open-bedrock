import { PrepareGovernanceRetirementPageAdapter } from "@/contexts/governance/infrastructure/adapters/prepare-governance-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PrepareGovernanceRetirementPageAdapter>[0]

/** governance記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyGovernanceRetirementPage {
  ...
  async execute(
    ...
    const parsed = governanceRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new GovernanceRetirementForbiddenError("retirement authentication required")
    const authority = await new GovernanceRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new GovernanceRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new GovernanceRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      return new GovernanceRetirementConflictError(
        "retirement plan unavailable or capability changed",
      )
    const repository = new GovernanceRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new GovernanceRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new GovernanceRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareGovernanceRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new GovernanceRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new GovernanceRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new GovernanceRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new GovernanceRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new GovernanceRetirementConflictError("retirement verification position already written")
  }
}
