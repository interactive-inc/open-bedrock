...
import { VerifyGovernanceCoveragePage } from "@/contexts/governance/application/verify-governance-coverage-page"
import {
  GovernanceCoverageForbiddenError,
  GovernanceCoverageConflictError,
} from "@/contexts/governance/application/errors"
import { zAppGovernanceCoveragePageReceipt } from "@/contexts/governance/interface/http/response-schemas"
...
export const POST = governanceRecordFactory.createHandlers(
  ...
  zValidator(
    "json",
    governanceCoveragePageCommandSchema.pick({
      purpose: true,
      recordKind: true,
      records: true,
    }),
  ...
  async (c) => {
    ...
    const command = governanceCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyGovernanceCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof GovernanceCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof GovernanceCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppGovernanceCoveragePageReceipt.parse({
