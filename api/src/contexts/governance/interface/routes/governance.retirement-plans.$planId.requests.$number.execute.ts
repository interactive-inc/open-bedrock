import { zAppGovernanceRetirementExecution } from "@/contexts/governance/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { governanceRecordFactory } from "@/contexts/governance/interface/request-environment/governance-record-factory"
import { ExecuteGovernanceRetirementAdapter } from "@/contexts/governance/infrastructure/adapters/execute-governance-retirement.adapter"
import {
...
export const POST = governanceRecordFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteGovernanceRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppGovernanceRetirementExecution.parse(submitted), 200)
  },
)
