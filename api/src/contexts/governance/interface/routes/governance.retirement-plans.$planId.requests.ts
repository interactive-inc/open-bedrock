import { zAppGovernanceRetirementRequest } from "@/contexts/governance/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { governanceRecordFactory } from "@/contexts/governance/interface/request-environment/governance-record-factory"
import { SubmitGovernanceRetirementRequestAdapter } from "@/contexts/governance/infrastructure/adapters/submit-governance-retirement-request.adapter"
import {
...
export const POST = governanceRecordFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitGovernanceRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof GovernanceRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof GovernanceRetirementConflictError)
    ...
    return c.json(
      zAppGovernanceRetirementRequest.parse(submitted.request),
