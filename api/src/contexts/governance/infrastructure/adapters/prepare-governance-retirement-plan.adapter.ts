...
/** governance記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareGovernanceRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = governanceRetirementPlanCommandSchema.safeParse(input)
    ...
    const chains = []
    for (const recordKind of governanceRecordKinds) {
    ...
    const currentReader = await new GovernanceActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "governance",
        capability: { revision: 1, recordKinds: governanceRecordKinds },
