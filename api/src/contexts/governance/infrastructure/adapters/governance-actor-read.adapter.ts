...
export class GovernanceActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new GovernanceError("forbidden", "invalid governance actor")
    ...
    if (authorization instanceof Error)
      return new GovernanceError("governance_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new GovernanceError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new GovernanceError("governance_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new GovernanceError("governance_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new GovernanceError("forbidden", "active employee is required")
    return {
