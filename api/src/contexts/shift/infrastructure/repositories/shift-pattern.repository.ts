...
export class ShiftPatternRepository {
   ...
   */
  async delete(pattern: ShiftPattern): Promise<true | null | Error> {
    if (pattern.id === null) return new Error("cannot delete unsaved shift pattern")
    try {
      const result = await this.c.var.database.run(
        sql`DELETE FROM shift_patterns
            WHERE id = ${pattern.id}
            AND NOT EXISTS (
              SELECT 1 FROM shift_assignments WHERE pattern_id = ${pattern.id}
            )`,
      )
