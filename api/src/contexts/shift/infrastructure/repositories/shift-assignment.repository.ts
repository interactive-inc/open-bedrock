...
export class ShiftAssignmentRepository {
  ...
  async findByEmployeeId(employeeId: EmployeeId): Promise<ReadonlyArray<ShiftAssignment> | Error> {
    try {
      const rows = await this.c.var.database
        .select()
        .from(shiftAssignments)
  ...
  async delete(assignment: ShiftAssignment): Promise<true | null | Error> {
    if (assignment.id === null) return new Error("cannot delete unsaved shift assignment")
    try {
      const rows = await this.c.var.database
