...
/** shift記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareShiftRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = shiftRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new ShiftActorReadAdapter(this.c).prepare()
    if (reader instanceof Error) return reader
    const baseAssertions = [...authority.assertions, ...reader.assertions]
    const chains = []
    for (const recordKind of shiftRecordKinds) {
    ...
    const currentReader = await new ShiftActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "shift",
        capability: { revision: 1, recordKinds: shiftRecordKinds },
