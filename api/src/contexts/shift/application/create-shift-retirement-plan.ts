...
type Context = ConstructorParameters<typeof PrepareShiftRetirementPlanAdapter>[0]

/** shift記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateShiftRetirementPlan {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new ShiftRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new ShiftRetirementConflictError("retirement plan creation conflicts")
  }
}
