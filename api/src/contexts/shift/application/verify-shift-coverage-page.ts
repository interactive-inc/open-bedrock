...
import {
  ...
  ShiftCoverageConflictError,
} from "@/contexts/shift/application/errors"
import { shiftCoveragePageCommandSchema } from "@/contexts/shift/domain/schemas/shift-coverage-page-command.schema"
...
import { CaptureFrozenShiftRecordPageAdapter } from "@/contexts/shift/infrastructure/adapters/capture-frozen-shift-record-page.adapter"
import { PrepareShiftCoverageRecordsAdapter } from "@/contexts/shift/infrastructure/adapters/prepare-shift-coverage-records.adapter"

type Context = ShiftContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからshift記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyShiftCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = shiftCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new ShiftCoverageForbiddenError("coverage authentication required")
    const proof = await new ShiftRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new ShiftCoverageForbiddenError("coverage authorization denied")
    const generation = await new ShiftRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new ShiftRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      return new ShiftCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new ShiftCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenShiftRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new ShiftCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareShiftCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new ShiftCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new ShiftCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: ShiftRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new ShiftCoverageConflictError("coverage terminal page changed")
    return page
  }
}
