...
import { z } from "zod"
import { shiftSourceFreezeResponseSchema } from "@/contexts/shift/interface/http/response-schemas"
...
/** 現在の管理権限でshift記録の停止世代を読む。 */
export function createShiftSourceFreezeReadHandlers() {
  return shiftFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "shift" ||
      ...
      return c.json(shiftSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
