import { zAppShiftRetirementRequest } from "@/contexts/shift/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { shiftFactory } from "@/contexts/shift/interface/request-environment/shift-factory"
import { SubmitShiftRetirementRequestAdapter } from "@/contexts/shift/infrastructure/adapters/submit-shift-retirement-request.adapter"
import {
...
export const POST = shiftFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitShiftRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    return c.json(
      zAppShiftRetirementRequest.parse(submitted.request),
