import { createShiftRetirementDecisionHandlers } from "@/contexts/shift/interface/operations/create-shift-retirement-decision-handlers"
import { shiftFactory } from "@/contexts/shift/interface/request-environment/shift-factory"
import { authenticateSystemAccessToken } from "@system/interface/middlewares/authenticate-system-access-token"

// @authorization service - 現在の人間の認証とCompanyの承認資格でSystemの判断を記録する
export const POST = shiftFactory.createHandlers(
  authenticateSystemAccessToken,
  ...createShiftRetirementDecisionHandlers("approve"),
)
