...
import { VerifyShiftCoveragePage } from "@/contexts/shift/application/verify-shift-coverage-page"
import {
  ShiftCoverageForbiddenError,
  ShiftCoverageConflictError,
} from "@/contexts/shift/application/errors"
import { zAppShiftCoveragePageReceipt } from "@/contexts/shift/interface/http/response-schemas"
...
export const POST = shiftFactory.createHandlers(
  ...
  zValidator(
    "json",
    shiftCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = shiftCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyShiftCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof ShiftCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof ShiftCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppShiftCoveragePageReceipt.parse({
