...
export class ReviewFormRepository {
  ...
  async discloseByCycle(reviewCycle: ReviewCycle): Promise<number | Error> {
    try {
      if (reviewCycle.id === null) return new Error("cannot disclose unsaved review cycle")
      const rows = await this.c.var.database
        .update(reviewForms)
        .set({ visibility: "disclosed" })
        .where(eq(reviewForms.cycleId, reviewCycle.id))
