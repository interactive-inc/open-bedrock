...
export class EvaluationSheetRepository {
  ...
  async createWithAuditLog(
    ...
    try {
      ...
      type SheetRow = {
        ...
        secondary_evaluator_id: EmployeeId | null
  ...
  async totalGoalWeight(sheetId: number): Promise<number | Error> {
    try {
      ...
      return error instanceof Error ? error : new Error("failed to load evaluation sheet goals")
    }
  }

  async submitWithAuditLog(
    ...
    try {
      if (transitioned.id === null) return new Error("cannot submit unsaved evaluation sheet")
      const db = this.c.env.DB
      await db.batch([
        db
          .prepare(
      ...
      return saved ?? new Error("failed to read back submitted evaluation sheet")
    } catch (error) {
      if (isAbortedByGuard(error)) {
        return new ConflictError(
      ...
      return error instanceof Error ? error : new Error("failed to submit evaluation sheet")
    }
  }
