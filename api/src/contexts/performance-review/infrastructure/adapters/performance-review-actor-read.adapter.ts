...
export class PerformanceReviewActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new PerformanceReviewError("forbidden", "invalid performance review actor")
    ...
    if (actors[0]?.employee === undefined)
      return new PerformanceReviewError("forbidden", "active employee is required")
    return {
