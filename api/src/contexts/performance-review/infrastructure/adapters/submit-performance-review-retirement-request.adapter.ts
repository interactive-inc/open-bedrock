import { performanceReviewRetirementSubmissionCommandSchema } from "@/contexts/performance-review/domain/schemas/performance-review-retirement-submission-command.schema"
...
export class SubmitPerformanceReviewRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = performanceReviewRetirementSubmissionCommandSchema.safeParse(input)
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new PerformanceReviewRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "performance-review.record-retirement.request",
          actorAccountId: authentication.accountId,
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new PerformanceReviewRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PreparePerformanceReviewRetirementCurrentStateAdapter(
        this.c,
      ).prepare(
        {
          planId: command.planId,
    ...
    if (task.resolved.guards.length === 0)
      return new PerformanceReviewRetirementForbiddenError(
