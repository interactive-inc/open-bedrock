...
/** performanceReview記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PreparePerformanceReviewRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = performanceReviewRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new PerformanceReviewActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new PerformanceReviewActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "performance-review",
        capability: { revision: 1, recordKinds: performanceReviewRecordKinds },
