...
import { z } from "zod"
import { performanceReviewSourceFreezeResponseSchema } from "@/contexts/performance-review/interface/http/response-schemas"
...
/** 現在の管理権限でperformanceReview記録の停止世代を読む。 */
export function createPerformanceReviewSourceFreezeReadHandlers() {
  return performanceReviewFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "performance-review" ||
      ...
      return c.json(performanceReviewSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
