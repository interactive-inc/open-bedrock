...
import { CreatePerformanceReviewRetirementPlan } from "@/contexts/performance-review/application/create-performance-review-retirement-plan"
...
import { zAppPerformanceReviewRetirementPlan } from "@/contexts/performance-review/interface/http/response-schemas"
...
export const POST = performanceReviewFactory.createHandlers(
  ...
  zValidator("json", performanceReviewRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = performanceReviewRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreatePerformanceReviewRetirementPlan(c).execute(
      command.data,
      stepUpToken,
    )
    ...
    return c.json(
      zAppPerformanceReviewRetirementPlan.parse({
