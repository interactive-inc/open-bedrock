import { zAppPerformanceReviewRetirementRequest } from "@/contexts/performance-review/interface/http/response-schemas"
import { z } from "zod"
...
import { SubmitPerformanceReviewRetirementRequestAdapter } from "@/contexts/performance-review/infrastructure/adapters/submit-performance-review-retirement-request.adapter"
import {
...
export const POST = performanceReviewFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitPerformanceReviewRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof PerformanceReviewRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof PerformanceReviewRetirementConflictError)
    ...
    return c.json(
      zAppPerformanceReviewRetirementRequest.parse(submitted.request),
