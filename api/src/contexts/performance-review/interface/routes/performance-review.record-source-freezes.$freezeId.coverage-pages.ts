...
import { VerifyPerformanceReviewCoveragePage } from "@/contexts/performance-review/application/verify-performance-review-coverage-page"
import {
...
import { zAppPerformanceReviewCoveragePageReceipt } from "@/contexts/performance-review/interface/http/response-schemas"
...
export const POST = performanceReviewFactory.createHandlers(
  ...
  zValidator(
    "json",
    performanceReviewCoveragePageCommandSchema.pick({
      purpose: true,
      recordKind: true,
      records: true,
    }),
  ...
  async (c) => {
    ...
    const command = performanceReviewCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyPerformanceReviewCoveragePage(c).execute(
      command.data,
      stepUpToken,
    )
    if (receipt instanceof PerformanceReviewCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof PerformanceReviewCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppPerformanceReviewCoveragePageReceipt.parse({
