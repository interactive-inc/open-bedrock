import { zAppPerformanceReviewRetirementExecution } from "@/contexts/performance-review/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecutePerformanceReviewRetirementAdapter } from "@/contexts/performance-review/infrastructure/adapters/execute-performance-review-retirement.adapter"
import {
...
export const POST = performanceReviewFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecutePerformanceReviewRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppPerformanceReviewRetirementExecution.parse(submitted), 200)
  },
)
