...
import { RevalidatePerformanceReviewRecordSourceAdapter } from "@/contexts/performance-review/infrastructure/adapters/revalidate-performance-review-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidatePerformanceReviewRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
