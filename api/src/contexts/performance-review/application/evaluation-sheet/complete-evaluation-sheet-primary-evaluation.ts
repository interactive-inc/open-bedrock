...
/** 評価シートの一次評価を完了する。 */
export class CompleteEvaluationSheetPrimaryEvaluation {
  ...
  async execute(command: Command): Promise<EvaluationSheet | ApplicationError> {
    ...
    const transitioned = existing.transition("secondary_eval", command.now)

    if (transitioned === null) {
      return new ConflictError(
        `cannot transition from ${existing.status} to secondary_eval`,
