...
/** 評価シートの自己評価を完了する。 */
export class CompleteEvaluationSheetSelfEvaluation {
  ...
  async execute(command: Command): Promise<EvaluationSheet | ApplicationError> {
    ...
    const transitioned = existing.transition("primary_eval", command.now)

    if (transitioned === null) {
      return new ConflictError(
        `cannot transition from ${existing.status} to primary_eval`,
