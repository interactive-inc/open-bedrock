import { PreparePerformanceReviewRetirementPageAdapter } from "@/contexts/performance-review/infrastructure/adapters/prepare-performance-review-retirement-page.adapter"
...
type Context = ConstructorParameters<typeof PreparePerformanceReviewRetirementPageAdapter>[0]

/** performanceReview記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyPerformanceReviewRetirementPage {
  ...
  async execute(
    ...
    const parsed = performanceReviewRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new PerformanceReviewRetirementForbiddenError("retirement authentication required")
    const authority = await new PerformanceReviewRecordSystemAdapter(
      this.c,
    ...
    if (authority === "forbidden")
      return new PerformanceReviewRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new PerformanceReviewRecordSystemAdapter(context)
      .retirementPlans()
      .find(command.planId)
    ...
    if (
      ...
      return new PerformanceReviewRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new PerformanceReviewRetirementConflictError("all plan pages already verified")
    const checked = await new PreparePerformanceReviewRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new PerformanceReviewRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new PerformanceReviewRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new PerformanceReviewRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new PerformanceReviewRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new PerformanceReviewRetirementConflictError(
