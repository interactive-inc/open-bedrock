...
beforeAll(async () => {
  local = await startLocalD1({
    migrated: ["stale-edit", "new-article-retry-key"],
  })
})
...
test("a stale knowledge edit cannot replace the text another edit already saved", async () => {
  const fixture = await createLocalD1Context(local, "stale-edit", { withCompanyOrganization: true })
...
test("new article API records the initial version and requires a stable retry key", async () => {
  const fixture = await createLocalD1Context(local, "new-article-retry-key", {
