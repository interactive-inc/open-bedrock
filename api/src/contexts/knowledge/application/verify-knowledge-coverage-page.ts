...
import {
  KnowledgeCoverageForbiddenError,
  KnowledgeCoverageConflictError,
} from "@/contexts/knowledge/application/errors"
import { knowledgeCoveragePageCommandSchema } from "@/contexts/knowledge/domain/schemas/knowledge-coverage-page-command.schema"
...
export class VerifyKnowledgeCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = knowledgeCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new KnowledgeCoverageForbiddenError("coverage authentication required")
    const proof = await new KnowledgeRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new KnowledgeCoverageForbiddenError("coverage authorization denied")
    const generation = await new KnowledgeRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new KnowledgeRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "knowledge-article-record")
    )
      return new KnowledgeCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new KnowledgeCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenKnowledgeRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
    ...
    if (
      ...
      return new KnowledgeCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareKnowledgeCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new KnowledgeCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new KnowledgeCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: KnowledgeRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new KnowledgeCoverageConflictError("coverage terminal page changed")
    return page
  }
}
