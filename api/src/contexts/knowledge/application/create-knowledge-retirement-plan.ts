...
type Context = ConstructorParameters<typeof PrepareKnowledgeRetirementPlanAdapter>[0]

/** knowledge記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateKnowledgeRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareKnowledgeRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new KnowledgeRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new KnowledgeRetirementConflictError("retirement plan creation conflicts")
  }
}
