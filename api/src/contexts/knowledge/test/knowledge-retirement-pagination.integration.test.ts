...
test("ID 0 を含む11件のナレッジ記事と全改訂履歴を保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createKnowledgePreservationFixture()
  ...
  await database.exec("DROP TRIGGER fail_knowledge_retirement")
  ...
  await database.exec(
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('knowledge-test-manager','system:record:export'); DROP TABLE knowledge_article_revisions; DROP TABLE knowledge_articles;",
  )
