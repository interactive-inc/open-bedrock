...
import { ListFrozenKnowledgeRecordPageAdapter } from "@/contexts/knowledge/infrastructure/adapters/list-frozen-knowledge-article-record-page.adapter"
...
/** 停止したknowledge記録の原文を分割取得し、全取得後と後続保存時にも同じ停止世代を検査する。 */
export class CaptureFrozenKnowledgeRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const page = await new ListFrozenKnowledgeRecordPageAdapter(this.c).prepare(request)
    if (page instanceof Error) return page
    const records = []
    for (const recordId of page.recordIds) {
      const record = await new CaptureKnowledgeRecordAdapter(this.c).prepare({
        articleId: recordId,
    ...
    try {
      ...
      if (checks.length !== page.assertions.length || checks.some((check) => !check.success))
        return new Error("frozen knowledge capture changed")
    } catch (cause) {
      return new Error("frozen knowledge capture changed", { cause })
    }
