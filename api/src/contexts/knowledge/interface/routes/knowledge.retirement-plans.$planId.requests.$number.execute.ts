import { zAppKnowledgeRetirementExecution } from "@/contexts/knowledge/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { knowledgeFactory } from "@/contexts/knowledge/interface/request-environment/knowledge-factory"
import { ExecuteKnowledgeRetirementAdapter } from "@/contexts/knowledge/infrastructure/adapters/execute-knowledge-retirement.adapter"
import {
...
export const POST = knowledgeFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteKnowledgeRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppKnowledgeRetirementExecution.parse(submitted), 200)
  },
)
