...
import { VerifyKnowledgeRetirementPage } from "@/contexts/knowledge/application/verify-knowledge-retirement-page"
...
import { zAppKnowledgeRetirementVerificationReceipt } from "@/contexts/knowledge/interface/http/response-schemas"
...
export const POST = knowledgeFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = knowledgeRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyKnowledgeRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof KnowledgeRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof KnowledgeRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppKnowledgeRetirementVerificationReceipt.parse({
