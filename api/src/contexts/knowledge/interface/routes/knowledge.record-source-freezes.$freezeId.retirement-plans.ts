...
import { CreateKnowledgeRetirementPlan } from "@/contexts/knowledge/application/create-knowledge-retirement-plan"
import { knowledgeRetirementPlanCommandSchema } from "@/contexts/knowledge/domain/schemas/knowledge-retirement-plan-command.schema"
import { zAppKnowledgeRetirementPlan } from "@/contexts/knowledge/interface/http/response-schemas"
...
export const POST = knowledgeFactory.createHandlers(
  ...
  zValidator("json", knowledgeRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = knowledgeRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateKnowledgeRetirementPlan(c).execute(command.data, stepUpToken)
    ...
    return c.json(
      zAppKnowledgeRetirementPlan.parse({
