...
import { VerifyKnowledgeCoveragePage } from "@/contexts/knowledge/application/verify-knowledge-coverage-page"
import {
  KnowledgeCoverageForbiddenError,
  KnowledgeCoverageConflictError,
} from "@/contexts/knowledge/application/errors"
import { zAppKnowledgeCoveragePageReceipt } from "@/contexts/knowledge/interface/http/response-schemas"
...
export const POST = knowledgeFactory.createHandlers(
  ...
  zValidator("json", knowledgeCoveragePageCommandSchema.pick({ purpose: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = knowledgeCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyKnowledgeCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof KnowledgeCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof KnowledgeCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppKnowledgeCoveragePageReceipt.parse({
