...
import { RevalidateKnowledgeRecordSourceAdapter } from "@/contexts/knowledge/infrastructure/adapters/revalidate-knowledge-article-record-source.adapter"
...
export const POST = knowledgeFactory.createHandlers(
  ...
  zValidator("param", z.strictObject({ id: knowledgeIdSchema, number: z.coerce.number().int().positive().safe() })),
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateKnowledgeRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
