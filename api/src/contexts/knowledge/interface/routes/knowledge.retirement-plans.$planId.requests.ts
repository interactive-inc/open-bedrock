import { zAppKnowledgeRetirementRequest } from "@/contexts/knowledge/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { knowledgeFactory } from "@/contexts/knowledge/interface/request-environment/knowledge-factory"
import { SubmitKnowledgeRetirementRequestAdapter } from "@/contexts/knowledge/infrastructure/adapters/submit-knowledge-retirement-request.adapter"
import {
...
export const POST = knowledgeFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitKnowledgeRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof KnowledgeRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof KnowledgeRetirementConflictError)
    ...
    return c.json(
      zAppKnowledgeRetirementRequest.parse(submitted.request),
