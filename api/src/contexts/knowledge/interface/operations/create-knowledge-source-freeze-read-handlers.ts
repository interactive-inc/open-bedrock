...
import { z } from "zod"
import { knowledgeSourceFreezeResponseSchema } from "@/contexts/knowledge/interface/http/response-schemas"
...
/** 現在の管理権限でknowledge記録の停止世代を読む。 */
export function createKnowledgeSourceFreezeReadHandlers() {
  return knowledgeFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "knowledge" ||
      ...
      return c.json(knowledgeSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
