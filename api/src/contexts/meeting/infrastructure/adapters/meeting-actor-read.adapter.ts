...
export class MeetingActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new MeetingError("forbidden", "invalid meeting actor")
    ...
    if (authorization instanceof Error)
      return new MeetingError("meeting_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new MeetingError("meeting_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new MeetingError("forbidden", "active employee is required")
    return {
