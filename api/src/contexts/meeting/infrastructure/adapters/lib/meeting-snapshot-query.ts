...
export function meetingSnapshotQuery(
  ...
  if (!parsed.success) return new Error("invalid meeting record id")
  const id = parsed.data
  if (recordKind === "meeting-record")
    return {
      sql: `SELECT json_object('format','meeting-record','version',2,'meeting',json_object(
        'id',id,'legacy_id',legacy_id,'code',code,'name',name,'cadence',cadence,'description',description,
        'status',status,'created_at',created_at)) AS snapshot_json FROM meetings WHERE id=?1`,
      values: [id],
    }
  if (recordKind === "meeting-minutes-record")
    return {
      sql: `SELECT json_object('format','meeting-minutes-record','version',2,'minutes',json_object(
        'id',id,'legacy_id',legacy_id,'meeting_id',meeting_id,'held_on',held_on,'title',title,'attendees',attendees,
  ...
  return {
    sql: `SELECT json_object('format','meeting-decision-record','version',2,'decision',json_object(
      'id',id,'legacy_id',legacy_id,'title',title,'decided_on',decided_on,'context',context,'decision',decision,
