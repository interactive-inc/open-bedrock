...
import { z } from "zod"
import { meetingSourceFreezeResponseSchema } from "@/contexts/meeting/interface/http/response-schemas"
...
/** 現在の管理権限でmeeting記録の停止世代を読む。 */
export function createMeetingSourceFreezeReadHandlers() {
  return meetingFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "meeting" ||
      ...
      return c.json(meetingSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
