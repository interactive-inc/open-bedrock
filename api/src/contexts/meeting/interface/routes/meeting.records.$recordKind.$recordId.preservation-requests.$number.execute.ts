...
import { RevalidateMeetingRecordSourceAdapter } from "@/contexts/meeting/infrastructure/adapters/revalidate-meeting-record-source.adapter"
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateMeetingRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
