...
import { VerifyMeetingCoveragePage } from "@/contexts/meeting/application/verify-meeting-coverage-page"
import {
  MeetingCoverageForbiddenError,
  MeetingCoverageConflictError,
} from "@/contexts/meeting/application/errors"
import { zAppMeetingCoveragePageReceipt } from "@/contexts/meeting/interface/http/response-schemas"
...
export const POST = meetingFactory.createHandlers(
  ...
  zValidator(
    "json",
    meetingCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = meetingCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyMeetingCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof MeetingCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof MeetingCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppMeetingCoveragePageReceipt.parse({
