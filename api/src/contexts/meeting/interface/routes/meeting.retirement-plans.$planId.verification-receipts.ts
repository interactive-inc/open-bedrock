...
import { VerifyMeetingRetirementPage } from "@/contexts/meeting/application/verify-meeting-retirement-page"
...
import { zAppMeetingRetirementVerificationReceipt } from "@/contexts/meeting/interface/http/response-schemas"
...
export const POST = meetingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = meetingRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyMeetingRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof MeetingRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppMeetingRetirementVerificationReceipt.parse({
