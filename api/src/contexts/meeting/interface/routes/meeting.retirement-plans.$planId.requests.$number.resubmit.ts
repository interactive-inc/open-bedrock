import { zAppMeetingRetirementRequest } from "@/contexts/meeting/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { meetingFactory } from "@/contexts/meeting/interface/request-environment/meeting-factory"
import { SubmitMeetingRetirementRequestAdapter } from "@/contexts/meeting/infrastructure/adapters/submit-meeting-retirement-request.adapter"
import {
...
export const POST = meetingFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitMeetingRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    return c.json(
      zAppMeetingRetirementRequest.parse(submitted.request),
