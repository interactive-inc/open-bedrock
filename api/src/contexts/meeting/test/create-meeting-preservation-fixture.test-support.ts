...
const secret = "meeting-integration-test-secret"

/** 実System・Companyと会議3台帳の原記録を接続する保全fixture。 */
export async function createMeetingPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "meeting-preservation",
    revision: 1,
    title: "Preserve meeting records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "meeting-pagination-test-pepper",
