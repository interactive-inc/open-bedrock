...
test("会議の停止世代は3台帳の追加・更新・削除をDB確定時に拒否する", () => {
  const database = new Database(":memory:")
  database.exec(`
    CREATE TABLE system_record_source_freezes (owner_context TEXT NOT NULL, revision INTEGER NOT NULL);
    CREATE TABLE meetings (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
    CREATE TABLE meeting_minutes_records (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
    CREATE TABLE decision_records (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
  `)
  const migration = readFileSync(
    new URL("../../../../migrations/0303_guard_meeting_record_source_freeze.sql", import.meta.url),
    "utf8",
  )
  database.exec(migration)
  const tables = ["meetings", "meeting_minutes_records", "decision_records"]
  ...
  database.exec(
    "UPDATE system_record_source_freezes SET revision = 2 WHERE owner_context = 'meeting'",
  )
  for (const table of tables) {
