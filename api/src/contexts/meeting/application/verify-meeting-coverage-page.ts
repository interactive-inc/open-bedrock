...
import {
  ...
  MeetingCoverageConflictError,
} from "@/contexts/meeting/application/errors"
import { meetingCoveragePageCommandSchema } from "@/contexts/meeting/domain/schemas/meeting-coverage-page-command.schema"
...
import { CaptureFrozenMeetingRecordPageAdapter } from "@/contexts/meeting/infrastructure/adapters/capture-frozen-meeting-record-page.adapter"
import { PrepareMeetingCoverageRecordsAdapter } from "@/contexts/meeting/infrastructure/adapters/prepare-meeting-coverage-records.adapter"

type Context = MeetingContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからmeeting記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyMeetingCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = meetingCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new MeetingCoverageForbiddenError("coverage authentication required")
    const proof = await new MeetingRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new MeetingCoverageForbiddenError("coverage authorization denied")
    const generation = await new MeetingRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new MeetingRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new MeetingRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      return new MeetingCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new MeetingCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenMeetingRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new MeetingCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareMeetingCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new MeetingCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new MeetingCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: MeetingRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new MeetingCoverageConflictError("coverage terminal page changed")
    return page
  }
}
