import { PrepareMeetingRetirementPlanAdapter } from "@/contexts/meeting/infrastructure/adapters/prepare-meeting-retirement-plan.adapter"
...
type Context = ConstructorParameters<typeof PrepareMeetingRetirementPlanAdapter>[0]

/** meeting記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateMeetingRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareMeetingRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new MeetingRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new MeetingRetirementConflictError("retirement plan creation conflicts")
  }
}
