...
type Context = ConstructorParameters<typeof PrepareDisciplinaryActionRetirementPlanAdapter>[0]

/** 懲戒記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateDisciplinaryActionRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareDisciplinaryActionRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new DisciplinaryActionRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new DisciplinaryActionRetirementConflictError("retirement plan creation conflicts")
  }
}
