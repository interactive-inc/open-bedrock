...
import {
  DisciplinaryActionCoverageForbiddenError,
  DisciplinaryActionCoverageConflictError,
} from "@/contexts/disciplinary-action/application/errors"
import { disciplinaryActionCoveragePageCommandSchema } from "@/contexts/disciplinary-action/domain/schemas/disciplinary-action-coverage-page-command.schema"
...
export class VerifyDisciplinaryActionCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = disciplinaryActionCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new DisciplinaryActionCoverageForbiddenError("coverage authentication required")
    const proof = await new DisciplinaryActionRecordSystemAdapter(
      this.c,
    ...
    if (proof === "forbidden")
      return new DisciplinaryActionCoverageForbiddenError("coverage authorization denied")
    const generation = await new DisciplinaryActionRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new DisciplinaryActionRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      (existing.snapshot.freezeId !== command.freezeId ||
        ...
        existing.snapshot.recordKind !== "disciplinary-action-record")
    )
      return new DisciplinaryActionCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new DisciplinaryActionCoverageConflictError("coverage scan already complete")
    const cursor =
    ...
    const page = await new CaptureFrozenDisciplinaryActionRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
    ...
    if (
      ...
      return new DisciplinaryActionCoverageConflictError("coverage mapping or purpose conflicts")
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new DisciplinaryActionCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new DisciplinaryActionCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: DisciplinaryActionRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new DisciplinaryActionCoverageConflictError("coverage terminal page changed")
    return page
  }
}
