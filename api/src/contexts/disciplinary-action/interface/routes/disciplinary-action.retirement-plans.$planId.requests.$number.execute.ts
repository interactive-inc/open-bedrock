import { zAppDisciplinaryActionRetirementExecution } from "@/contexts/disciplinary-action/interface/http/response-schemas"
import { z } from "zod"
...
import { ExecuteDisciplinaryActionRetirementAdapter } from "@/contexts/disciplinary-action/infrastructure/adapters/execute-disciplinary-action-retirement.adapter"
import {
...
export const POST = disciplinaryActionFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteDisciplinaryActionRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppDisciplinaryActionRetirementExecution.parse(submitted), 200)
  },
)
