...
import { z } from "zod"
import { disciplinaryActionSourceFreezeResponseSchema } from "@/contexts/disciplinary-action/interface/http/response-schemas"
...
/** 現在の管理権限で懲戒記録の停止世代を読む。 */
export function createDisciplinaryActionSourceFreezeReadHandlers() {
  return disciplinaryActionFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "disciplinary-action" ||
      ...
      return c.json(disciplinaryActionSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
