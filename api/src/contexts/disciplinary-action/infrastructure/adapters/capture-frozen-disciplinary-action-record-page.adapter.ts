...
import { ListFrozenDisciplinaryActionRecordPageAdapter } from "@/contexts/disciplinary-action/infrastructure/adapters/list-frozen-disciplinary-action-record-page.adapter"
...
/** 停止した懲戒記録の原文を分割取得し、全取得後と後続保存時にも同じ停止世代を検査する。 */
export class CaptureFrozenDisciplinaryActionRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const page = await new ListFrozenDisciplinaryActionRecordPageAdapter(this.c).prepare(request)
    if (page instanceof Error) return page
    const records = []
    for (const recordId of page.recordIds) {
      const record = await new CaptureDisciplinaryActionRecordAdapter(this.c).prepare({
        disciplinaryActionId: recordId,
    ...
    try {
      ...
      if (checks.length !== page.assertions.length || checks.some((check) => !check.success))
        return new Error("frozen disciplinary-action capture changed")
    } catch (cause) {
      return new Error("frozen disciplinary-action capture changed", { cause })
    }
