...
/** 懲戒記録の照合終端と現在の収集資格を検査し、撤去前の検証対象を固定する。 */
export class PrepareDisciplinaryActionRetirementPlanAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(input: unknown, stepUpToken: string) {
    const parsed = disciplinaryActionRetirementPlanCommandSchema.safeParse(input)
    ...
    const reader = await new DisciplinaryActionActorReadAdapter(this.c).prepare()
    ...
    const currentReader = await new DisciplinaryActionActorReadAdapter(this.c).prepare()
    ...
    try {
      ...
      const plan = await RecordRetirementVerificationPlanEntity.create({
        ...request,
        ownerContext: "disciplinary-action",
        capability: { revision: 1, recordKinds: ["disciplinary-action-record"] },
        coverage: [
          {
