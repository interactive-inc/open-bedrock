      ...
      const page =
        request.afterId === null
          ? this.c.env.DB.prepare(
              "SELECT id FROM disciplinary_actions ORDER BY id COLLATE BINARY LIMIT ?1",
            ).bind(request.limit + 1)
          : this.c.env.DB.prepare(
              "SELECT id FROM disciplinary_actions WHERE id COLLATE BINARY>?1 ORDER BY id COLLATE BINARY LIMIT ?2",
