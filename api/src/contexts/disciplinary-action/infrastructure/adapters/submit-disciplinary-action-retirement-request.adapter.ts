...
export class SubmitDisciplinaryActionRetirementRequestAdapter {
  ...
  async execute(input: unknown, stepUpToken: string) {
    const parsed = disciplinaryActionRetirementSubmissionCommandSchema.safeParse(input)
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new DisciplinaryActionRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "disciplinary-action.record-retirement.request",
          actorAccountId: authentication.accountId,
