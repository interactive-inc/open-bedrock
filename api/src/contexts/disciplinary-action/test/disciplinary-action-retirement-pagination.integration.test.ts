...
test("11件の懲戒記録を全件保全し、人の承認・取消・再提出を経て原記録を残して撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createDisciplinaryActionPreservationFixture()
  ...
  expect(
    (await post("/disciplinary-action/record-source-freezes", freezeId, { reason: "Preserve disciplinary-action" }))
      .status,
  ).toBe(201)
  expect(
    (
  ...
  await database.exec("DROP TRIGGER fail_disciplinary_action_retirement")
