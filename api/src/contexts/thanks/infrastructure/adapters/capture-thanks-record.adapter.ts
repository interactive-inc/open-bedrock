...
import { thanksRecordKindSchema, type ThanksRecordKind } from "@/contexts/thanks/domain/thanks-record-kind"
...
function snapshotQuery(recordKind: ThanksRecordKind, recordId: string): SnapshotQuery | Error {
  ...
  if (!parsed.success || String(parsed.data) !== recordId) return new Error("invalid thanks record id")
  const id = parsed.data
  if (recordKind === "thanks-message-record") return {
    sql: `SELECT json_object('format','thanks-message-record','version',1,'message',json_object(
      ...
      FROM thanks_messages WHERE id=?1`, values: [id],
  }
  if (recordKind === "thanks-point-budget-record") return {
    sql: `SELECT json_object('format','thanks-point-budget-record','version',1,'budget',json_object(
      ...
      FROM thanks_point_budgets WHERE id=?1`, values: [id],
  }
  if (recordKind === "thanks-reward-record") return {
    sql: `SELECT json_object('format','thanks-reward-record','version',1,'reward',json_object(
      ...
      AS snapshot_json FROM thanks_rewards WHERE id=?1`, values: [id],
  }
  return {
    sql: `SELECT json_object('format','thanks-redemption-record','version',1,'redemption',json_object(
      ...
      FROM thanks_redemptions WHERE id=?1`, values: [id],
  }
}
...
export class CaptureThanksRecordAdapter {
  ...
  async prepare(input: Readonly<{ recordKind: ThanksRecordKind; recordId: string; sourceNamespace: string }>) {
    ...
    if (query instanceof Error) return new ThanksError("forbidden", "invalid source record", { cause: query })
    ...
    try {
      ...
      const source = PreservedRecordSourceValue.create({
        sourceNamespace: input.sourceNamespace, ownerContext: "thanks", recordKind: kind.data,
        recordId: input.recordId, formatId: kind.data, formatVersion: 1, sourceRevision: null,
        sourceRecordedAt: null, capturedAt: actor.now.toISOString(), contentDigest: digest.toString(),
      })
      if (source instanceof Error) return source
      return {
        source, content: new TextEncoder().encode(canonical.toString()), actorAccountId: actor.accountId,
        sourceAuthorizationRef: Object.freeze({
          context: "thanks", kind: "record-snapshot", id: input.recordId, version: digest.toString(),
        }),
        assertions: [
          ...actor.assertions,
