...
import { thanksRetirementSubmissionCommandSchema } from "@/contexts/thanks/domain/schemas/thanks-retirement-submission-command.schema"
import { PrepareThanksRetirementCurrentStateAdapter } from "@/contexts/thanks/infrastructure/adapters/prepare-thanks-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareThanksRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitThanksRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = thanksRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new ThanksRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareThanksRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new ThanksRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "thanks.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new ThanksRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new ThanksRetirementForbiddenError("retirement applicant differs")
      if (
        ...
        return new ThanksRetirementConflictError("retirement previous version is not resubmittable")
      ...
      if (
        ...
        return new ThanksRetirementConflictError(
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new ThanksRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareThanksRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new ThanksRetirementForbiddenError("retirement decision qualification required")
    ...
    if (started instanceof Error) {
      ...
      return new ThanksRetirementConflictError("retirement submission changed before persistence", {
        cause: started,
      })
    }
    return {
