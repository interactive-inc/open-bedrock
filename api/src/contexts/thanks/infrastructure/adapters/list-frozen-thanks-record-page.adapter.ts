...
const inputSchema = z.strictObject({
  ...
  recordKind: thanksRecordKindSchema, afterCursor: z.string().nullable(),
...
export class ListFrozenThanksRecordPageAdapter {
  ...
  async prepare(input: unknown) {
    ...
    const generation = await new RecordSourceFreezeRepository({ env: this.c.env, assertions: actor.assertions })
      .prepareActiveGeneration({ id: request.freezeId, sourceNamespace: request.sourceNamespace, ownerContext: "thanks" })
    ...
    try {
      const page = this.c.env.DB.prepare(`SELECT id AS record_id FROM ${tableByKind[request.recordKind]} WHERE id>?1 ORDER BY id LIMIT ?2`)
      ...
      return Object.freeze({
        freezeId: generation.freeze.snapshot.id, recordIds: Object.freeze(recordIds),
