...
export class ThanksActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!account.success || !Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new ThanksError("forbidden", "invalid thanks actor")
    ...
    if (authorization instanceof Error)
      return new ThanksError("thanks_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new ThanksError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new ThanksError("thanks_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new ThanksError("thanks_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new ThanksError("forbidden", "active employee is required")
    return {
