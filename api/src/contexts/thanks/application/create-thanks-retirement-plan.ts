import { PrepareThanksRetirementPlanAdapter } from "@/contexts/thanks/infrastructure/adapters/prepare-thanks-retirement-plan.adapter"
...
import { ThanksRetirementConflictError } from "@/contexts/thanks/application/errors"

type Context = ConstructorParameters<typeof PrepareThanksRetirementPlanAdapter>[0]

/** thanks記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateThanksRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareThanksRetirementPlanAdapter(this.c).prepare(
      input,
      stepUpToken,
    )
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new ThanksRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new ThanksRetirementConflictError("retirement plan creation conflicts")
  }
}
