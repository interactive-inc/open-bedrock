...
import { VerifyThanksCoveragePage } from "@/contexts/thanks/application/verify-thanks-coverage-page"
import {
  ThanksCoverageForbiddenError,
  ThanksCoverageConflictError,
} from "@/contexts/thanks/application/errors"
import { zAppThanksCoveragePageReceipt } from "@/contexts/thanks/interface/http/response-schemas"
...
export const POST = thanksFactory.createHandlers(
  ...
  zValidator("json", thanksCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = thanksCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyThanksCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof ThanksCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof ThanksCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppThanksCoveragePageReceipt.parse({
