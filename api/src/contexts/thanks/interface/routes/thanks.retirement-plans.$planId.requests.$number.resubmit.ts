import { zAppThanksRetirementRequest } from "@/contexts/thanks/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { thanksFactory } from "@/contexts/thanks/interface/request-environment/thanks-factory"
import { SubmitThanksRetirementRequestAdapter } from "@/contexts/thanks/infrastructure/adapters/submit-thanks-retirement-request.adapter"
import {
...
export const POST = thanksFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitThanksRetirementRequestAdapter(c).execute(
      {
        revision: {
          mode: "resubmit",
    ...
    if (
      submitted instanceof ThanksRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof ThanksRetirementConflictError)
    ...
    return c.json(
      zAppThanksRetirementRequest.parse(submitted.request),
