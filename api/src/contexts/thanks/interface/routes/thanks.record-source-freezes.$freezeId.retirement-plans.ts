...
import { CreateThanksRetirementPlan } from "@/contexts/thanks/application/create-thanks-retirement-plan"
import { thanksRetirementPlanCommandSchema } from "@/contexts/thanks/domain/schemas/thanks-retirement-plan-command.schema"
import { zAppThanksRetirementPlan } from "@/contexts/thanks/interface/http/response-schemas"
...
export const POST = thanksFactory.createHandlers(
  ...
  zValidator("json", thanksRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = thanksRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateThanksRetirementPlan(c).execute(command.data, stepUpToken)
    if (receipt instanceof ForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof ThanksRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppThanksRetirementPlan.parse({
