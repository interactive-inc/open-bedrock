...
import { RevalidateThanksRecordSourceAdapter } from "@/contexts/thanks/infrastructure/adapters/revalidate-thanks-record-source.adapter"
...
export const POST = thanksFactory.createHandlers(
  ...
  zValidator("param", thanksRecordRouteSchema.extend({ number: z.coerce.number().int().positive().safe() })),
  ...
  async (c) => {
    ...
    const result = await new ExecuteRecordPreservationAdapter({
      ...
      source: {
        ...
        sourceNamespace,
        revalidate: (source) =>
          new RevalidateThanksRecordSourceAdapter({
            env: c.env,
            var: c.var,
            sourceNamespace,
          }).prepare(source),
