...
import { VerifyThanksRetirementPage } from "@/contexts/thanks/application/verify-thanks-retirement-page"
...
import { zAppThanksRetirementVerificationReceipt } from "@/contexts/thanks/interface/http/response-schemas"
...
export const POST = thanksFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = thanksRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyThanksRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof ThanksRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppThanksRetirementVerificationReceipt.parse({
