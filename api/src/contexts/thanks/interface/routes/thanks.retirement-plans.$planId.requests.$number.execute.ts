import { zAppThanksRetirementExecution } from "@/contexts/thanks/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { thanksFactory } from "@/contexts/thanks/interface/request-environment/thanks-factory"
import { ExecuteThanksRetirementAdapter } from "@/contexts/thanks/infrastructure/adapters/execute-thanks-retirement.adapter"
import {
...
export const POST = thanksFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteThanksRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppThanksRetirementExecution.parse(submitted), 200)
  },
)
