...
import { z } from "zod"
import { thanksSourceFreezeResponseSchema } from "@/contexts/thanks/interface/http/response-schemas"
...
/** 現在の管理権限でthanks記録の停止世代を読む。 */
export function createThanksSourceFreezeReadHandlers() {
  return thanksFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "thanks" ||
      ...
      return c.json(thanksSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
