...
const secret = "thanks-integration-test-secret"

/** 実System・Companyとサンクス4台帳の原記録を接続する保全fixture。 */
export async function createThanksPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "thanks-preservation",
    revision: 1,
    title: "Preserve thanks records",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "thanks-pagination-test-pepper",
