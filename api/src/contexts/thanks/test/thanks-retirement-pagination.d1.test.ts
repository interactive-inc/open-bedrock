...
import { createThanksPreservationFixture } from "@/contexts/thanks/test/create-thanks-preservation-fixture.test-support"
...
test("感謝・原資・景品・交換申請を全件保全し、人の承認を経て4台帳を撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createThanksPreservationFixture(await pool.next())
  ...
  for (const sourceRecord of sourceRecords) {
    const path = `/thanks/records/${sourceRecord.recordKind}/${encodeURIComponent(sourceRecord.recordId)}/preservation-requests`
  ...
  expect(publicPlan).toMatchObject({ totalPages: 5, recordKinds: thanksRecordKinds })
  const retirementPath = `/thanks/retirement-plans/${planId}/requests`
  const retirementDefinition = ProcedureDefinitionEntity.create({
    key: "thanks-retirement",
    revision: 1,
    title: "Retire preserved thanks",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const verificationPath = `/thanks/retirement-plans/${planId}/verification-receipts`
  ...
  const decisionBody = {
    ...
    comment: "Reviewed preserved thanks",
  }
  expect(
  ...
  await execSql(
    database,
    "CREATE TRIGGER fail_thanks_retirement BEFORE INSERT ON system_record_source_retirements BEGIN SELECT RAISE(ABORT,'injected finalization failure'); END;",
  )
  ...
  await execSql(database, "DROP TRIGGER fail_thanks_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('bb48455f-2592-4316-882c-d000485aa041','system:record:export'); DROP TABLE thanks_redemptions; DROP TABLE thanks_rewards; DROP TABLE thanks_point_budgets; DROP TABLE thanks_messages;",
  )
