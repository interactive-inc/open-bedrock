...
test("感謝・原資・景品・交換申請を全件保全し、人の承認を経て4台帳を撤去確定する", async () => {
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('thanks-test-manager','system:record:export'); DROP TABLE thanks_redemptions; DROP TABLE thanks_rewards; DROP TABLE thanks_point_budgets; DROP TABLE thanks_messages;",
  )
