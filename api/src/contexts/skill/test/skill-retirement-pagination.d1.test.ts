...
test("スキル定義・従業員スキルを全件保全し、人の承認を経て2台帳を撤去確定する", async () => {
  const {
    ...
    tokenFor,
    request: apiRequest,
  } = await createSkillPreservationFixture(await pool.next())
  ...
  await execSql(database, "DROP TRIGGER fail_skill_retirement")
  ...
  await execSql(
    database,
    "INSERT INTO system_iam_role_permissions(role_id,permission_key) VALUES ('7c49772f-6388-4dc9-8d8f-34664a93f04a','system:record:export'); DROP TABLE employee_skills; DROP TABLE skill_definitions;",
  )
