...
const secret = "skill-integration-test-secret"

/** 実System・Companyとスキル2台帳の原記録を接続する保全fixture。 */
export async function createSkillPreservationFixture(database: D1Database) {
  ...
  const definition = ProcedureDefinitionEntity.create({
    key: "skill-preservation",
    revision: 1,
    title: "Preserve skill_definitions",
    category: "system",
    description: null,
    inputSchema: { fields: [] },
  ...
  const bindings = {
    ...
    DB: database,
    JWT_SECRET: secret,
    PEPPER_SECRET: "skill-pagination-test-pepper",
