...
import { z } from "zod"
import { skillSourceFreezeResponseSchema } from "@/contexts/skill/interface/http/response-schemas"
...
/** 現在の管理権限でskill記録の停止世代を読む。 */
export function createSkillSourceFreezeReadHandlers() {
  return skillFactory.createHandlers(
    ...
    async (c) => {
      ...
      if (
        freeze === null ||
        freeze.snapshot.ownerContext !== "skill" ||
      ...
      return c.json(skillSourceFreezeResponseSchema.parse({ freeze: freeze.snapshot }))
    },
  )
}
