...
import { VerifySkillRetirementPage } from "@/contexts/skill/application/verify-skill-retirement-page"
...
import { zAppSkillRetirementVerificationReceipt } from "@/contexts/skill/interface/http/response-schemas"
...
export const POST = skillFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = skillRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifySkillRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof SkillRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof SkillRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppSkillRetirementVerificationReceipt.parse({
