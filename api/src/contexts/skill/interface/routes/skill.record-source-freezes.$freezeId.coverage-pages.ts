...
import { VerifySkillCoveragePage } from "@/contexts/skill/application/verify-skill-coverage-page"
import {
  SkillCoverageForbiddenError,
  SkillCoverageConflictError,
} from "@/contexts/skill/application/errors"
import { zAppSkillCoveragePageReceipt } from "@/contexts/skill/interface/http/response-schemas"
...
export const POST = skillFactory.createHandlers(
  ...
  zValidator("json", skillCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = skillCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifySkillCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof SkillCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof SkillCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppSkillCoveragePageReceipt.parse({
