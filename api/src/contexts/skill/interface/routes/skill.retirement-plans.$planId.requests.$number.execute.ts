import { zAppSkillRetirementExecution } from "@/contexts/skill/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { skillFactory } from "@/contexts/skill/interface/request-environment/skill-factory"
import { ExecuteSkillRetirementAdapter } from "@/contexts/skill/infrastructure/adapters/execute-skill-retirement.adapter"
import {
...
export const POST = skillFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteSkillRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppSkillRetirementExecution.parse(submitted), 200)
  },
)
