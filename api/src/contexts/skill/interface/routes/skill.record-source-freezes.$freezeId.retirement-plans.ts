...
import { CreateSkillRetirementPlan } from "@/contexts/skill/application/create-skill-retirement-plan"
import { skillRetirementPlanCommandSchema } from "@/contexts/skill/domain/schemas/skill-retirement-plan-command.schema"
import { zAppSkillRetirementPlan } from "@/contexts/skill/interface/http/response-schemas"
...
export const POST = skillFactory.createHandlers(
  ...
  zValidator("json", skillRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = skillRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateSkillRetirementPlan(c).execute(command.data, stepUpToken)
