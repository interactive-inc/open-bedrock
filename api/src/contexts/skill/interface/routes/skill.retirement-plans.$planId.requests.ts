import { zAppSkillRetirementRequest } from "@/contexts/skill/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { skillFactory } from "@/contexts/skill/interface/request-environment/skill-factory"
import { SubmitSkillRetirementRequestAdapter } from "@/contexts/skill/infrastructure/adapters/submit-skill-retirement-request.adapter"
import {
...
export const POST = skillFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitSkillRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof SkillRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof SkillRetirementConflictError)
    ...
    return c.json(
      zAppSkillRetirementRequest.parse(submitted.request),
