...
import {
  SkillCoverageForbiddenError,
  SkillCoverageConflictError,
} from "@/contexts/skill/application/errors"
import { skillCoveragePageCommandSchema } from "@/contexts/skill/domain/schemas/skill-coverage-page-command.schema"
...
export class VerifySkillCoveragePage {
  ...
  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = skillCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new SkillCoverageForbiddenError("coverage authentication required")
    const proof = await new SkillRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new SkillCoverageForbiddenError("coverage authorization denied")
    const generation = await new SkillRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new SkillRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    })
    const repository = new SkillRecordSystemAdapter({
      env: this.c.env,
      assertions: generation.assertions,
    }).coveragePages()
    ...
    if (
      ...
      return new SkillCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new SkillCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenSkillRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new SkillCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareSkillCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new SkillCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new SkillCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: SkillRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new SkillCoverageConflictError("coverage terminal page changed")
    return page
  }
}
