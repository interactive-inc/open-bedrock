import { PrepareSkillRetirementPlanAdapter } from "@/contexts/skill/infrastructure/adapters/prepare-skill-retirement-plan.adapter"
...
import { SkillRetirementConflictError } from "@/contexts/skill/application/errors"

type Context = ConstructorParameters<typeof PrepareSkillRetirementPlanAdapter>[0]

/** skill記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateSkillRetirementPlan {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const prepared = await new PrepareSkillRetirementPlanAdapter(this.c).prepare(input, stepUpToken)
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new SkillRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new SkillRetirementConflictError("retirement plan creation conflicts")
  }
}
