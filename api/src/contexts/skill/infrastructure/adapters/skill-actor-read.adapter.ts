...
export class SkillActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (!Number.isSafeInteger(now.getTime()) || now.getTime() < 0)
      return new SkillError("forbidden", "invalid skill actor")
    ...
    if (authorization instanceof Error)
      return new SkillError("skill_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new SkillError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new SkillError("skill_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new SkillError("skill_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new SkillError("forbidden", "active employee is required")
    return {
