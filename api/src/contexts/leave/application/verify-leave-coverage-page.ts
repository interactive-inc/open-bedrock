...
import {
  LeaveCoverageForbiddenError,
  LeaveCoverageConflictError,
} from "@/contexts/leave/application/errors"
import { leaveCoveragePageCommandSchema } from "@/contexts/leave/domain/schemas/leave-coverage-page-command.schema"
...
import { CaptureFrozenLeaveRecordPageAdapter } from "@/contexts/leave/infrastructure/adapters/capture-frozen-leave-record-page.adapter"
import { PrepareLeaveCoverageRecordsAdapter } from "@/contexts/leave/infrastructure/adapters/prepare-leave-coverage-records.adapter"

type Context = LeaveContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからleave記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyLeaveCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = leaveCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new LeaveCoverageForbiddenError("coverage authentication required")
    const proof = await new LeaveRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new LeaveCoverageForbiddenError("coverage authorization denied")
    const generation = await new LeaveRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new LeaveRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      return new LeaveCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new LeaveCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenLeaveRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new LeaveCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareLeaveCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new LeaveCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new LeaveCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: LeaveRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new LeaveCoverageConflictError("coverage terminal page changed")
    return page
  }
}
