...
type Context = ConstructorParameters<typeof PrepareLeaveRetirementPlanAdapter>[0]

/** leave記録の全照合ページを検査する計画を固定し、同一IDの再送で検査対象を増減させない。 */
export class CreateLeaveRetirementPlan {
  ...
  async execute(input: unknown, stepUpToken: string) {
    ...
    if (existing !== null)
      return matches(existing)
        ? existing
        : new LeaveRetirementConflictError("retirement plan replay differs")
    ...
    return raced !== null && matches(raced)
      ? raced
      : new LeaveRetirementConflictError("retirement plan creation conflicts")
  }
}
