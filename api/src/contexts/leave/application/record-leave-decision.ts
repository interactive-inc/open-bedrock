...
/** 表示した休暇への判断を現在の会社資格で記録し、必要人数と次の段階を反映する。 */
export class RecordLeaveDecision {
  ...
  async run(command: Command): Promise<Result | ApplicationError> {
    ...
    if (human === "forbidden")
      return new ForbiddenError("休暇を判断する権限がありません", "forbidden")
    ...
    if (request === null) return new NotFoundError("休暇が見つかりません", "not_found")
    ...
    if (
      ...
      return new ConflictError("休暇と承認対象が一致しません", "decision_target_changed")
    const receiptInput = {
      binding,
    ...
    if (receipt === "matching") {
      if (proposal.status === "cancelled")
        return new ConflictError("休暇は取り消されています", "decision_changed")
      return {
    ...
    const audit = SystemAuditEventEntity.create({
      actorAccountId: command.session.accountId,
      action: `leave.request.${prepared.action}`,
      ...
      if (concurrent === "matching") {
        ...
        if (current !== null && !(current instanceof Error) && current.status !== "cancelled")
          return {
            ...
            needsExecution: current.status === "approved" || current.status === "rejected",
            replayed: true,
          }
      }
    ...
    return {
      ...
      needsExecution: saved.caseStatus === "approved" || saved.caseStatus === "rejected",
      replayed: false,
    }
  }
}
