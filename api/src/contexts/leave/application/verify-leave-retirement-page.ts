import { PrepareLeaveRetirementPageAdapter } from "@/contexts/leave/infrastructure/adapters/prepare-leave-retirement-page.adapter"
import { LeaveRecordSystemAdapter } from "@/contexts/leave/infrastructure/adapters/leave-record-system.adapter"
import { leaveRetirementVerificationCommandSchema } from "@/contexts/leave/domain/schemas/leave-retirement-verification-command.schema"
import {
  LeaveRetirementForbiddenError,
  LeaveRetirementConflictError,
} from "@/contexts/leave/application/errors"
...
type Context = ConstructorParameters<typeof PrepareLeaveRetirementPageAdapter>[0]

/** leave記録の保存済み計画の次ページを再検査し、再送でも現在の原記録と保持条件を要求する。 */
export class VerifyLeaveRetirementPage {
  ...
  async execute(
    ...
    const parsed = leaveRetirementVerificationCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new LeaveRetirementForbiddenError("retirement authentication required")
    const authority = await new LeaveRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (authority === "forbidden")
      return new LeaveRetirementForbiddenError("retirement authorization denied")
    const context = { env: this.c.env, assertions: authority.assertions }
    const plan = await new LeaveRecordSystemAdapter(context).retirementPlans().find(command.planId)
    if (plan instanceof Error) return plan
    if (
      plan === null ||
      plan.snapshot.sourceNamespace !== command.sourceNamespace ||
      plan.snapshot.ownerContext !== "leave" ||
      plan.snapshot.capability.revision !== 1 ||
      JSON.stringify(plan.snapshot.capability.recordKinds) !== JSON.stringify(leaveRecordKinds)
    )
      return new LeaveRetirementConflictError("retirement plan unavailable or capability changed")
    const repository = new LeaveRecordSystemAdapter(context).retirementReceipts()
    ...
    if (
      ...
      return new LeaveRetirementConflictError("retirement receipt replay differs")
    ...
    if (target instanceof Error)
      return new LeaveRetirementConflictError("all plan pages already verified")
    const checked = await new PrepareLeaveRetirementPageAdapter(this.c).prepare(
      {
    ...
    const keys = await new LeaveRecordSystemAdapter({
      env: this.c.env,
    ...
    const guarded = new LeaveRecordSystemAdapter(guardedContext).retirementReceipts()
    ...
    if (existing !== null) {
      ...
      return current !== null && matches(current)
        ? current
        : new LeaveRetirementConflictError("retirement receipt replay changed")
    }
    const page = await new LeaveRecordSystemAdapter(guardedContext)
      .coveragePages()
    ...
    return raced !== null && matches(raced)
      ? raced
      : new LeaveRetirementConflictError("retirement verification position already written")
  }
}
