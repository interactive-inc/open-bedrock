...
import { CreateLeaveRetirementPlan } from "@/contexts/leave/application/create-leave-retirement-plan"
import { leaveRetirementPlanCommandSchema } from "@/contexts/leave/domain/schemas/leave-retirement-plan-command.schema"
import { zAppLeaveRetirementPlan } from "@/contexts/leave/interface/http/response-schemas"
...
export const POST = leaveFactory.createHandlers(
  ...
  zValidator("json", leaveRetirementPlanCommandSchema.pick({ purpose: true })),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = leaveRetirementPlanCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new CreateLeaveRetirementPlan(c).execute(command.data, stepUpToken)
