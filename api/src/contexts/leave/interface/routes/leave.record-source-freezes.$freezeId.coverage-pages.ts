...
import { VerifyLeaveCoveragePage } from "@/contexts/leave/application/verify-leave-coverage-page"
import {
  LeaveCoverageForbiddenError,
  LeaveCoverageConflictError,
} from "@/contexts/leave/application/errors"
import { zAppLeaveCoveragePageReceipt } from "@/contexts/leave/interface/http/response-schemas"
...
export const POST = leaveFactory.createHandlers(
  ...
  zValidator(
    "json",
    leaveCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = leaveCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyLeaveCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof LeaveCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof LeaveCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppLeaveCoveragePageReceipt.parse({
