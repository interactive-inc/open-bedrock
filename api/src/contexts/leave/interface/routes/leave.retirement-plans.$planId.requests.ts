import { zAppLeaveRetirementRequest } from "@/contexts/leave/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { leaveFactory } from "@/contexts/leave/interface/request-environment/leave-factory"
import { SubmitLeaveRetirementRequestAdapter } from "@/contexts/leave/infrastructure/adapters/submit-leave-retirement-request.adapter"
import {
...
export const POST = leaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new SubmitLeaveRetirementRequestAdapter(c).execute(
      {
    ...
    if (
      submitted instanceof LeaveRetirementForbiddenError ||
      submitted instanceof CompanyForbiddenError ||
      submitted instanceof ForbiddenError
    )
      throw new SystemForbiddenError()
    if (submitted instanceof LeaveRetirementConflictError)
    ...
    return c.json(
      zAppLeaveRetirementRequest.parse(submitted.request),
