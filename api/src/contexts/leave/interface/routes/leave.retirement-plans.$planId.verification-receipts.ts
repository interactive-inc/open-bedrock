...
import { VerifyLeaveRetirementPage } from "@/contexts/leave/application/verify-leave-retirement-page"
...
import { zAppLeaveRetirementVerificationReceipt } from "@/contexts/leave/interface/http/response-schemas"
...
export const POST = leaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = leaveRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyLeaveRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof LeaveRetirementForbiddenError || receipt instanceof ForbiddenError)
      throw new SystemForbiddenError()
    if (receipt instanceof LeaveRetirementConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppLeaveRetirementVerificationReceipt.parse({
