import { zAppLeaveRetirementExecution } from "@/contexts/leave/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { leaveFactory } from "@/contexts/leave/interface/request-environment/leave-factory"
import { ExecuteLeaveRetirementAdapter } from "@/contexts/leave/infrastructure/adapters/execute-leave-retirement.adapter"
import {
...
export const POST = leaveFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteLeaveRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppLeaveRetirementExecution.parse(submitted), 200)
  },
)
