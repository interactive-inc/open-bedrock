...
beforeAll(async () => {
  local = await startLocalD1({
    migrated: [
      ...
      "unit-annual-half_day_am",
      "unit-annual-hourly",
      "unit-compensatory-full_day",
      "legacy-url-approve",
      "legacy-url-reject",
      "two-stage",
      "delegation",
...
test.each(["approve", "reject"] as const)(
  ...
  async (action) => {
    const c = await fixture(`full-flow-${action}`)
...
test("本人の取消は履歴を残し、同じ期間の再申請を妨げない", async () => {
  const c = await fixture("withdraw")
...
test.each([
  ...
  const c = await fixture(`unit-${kind.type}-${kind.unit}`)
  await c.database
    .prepare(
...
test.each(["approve", "reject"])(
  ...
  async (action) => {
    const c = await fixture(`legacy-url-${action}`)
    const actor = c.people[1]
    if (actor === undefined) throw new Error("actor missing")
    expect(
      (
...
test("委任による判断は本人と代理先を記録し、確定前の委任失効で残数消費を拒否する", async () => {
  const c = await fixture("delegation")
  const first = c.people[1],
    delegate = c.people[3]
  ...
  await execSql(c.database, "DROP TRIGGER prevent_leave_delivery")
  expect(
...
test("未提出の記録をDBから直接判断へ変更できない", async () => {
  const c = await fixture("direct-db-decision")
...
test("確認対象の欠落・旧形式をHTTPの入力段階で拒否する", async () => {
  const c = await fixture("http-input-validation")
  const actor = c.people[1]
  if (actor === undefined) throw new Error("actor missing")
  for (const target of [
    undefined,
