...
/** 休暇4台帳の原文を固定し、Systemへの保全直前にも同じ版を要求する。 */
export class CaptureLeaveRecordAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async prepare(
    input: Readonly<{ recordKind: LeaveRecordKind; recordId: string; sourceNamespace: string }>,
  ) {
    const kind = leaveRecordKindSchema.safeParse(input.recordKind)
    if (!kind.success) return new LeaveError("forbidden", "invalid source record")
    const query = leaveSnapshotQuery(kind.data, input.recordId)
    if (query instanceof Error)
      return new LeaveError("forbidden", "invalid source record", { cause: query })
    ...
    try {
      ...
      if (reads.length !== actor.assertions.length + 1 || reads.some((read) => !read.success))
        return new Error("leave source is unavailable")
      const snapshot = reads.at(-1)?.results[0]?.snapshot_json
      if (snapshot === undefined) return new Error("leave source is unavailable")
      ...
      return new Error("leave source capture failed", { cause })
    }
  }
}
