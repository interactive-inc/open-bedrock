...
export class LeaveActorReadAdapter {
  ...
  async prepare(employeeIds: ReadonlyArray<EmployeeId> = []) {
    ...
    if (authorization instanceof Error)
      return new LeaveError("leave_unavailable", "authorization is unavailable", {
        cause: authorization,
      })
    if (authorization === "forbidden")
      return new LeaveError("forbidden", "human record preserver is required")
    ...
    if (guard instanceof Error)
      return new LeaveError("leave_unavailable", "Company snapshot is unavailable", {
        cause: guard,
      })
    ...
    if (actors instanceof Error || employees instanceof Error)
      return new LeaveError("leave_unavailable", "Company directory is unavailable")
    if (actors[0]?.employee === undefined)
      return new LeaveError("forbidden", "active employee is required")
    return {
