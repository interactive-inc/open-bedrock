import { leaveRetirementSubmissionCommandSchema } from "@/contexts/leave/domain/schemas/leave-retirement-submission-command.schema"
import { PrepareLeaveRetirementCurrentStateAdapter } from "@/contexts/leave/infrastructure/adapters/prepare-leave-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareLeaveRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitLeaveRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = leaveRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new LeaveRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareLeaveRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new LeaveRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "leave.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new LeaveRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new LeaveRetirementForbiddenError("retirement applicant differs")
      if (
        ...
        return new LeaveRetirementConflictError("retirement previous version is not resubmittable")
      ...
      if (
        ...
        return new LeaveRetirementConflictError(
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new LeaveRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareLeaveRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new LeaveRetirementForbiddenError("retirement decision qualification required")
    ...
    if (started instanceof Error) {
      ...
      return new LeaveRetirementConflictError("retirement submission changed before persistence", {
        cause: started,
      })
    }
    return {
