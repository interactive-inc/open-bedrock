...
test("稟議の停止世代は2台帳の追加・更新・削除をDB確定時に拒否する", () => {
  const database = new Database(":memory:")
  database.exec(`
    CREATE TABLE system_record_source_freezes (owner_context TEXT NOT NULL, revision INTEGER NOT NULL);
    CREATE TABLE ringi_requests (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
    CREATE TABLE ringi_procedure_bindings (id INTEGER PRIMARY KEY, value TEXT NOT NULL);
  `)
  const migration = readFileSync(
    new URL("../../../../migrations/0309_guard_ringi_record_source_freeze.sql", import.meta.url),
    "utf8",
  )
  database.exec(migration)
  const tables = ["ringi_requests", "ringi_procedure_bindings"]
  ...
  database.exec("INSERT INTO system_record_source_freezes VALUES ('ringi', 1)")
  for (const table of tables) {
    expect(() => database.exec(`INSERT INTO ${table} (id, value) VALUES (2, 'blocked')`)).toThrow(
      "ringi_record_source_frozen",
    )
  ...
  database.exec(
    "UPDATE system_record_source_freezes SET revision = 2 WHERE owner_context = 'ringi'",
  )
  for (const table of tables) {
