...
async function pending() {
  const c = await createRingiProcedureTestContext(await pool.next())
...
describe("稟議と共通の承認・実行基盤", () => {
  ...
  test.each(["ringi", "binding", "audit"])(
    ...
    async (failure) => {
      ...
      await execSql(c.database, "DROP TRIGGER reject_ringi_creation")
      expect(await c.repository.createWithProcedure(c.submission)).not.toBeInstanceOf(Error)
    },
  )
  ...
  test.each(["permission", "authority", "audit", "target"])(
    ...
    async (change) => {
      ...
      if (change === "audit") {
        await execSql(c.database, "DROP TRIGGER reject_ringi_execution_audit")
