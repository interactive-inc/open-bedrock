import { zAppRingiRetirementExecution } from "@/contexts/ringi/interface/http/response-schemas"
import { z } from "zod"
import { zValidator } from "@hono/zod-validator"
import { ringiFactory } from "@/contexts/ringi/interface/request-environment/ringi-factory"
import { ExecuteRingiRetirementAdapter } from "@/contexts/ringi/infrastructure/adapters/execute-ringi-retirement.adapter"
import {
...
export const POST = ringiFactory.createHandlers(
  ...
  async (c) => {
    ...
    const submitted = await new ExecuteRingiRetirementAdapter(c).execute(
      {
    ...
    return c.json(zAppRingiRetirementExecution.parse(submitted), 200)
  },
)
