...
import { VerifyRingiRetirementPage } from "@/contexts/ringi/application/verify-ringi-retirement-page"
...
import { zAppRingiRetirementVerificationReceipt } from "@/contexts/ringi/interface/http/response-schemas"
...
export const POST = ringiFactory.createHandlers(
  ...
  async (c) => {
    ...
    const command = ringiRetirementVerificationCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyRingiRetirementPage(c).execute(command.data, stepUpToken)
    if (receipt instanceof RingiRetirementForbiddenError || receipt instanceof ForbiddenError)
    ...
    return c.json(
      zAppRingiRetirementVerificationReceipt.parse({
