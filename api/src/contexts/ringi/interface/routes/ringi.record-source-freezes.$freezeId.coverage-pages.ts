...
import { VerifyRingiCoveragePage } from "@/contexts/ringi/application/verify-ringi-coverage-page"
import {
  RingiCoverageForbiddenError,
  RingiCoverageConflictError,
} from "@/contexts/ringi/application/errors"
import { zAppRingiCoveragePageReceipt } from "@/contexts/ringi/interface/http/response-schemas"
...
export const POST = ringiFactory.createHandlers(
  ...
  zValidator(
    "json",
    ringiCoveragePageCommandSchema.pick({ purpose: true, recordKind: true, records: true }),
  ),
  async (c) => {
    c.header("Cache-Control", "no-store")
    const stepUpToken = c.req.header("x-system-step-up")
    if (stepUpToken === undefined) throw new SystemStepUpRequiredError()
    const command = ringiCoveragePageCommandSchema.safeParse({
      ...c.req.valid("json"),
    ...
    const receipt = await new VerifyRingiCoveragePage(c).execute(command.data, stepUpToken)
    if (receipt instanceof RingiCoverageForbiddenError) throw new SystemForbiddenError()
    if (receipt instanceof RingiCoverageConflictError)
      throw new SystemHTTPException({
        status: 409,
    ...
    return c.json(
      zAppRingiCoveragePageReceipt.parse({
