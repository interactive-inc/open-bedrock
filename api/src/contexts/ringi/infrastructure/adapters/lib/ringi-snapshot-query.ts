...
export function ringiSnapshotQuery(
  ...
  if (recordKind === "ringi-request-record") {
    ...
    if (!parsed.success) return new Error("invalid ringi request id")
    return {
      sql: `SELECT json_object('format','ringi-request-record','version',2,'request',json_object(
        'id',id,'legacy_id',legacy_id,'applicant_id',applicant_id,'approver_id',approver_id,'title',title,
  ...
  return {
    sql: `SELECT json_object('format','ringi-procedure-binding-record','version',2,'binding',json_object(
      'id',id,'previous_ringi_id',previous_ringi_id,'request_key',request_key,'ringi_id',ringi_id,
