import { ringiRetirementSubmissionCommandSchema } from "@/contexts/ringi/domain/schemas/ringi-retirement-submission-command.schema"
import { PrepareRingiRetirementCurrentStateAdapter } from "@/contexts/ringi/infrastructure/adapters/prepare-ringi-retirement-current-state.adapter"
...
type Context = ConstructorParameters<typeof PrepareRingiRetirementCurrentStateAdapter>[0]

/** 全件検査の計画と終端を固定し、会社資格で解決した人の判断待ち案件を作成する。 */
export class SubmitRingiRetirementRequestAdapter {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string) {
    const parsed = ringiRetirementSubmissionCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RingiRetirementForbiddenError("retirement authentication required")
    const current = await new PrepareRingiRetirementCurrentStateAdapter(this.c).prepare(
      {
    ...
    if (receipt === null || receipt.digest !== current.terminalReceiptDigest)
      return new RingiRetirementConflictError("retirement terminal receipt differs")
    ...
    const resolveRevision = async () => {
      if (command.revision.mode === "create") {
        const identity = CanonicalSystemJsonValue.create({
          operation: "ringi.record-retirement.request",
          actorAccountId: authentication.accountId,
      ...
      if (previous === null)
        return new RingiRetirementConflictError("retirement previous version is missing")
      if (previous.createdByAccountId !== authentication.accountId)
        return new RingiRetirementForbiddenError("retirement applicant differs")
      if (
        ...
        return new RingiRetirementConflictError("retirement previous version is not resubmittable")
      ...
      if (
        ...
        return new RingiRetirementConflictError(
    ...
    const replay = async () => {
      ...
      if (
        ...
        return new RingiRetirementConflictError("retirement submission replay differs")
      const rechecked = await new PrepareRingiRetirementCurrentStateAdapter(this.c).prepare(
        {
          planId: command.planId,
          planDigest: command.planDigest,
          sourceNamespace: command.sourceNamespace,
    ...
    if (task.resolved.guards.length === 0)
      return new RingiRetirementForbiddenError("retirement decision qualification required")
    ...
    if (started instanceof Error) {
      ...
      return new RingiRetirementConflictError("retirement submission changed before persistence", {
        cause: started,
      })
    }
    return {
