...
/** 稟議に適用する会社の承認規程を版付きで公開する。 */
export class PublishRingiProcedure {
  ...
  async run(command: Command): Promise<ProcedureDefinitionEntity | ApplicationError> {
    ...
    const human = await new SystemHumanOperationAuthorizationAdapter(this.c).prepare({
      ...
      permissions: ["ringi:procedure:manage"],
    ...
    const definition = ProcedureDefinitionEntity.create({
      ...
      inputSchema: {
        fields: [
          { key: "title", type: "text", label: "件名", required: true },
          { key: "amount", type: "number", label: "金額", required: true },
          { key: "reason", type: "textarea", label: "理由", required: true },
        ],
      },
    ...
    const audit = SystemAuditEventEntity.create({
      ...
      action: "ringi.procedure.published",
      targetType: "ringi.procedure",
      targetId: definition.key,
      outcome: "succeeded",
      reasonCode: null,
      authorizationJson: JSON.stringify({ permission: "ringi:procedure:manage" }),
