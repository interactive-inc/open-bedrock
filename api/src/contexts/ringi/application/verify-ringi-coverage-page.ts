...
import {
  RingiCoverageForbiddenError,
  RingiCoverageConflictError,
} from "@/contexts/ringi/application/errors"
import { ringiCoveragePageCommandSchema } from "@/contexts/ringi/domain/schemas/ringi-coverage-page-command.schema"
...
import { CaptureFrozenRingiRecordPageAdapter } from "@/contexts/ringi/infrastructure/adapters/capture-frozen-ringi-record-page.adapter"
import { PrepareRingiCoverageRecordsAdapter } from "@/contexts/ringi/infrastructure/adapters/prepare-ringi-coverage-records.adapter"

type Context = RingiContext & SystemDatabaseContext & SystemAttachmentStorageContext

/** 保存済みの続きからringi記録の原文を照合し、同じ停止世代のページと監査を保存する。 */
export class VerifyRingiCoveragePage {
  constructor(private readonly c: Context) {
    Object.freeze(this)
  }

  async execute(input: unknown, stepUpToken: string): Promise<RecordCoveragePageEntity | Error> {
    const parsed = ringiCoveragePageCommandSchema.safeParse(input)
    if (!parsed.success) return parsed.error
    const command = parsed.data
    const authentication = this.c.var.bearerReadAuthentication
    if (authentication === undefined)
      return new RingiCoverageForbiddenError("coverage authentication required")
    const proof = await new RingiRecordSystemAdapter(this.c).prepareSourceFreezeAuthorization({
      authentication,
      now: this.c.var.now(),
      stepUpToken,
    })
    ...
    if (proof === "forbidden")
      return new RingiCoverageForbiddenError("coverage authorization denied")
    const generation = await new RingiRecordSystemAdapter({
      env: this.c.env,
    ...
    const completion = new RingiRecordSystemAdapter({
      env: this.c.env,
    ...
    if (
      ...
      return new RingiCoverageConflictError("coverage replay conflicts")
    ...
    if (existing === null && previous?.snapshot.nextCursor === null)
      return new RingiCoverageConflictError("coverage scan already complete")
    const cursor =
      existing === null ? (previous?.snapshot.nextCursor ?? null) : existing.snapshot.afterCursor
    const page = await new CaptureFrozenRingiRecordPageAdapter(this.c).prepare({
      freezeId: command.freezeId,
      sourceNamespace: command.sourceNamespace,
      recordKind: command.recordKind,
      afterCursor: cursor,
    ...
    if (
      ...
      return new RingiCoverageConflictError("coverage mapping or purpose conflicts")
    const verified = await new PrepareRingiCoverageRecordsAdapter({
      env: this.c.env,
      var: this.c.var,
      assertions: generation.assertions,
    ...
    if (existing !== null) {
      if (JSON.stringify(existing.snapshot.records) !== JSON.stringify(records))
        return new RingiCoverageConflictError("coverage replay differs")
    ...
    if (saved === "conflict")
      return new RingiCoverageConflictError(
  ...
  private async confirmTerminal(
    ...
    completion: RingiRecordSystemAdapter,
  ) {
    ...
    if (verified.summary.terminalDigest !== page.digest)
      return new RingiCoverageConflictError("coverage terminal page changed")
    return page
  }
}
