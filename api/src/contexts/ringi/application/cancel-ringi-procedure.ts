...
/** 本人が確認した承認待ちの稟議を取り消し、判断と取消の同時確定を防ぐ。 */
export class CancelRingiProcedure {
  ...
  async run(
    ...
    if (human === "forbidden")
      return new ForbiddenError("稟議を取り消す権限がありません", "forbidden")
    ...
    const owned = await repository.readSubmissionReceipt({
      ...
      previousRingiId: binding.previousRingiId,
      guards: [...human.assertions, companyGuard],
    })
    if (owned !== true) return new ForbiddenError("本人の稟議だけを取り消せます", "forbidden")
    ...
    if (expected instanceof Error || target instanceof Error || !expected.equals(target))
      return new ConflictError("確認した稟議の判断対象が変わっています", "decision_target_changed")
    if (proposal.status === "cancelled") return { status: "cancelled", replayed: true }
    if (proposal.status !== "pending")
      return new ConflictError("承認待ちの稟議だけを取り消せます", "not_pending")
    const audit = SystemAuditEventEntity.create({
      actorAccountId: command.session.accountId,
      action: "ringi.request.cancelled",
      targetType: "ringi.request",
      targetId: String(command.ringiId),
      outcome: "succeeded",
      reasonCode: null,
      authorizationJson: JSON.stringify({ permission: "ringi:submit" }),
