 ...
 * 主務所属を書ける状態にする。Company側の所属fixtureのうち、業務の手続testが使う操作だけを持つ。
 */
export async function createLocalD1CompanyAssignment(database: D1Database) {
  ...
  if (Number(preview.status) !== 200) throw new Error(`adoption preview failed: ${preview.status}`)
  const body = z
    .object({ expectedRevision: z.number(), snapshotDigest: z.string(), observedOn: z.string() })
    .parse(await preview.json())
  const adopted = await client.adoptions.$post({
  ...
  const assignEmployeeCode = async (targetId = employeeId, code = "EMPLOYEE-001") => {
    ...
    const response = await client.employees.$post({
      header: {
