...
const TABLES = ["onboarding_templates", "onboarding_assignments", "onboarding_tasks"] as const
...
const REFERENCES = [
  ["onboarding_tasks", "assignment_id", "onboarding_assignments"],
  ["onboarding_lifecycle_deliveries", "assignment_id", "onboarding_assignments"],
] as const

let local: LocalD1

beforeAll(async () => {
  local = await startLocalD1({ empty: ["onboarding-skill-asset-uuid-migration"] })
}, 120_000)
...
test("seed 済みのローカルD1で全行を UUID に移し、旧主キーを legacy_id に残す", async () => {
  const database = await local.database("onboarding-skill-asset-uuid-migration")
  ...
  await expect(
    database
      .prepare(
        "INSERT INTO onboarding_templates (id, code, name, kind) VALUES (100, 'x', 'x', 'join')",
      )
      .run(),
