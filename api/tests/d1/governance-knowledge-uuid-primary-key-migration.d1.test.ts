...
const TABLES = ["governance_org_role_assignments", "knowledge_articles"] as const
...
const REFERENCES = [
  ["governance_documents", "current_version_id", "governance_document_versions"],
  ["governance_document_versions", "document_id", "governance_documents"],
  ["governance_acknowledgements", "version_id", "governance_document_versions"],
  ["governance_document_references", "version_id", "governance_document_versions"],
  ["governance_publication_approvals", "version_id", "governance_document_versions"],
  ["knowledge_article_revisions", "article_id", "knowledge_articles"],
] as const

let local: LocalD1

beforeAll(async () => {
  local = await startLocalD1({ empty: ["governance-knowledge-uuid-migration"] })
}, 120_000)
...
test("seed 済みのローカルD1で全行を UUID に移し、旧主キーを legacy_id に残す", async () => {
  const database = await local.database("governance-knowledge-uuid-migration")
  ...
  expect(migratedRows).toBeGreaterThan(4)
  ...
  await expect(
    database
      .prepare(
        "INSERT INTO knowledge_articles (id, title, category, body_md, author_id, created_at) SELECT 100, 'x', 'x', 'x', id, '2026-01-01' FROM company_employees LIMIT 1",
      )
      .run(),
