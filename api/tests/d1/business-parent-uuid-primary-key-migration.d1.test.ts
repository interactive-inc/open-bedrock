...
const REFERENCES = [
  ["career_applications", "posting_id", "career_postings"],
  ["employee_certifications", "certification_id", "certification_definitions"],
  ["decision_records", "superseded_by_id", "decision_records"],
  ["meeting_minutes_records", "meeting_id", "meetings"],
  ["partner_contracts", "partner_id", "partners"],
  ["regulation_versions", "regulation_id", "regulations"],
] as const

let local: LocalD1

beforeAll(async () => {
  local = await startLocalD1({ empty: ["parent-uuid-migration"] })
}, 120_000)
...
test("seed 済みのローカルD1で全行を UUID に移し、旧主キーを legacy_id に残す", async () => {
  const database = await local.database("parent-uuid-migration")
  ...
  await expect(
    database
      .prepare(
        "INSERT INTO partners (id, code, name, status, created_at) VALUES (100, 'P9999', 'x', 'active', '2026-01-01T00:00:00Z')",
      )
      .run(),
