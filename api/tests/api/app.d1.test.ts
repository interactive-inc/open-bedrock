...
async function makeBindings(corsOrigin?: string): Promise<Bindings> {
  return {
    DB: await pool.next(),
...
async function preflight(origin: string, corsOrigin?: string): Promise<Response> {
  const bindings = await makeBindings(corsOrigin)
...
describe("security headers", () => {
  test("レスポンスに X-Content-Type-Options: nosniff が付く", async () => {
    const response = await app.request("/system/health", { method: "GET" }, await makeBindings())
...
describe("rate limiting", () => {
  ...
  test("上限超過(success:false)なら 429 を返す（認証前に弾く）", async () => {
    const bindings = { ...(await makeBindings()), API_RATE_LIMITER: makeLimiter(false) }

    const response = await app.request("/company/employees", { method: "GET" }, bindings)
  ...
  test("上限内(success:true)なら通常処理に進む（未認証で 401）", async () => {
    const bindings = { ...(await makeBindings()), API_RATE_LIMITER: makeLimiter(true) }
  ...
  test("/system/health は binding があっても対象外", async () => {
    const bindings = { ...(await makeBindings()), API_RATE_LIMITER: makeLimiter(false) }

    const response = await app.request("/system/health", { method: "GET" }, bindings)
...
describe("rate limiting fail-closed (#1035)", () => {
  test("本番相当（CORS_ORIGIN 設定済み）で binding 未設定なら 503 で拒否する", async () => {
    const bindings = await makeBindings("https://app.example.com")
  ...
  test("本番相当でも /system/health は binding 未設定のまま通す（監視用）", async () => {
    const bindings = await makeBindings("https://app.example.com")
  ...
  test("System Session は PEPPER_SECRET 未設定なら 503 で拒否する", async () => {
    const bindings = {
      ...(await makeBindings("https://app.example.com")),
