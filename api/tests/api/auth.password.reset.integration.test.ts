...
import {
  seedPasswordHash,
  seedPepperSecret,
} from "@tests/api/support/company/seed-password-hash.test-support"
...
describe("System password reset HTTP", () => {
  test("raw tokenを保存せず、credential変更・全Session失効・challenge単回利用を不可分にする", async () => {
    ...
    const requested = await client.system.auth.password.reset.$post({
    ...
    const completed = await client.system.auth.password.reset.$patch({
    ...
    const replayed = await client.system.auth.password.reset.$patch({
      json: { token: rawToken, new_password: "another-password" },
    })
    expect(replayed.status).toBe(400)

    const unknownRecipient = await client.system.auth.password.reset.$post({
      json: { email: "unknown@example.com" },
    })
    expect(unknownRecipient.status).toBe(200)

    emailText = ""
    const secondRequest = await client.system.auth.password.reset.$post({
      json: { email: "you+e001@example.com" },
    })
    ...
    const concurrentCompletions = await Promise.all([
      client.system.auth.password.reset.$patch({
