...
INSERT INTO application_templates (id, code, name, category, description, schema_json, approver_roles) VALUES
  (1, 'paid_leave', 'Paid Leave Request', 'attendance', 'Request for paid leave', '{"type":"object","properties":{"start_date":{"type":"string","format":"date"},"end_date":{"type":"string","format":"date"},"reason":{"type":"string"}},"required":["start_date","end_date"]}', '["manager","admin"]'),
  (2, 'expense', 'Expense Reimbursement', 'accounting', 'Request for reimbursement of paid expenses', '{"type":"object","properties":{"amount":{"type":"number"},"category":{"type":"string"},"note":{"type":"string"}},"required":["amount","category"]}', '["manager","admin"]'),
  ...
  (4, 'equipment', 'Equipment Purchase Request', 'general_affairs', 'Request to purchase work equipment', '{"type":"object","properties":{"item":{"type":"string"},"amount":{"type":"number"},"reason":{"type":"string"}},"required":["item","amount"]}', '["manager","admin"]');
