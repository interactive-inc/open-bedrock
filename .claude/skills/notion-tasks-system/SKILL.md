...
description: Shared Notion task template.
when_to_use: Referenced by Notion task-creation skills.
user-invocable: false
disable-model-invocation: false
metadata:
  author: shigurenimo
  description: Notionでタスクを作成する際の共通テンプレートとガイドライン。他スキルから参照される。
  dev: true
  tags: [docs]
---
...
| 作業待ち       | 仕様承認済み、実装開始待ち             | issue-{番号} メンバーが実装 → 作業中\_CLAUDE へ     |
| 作業中\_CLAUDE | Claude が自走で実装中                  | PR 完成 → 作業確認待ちへ                            |
