...
製品内はSystem、Company、業務に分ける。詳細は `.docs/company-foundation.md` に従う。Systemの正本は `contexts/system`、会社コアの正本は `contexts/company`、それ以外は独立した業務contextが所有する。知識・台帳・勤怠・採用なども業務であり、会社の補助機能という別区分は設けない。業務の重要度や既定有効状態を所有境界と混同しない。機能の除去と記録の消去を分け、会社の正本と既存の証跡を保全する。詳細と実装状態は `.docs/feature-tiers.md` と `.docs/capability-map.md` に記録する。
...
- 自社・他社の固有名詞、製品名、リポジトリ名、個人情報、認証情報
- 社内のインフラ構成、社内の別システムとの統合の詳細。社内固有の統合が必要なときは、環境変数・アダプタ・webhook のような汎用のプラグイン点として設計し、固有部分はこのリポジトリの外に置く
- 他リポジトリの識別子。`owner/repo` 形式、GitHub の URL、他リポジトリの commit SHA・Issue 番号・PR 番号

書く場所は問わない。コード、コメント、migration、設定、検査用のリスト、テスト、生成物、ドキュメント、コミットメッセージ、Issue、PR、CI のログをすべて対象とする。

過去に踏んだ失敗なので、次を守る。

- 禁止語のリストそのものをリポジトリに置かない。伏せたい語は環境変数かリポジトリ外のファイルから受け取り、読めないときに拒否するか skip するかを明示する。検査が違反を報告するときも語そのものを出力せず、ファイルと行だけを示す。公開リポジトリの CI ログは誰でも読める
- 複数の製品で同一に保つファイルに、製品固有の値を置かない。同一に保つファイルへ書いた内容は、その基盤を採用するすべての製品へそのまま複製される。製品固有の値は製品ごとのファイルに置く
- 別の製品やリポジトリを参照したいときは、名前や番号ではなく「共有する System context」のような一般名で書く。同期元を機械的に辿る必要があるなら、その対応はリポジトリ外の設定に持つ
- サンプルは `you@example.com` のような汎用値を使う

コミット前の確認は省略しない。次を必ず行う。

1. `git diff --cached` で差分全体を見る。コメント、migration、設定、テスト、生成物も対象にする
2. 書こうとしているコミットメッセージを最後まで読む。差分が清潔でもメッセージから漏れる
3. 次のいずれかが差分かメッセージにあれば、コミットせずに止めて確認する
   - 自社・他社の固有名詞、製品名、リポジトリ名、個人名、認証情報
   - `owner/repo` 形式の参照、GitHub の URL、このリポジトリ以外の commit SHA・Issue 番号・PR 番号
   - 伏せ語として外部の設定に登録した語
4. 自動検査があっても、その結果を見ないままコミットしない。検査は書き忘れを拾うだけで、正しさは確認しない

push したものは取り消せないものとして扱う。force push しても PR の ref とキャッシュに残り、消すには GitHub Support への依頼が要る。書く前に止める。
...
- `api/src/api/` … HTTPと定期起動のruntimeのcomposition root。`scheduled/`は設定を検査し、所有contextの処理を呼び出す。`scheduled/jobs.ts`、`http/inbox/inbox-count-providers.ts`、`http/dashboard/dashboard-metric-providers.ts`、`http/dashboard/management/management-dashboard-metric-providers.ts`は`bun run gen:composition`が業務ごとの部品（`scheduled/run-*.ts`と各`providers/*.ts`）から生成する一覧で、手で編集しない。入口はこの一覧だけを参照し、業務を外すときは部品を消して再生成する。業務が無い構成でも応答の形は変えず、その業務の値を0にする。手書きmiddleware、route registry、生成app、複数contextを正本なしで集約するrouteだけを置く。横断処理は`audit/`や`company-employees/`のように責務を表す最小のディレクトリへ置き、`utils/`のような汎用bucketは作らない。Domainや業務実装は置かない
...
- APIテストは依存境界で分ける。業務判断はDBなしで検証し、Applicationはそのために Repository と Adapter を port として `Context` に受け取り、テストは型付きfakeを渡す（詳細は `api/src/contexts/CLAUDE.md`）。SQL・D1接続・HTTP入口のDB状態はCloudflare提供のローカルD1（Miniflare、本番そのものではない）で`*.d1.test.ts`として検証する（`bun run test:d1`）。D1契約と共有ハーネスは`api/tests/d1/`へ置き、Repositoryの検証は`createLocalD1Context`で独立DBと本番同形のContextを作る。DBは`startLocalD1({ migrated: [...] })`でtestごとの名前を宣言し、プロセスで一度だけ全migrationを適用したtemplateの複製を使う（DBを共有しない）。共通fixtureがtestごとにDBを作る場合は`startLocalD1Pool(数)`の`next()`で未使用のDBを受け取る。D1の`exec`は改行ごとに1文として扱うため、複数行のSQLやtriggerは`execSql`で流す。workerdはプロセスで1つを共有し、`api/bunfig.toml`のpreloadが全testの後に止める。DBへはMiniflareの同期proxyを使わず`local-d1-fetch-client.ts`のfetchで送る。bun:sqliteでD1 APIを模倣する互換ラッパー（`tests/api/support/d1-test-database.ts`ほか）は移行中のみ残し、機能を足さず新しい利用もしない。`lint:test-database-wrapper`が`api/test-database-wrapper-migration.json`の移行一覧にない依存と移行済みの残存項目を拒否する。migration専用のSQL検査は`migrated-sqlite-database.ts`を使う
...
これらの検査（gen:app:check、lint:route-authorization、lint:system-boundary、lint:table-ownership、verify-seed）は `bun run check`（api）に組み込んである。`lint:table-ownership` は全 migration 適用後の外部キーと production source の SQL 文字列を照合し、table への依存が `業務 -> company -> system` の一方向であることを検査する。業務と API composition から Company の table への SQL 参照も拒否し、Company の公開 operation と adapter を使わせる。外部キーによる参照整合性は許可する。外部キーのない列名だけの参照も、`<記録>_id` や `<記録>_code` が別の業務の table 名と一致すれば拒否する。業務間の関連は所有 context が解釈しない不透明な参照か、API composition の read model で表す。import 境界の検査は SQL と外部キーによる他 context への依存を検出できないため、業務の物理除去が他の業務と基盤を壊さないことをここで固定する。`lint:system-boundary` はシステム層（`src/domain/system` ほか）が、自動検出した下位 context の語彙・モジュールや混在 schema へ依存していないことを TypeScript AST で検査する。`verify-seed` は migration と `seeds/*.sql` を in-memory SQLite に適用し、schema 変更への seed の追従漏れを検出する。
