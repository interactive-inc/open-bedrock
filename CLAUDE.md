...
- `api/src/api/` … HTTPと定期起動のruntimeのcomposition root。`scheduled/`は設定を検査し、所有contextの処理を呼び出す。`scheduled/jobs.ts`と`http/inbox/inbox-count-providers.ts`は`bun run gen:composition`が業務ごとの部品（`scheduled/run-*.ts`、`http/inbox/providers/*.ts`）から生成する一覧で、手で編集しない。入口はこの一覧だけを参照し、業務を外すときは部品を消して再生成する。業務が無い構成でも応答の形は変えず、その業務の値を0にする。手書きmiddleware、route registry、生成app、複数contextを正本なしで集約するrouteだけを置く。横断処理は`audit/`や`company-employees/`のように責務を表す最小のディレクトリへ置き、`utils/`のような汎用bucketは作らない。Domainや業務実装は置かない
