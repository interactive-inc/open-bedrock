...
- APIテストは依存境界で分ける。業務判断はDBなしで検証し、SQL・D1接続・HTTP入口のDB状態はCloudflare提供のローカルD1（Miniflare、本番そのものではない）で`*.d1.test.ts`として検証する（`bun run test:d1`）。D1契約と共有ハーネスは`api/tests/d1/`へ置く。bun:sqliteでD1 APIを模倣する互換ラッパー（`tests/api/support/d1-test-database.ts`ほか）は移行中のみ残し、機能を足さず新しい利用もしない。`lint:test-database-wrapper`が`api/test-database-wrapper-migration.json`の移行一覧にない依存と移行済みの残存項目を拒否する。migration専用のSQL検査は`migrated-sqlite-database.ts`を使う
