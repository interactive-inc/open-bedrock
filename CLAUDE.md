...
これらの検査（gen:app:check、lint:route-authorization、lint:system-boundary、lint:table-ownership、verify-seed）は `bun run check`（api）に組み込んである。`lint:table-ownership` は全 migration 適用後の外部キーと production source の SQL 文字列を照合し、table への依存が `業務 -> company -> system` の一方向であることを検査する。業務から Company の table への SQL 参照も拒否し、Company の公開 operation を使わせる。外部キーによる参照整合性は許可する。import 境界の検査は SQL と外部キーによる他 context への依存を検出できないため、業務の物理除去が他の業務と基盤を壊さないことをここで固定する。`lint:system-boundary` はシステム層（`src/domain/system` ほか）が、自動検出した下位 context の語彙・モジュールや混在 schema へ依存していないことを TypeScript AST で検査する。`verify-seed` は migration と `seeds/*.sql` を in-memory SQLite に適用し、schema 変更への seed の追従漏れを検出する。
