...
- APIテストは依存境界で分ける。業務判断はDBなしで検証し、Applicationはそのために Repository と Adapter を port として `Context` に受け取り、テストは型付きfakeを渡す（詳細は `api/src/contexts/CLAUDE.md`）。SQL・D1接続・HTTP入口のDB状態はCloudflare提供のローカルD1（Miniflare、本番そのものではない）で`*.d1.test.ts`として検証する（`bun run test:d1`）。D1契約と共有ハーネスは`api/tests/d1/`へ置き、Repositoryの検証は`createLocalD1Context`で独立DBと本番同形のContextを作る。DBは`startLocalD1({ migrated: [...] })`でtestごとの名前を宣言し、プロセスで一度だけ全migrationを適用したtemplateの複製を使う（DBを共有しない）。bun:sqliteでD1 APIを模倣する互換ラッパー（`tests/api/support/d1-test-database.ts`ほか）は移行中のみ残し、機能を足さず新しい利用もしない。`lint:test-database-wrapper`が`api/test-database-wrapper-migration.json`の移行一覧にない依存と移行済みの残存項目を拒否する。migration専用のSQL検査は`migrated-sqlite-database.ts`を使う
