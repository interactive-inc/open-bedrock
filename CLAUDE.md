...
- `api/src/api/` … HTTPと定期起動のruntimeのcomposition root。`scheduled/`は設定を検査し、所有contextの処理を呼び出す。手書きmiddleware、route registry、生成app、複数contextを正本なしで集約するrouteだけを置く。横断処理は`audit/`や`company-employees/`のように責務を表す最小のディレクトリへ置き、`utils/`のような汎用bucketは作らない。Domainや業務実装は置かない
