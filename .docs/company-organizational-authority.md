...
同一時点に複数の上長関係がある場合は、全ての関係を循環判定に含める。登録順で上長を一人に絞らず、兼務先に閉路のない経路があっても他の経路の循環を許容しない。同一時点の判定は社員と関係の数に比例する処理で行い、分岐から生じる全経路を列挙しない。公開resourceの組織変更検証は変更後のheadを対象に、開始日を含み終了日を含まない期間を使い、将来の重複で生じる循環も拒否する。
