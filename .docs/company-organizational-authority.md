...
既存の未接続Employeeや所属期間を発令時に自動移行しない。公開Employeeの新しい所属には接続済みのOrgUnitが必要であり、上長を持つ所属とReportingRelationの統合、既存履歴の移行は未完成である。公開APIで雇用短縮と所属終了を組み合わせる保存順序も追加の確認が必要になる。
