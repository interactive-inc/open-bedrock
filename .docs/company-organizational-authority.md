...
既存の未接続Employeeや所属期間を発令時に自動移行しない。公開Employeeの新しい所属には接続済みのOrgUnitが必要であり、既存の上長履歴は[所属と上長履歴の接続](company-api.md#既存の所属と上長履歴の接続)で明示的に確認して移行する。未接続の所属に残る上長履歴の終了・再割当、既存責務の公開履歴への移行は未完成である。公開APIで雇用・所属・任用を同時に変更する場合の整合性は、[退職と組織責務](company-api.md#退職と組織責務)に従う。
...
未接続の既存責務をcode・保持者・期間の一致だけで公開履歴へ採用しない。公開APIで独立して登録した責務割当も、既存の期間台帳との同一性を推測しない。これらの履歴を確認して接続する操作は未完成である。Systemの委任と判断条件は引き続きSystemが所有する。
