...
未移行の所属に記録された上長関係も、元の期間IDと版を保持する。ただし、同じEmployee・OrgUnitで公開関係と重なる場合は`organizational_authority_reporting_source_conflict`を返す。同じ上長であっても同一の記録と推測しない。範囲が異なる関係は合わせて循環を検査する。公開Assignmentは公開履歴から業務の所属期間へ反映する。既存の上長履歴の移行と、人事発令を含む更新経路を一つの正本へそろえる作業は未完成である。
...
所属期間の対応は公開AssignmentのID・版と、期間台帳のID・版を保持する。公開履歴から反映した所属を既存の人事発令が変更するには、この対応と公開履歴も同じtransactionで更新する必要がある。期間台帳だけを変更するoperationは完了できない。既存履歴の移行、発令から公開所属への更新、雇用期間の変更と配属の変更を組み合わせる経路は未完成である。
