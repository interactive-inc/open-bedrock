...
既存の未接続Employeeや所属期間を発令時に自動移行しない。公開Employeeの新しい所属には接続済みのOrgUnitが必要であり、既存の上長履歴の確認と移行は未完成である。未接続の所属に残る上長履歴の終了・再割当、責務・Account対応の全writer統合、公開APIで雇用短縮と所属終了を組み合わせる保存順序も追加の確認が必要になる。
