...
担当者による作業の受領、成果の提出と人の完了確認は[作業の依頼・成果確認・責任の引き継ぎ](system-work-items.md)に定義する。
...
`GET /system/proposals/:number/versions/:version`は、公開番号と提案版を指定して本文の原文、digest、作成主体・時刻、置換元の提案ID、案件状態、判断段階と証言を返す。取消済みの版も保持し、存在しない版を最新版で補わない。本文とdigestの不一致では開示しない。

閲覧には`system:procedure:read`と、その版の申請者・判断者・代理元であることが必要となる。関係のない版の閲覧には、さらに`system:procedure:read:all`を明示して付与する。技術管理者の権限だけでは履歴の閲覧範囲を広げない。現在の判断候補に選ばれているだけでは、この履歴APIの閲覧資格にならない。

閲覧者の現在のAccount、資格、権限、認証の発行元・期限を確認する。申請者の現在の在籍状態を履歴の保持条件にせず、Companyや業務のコードを読み込みに必要としない。開示の前後で同じ閲覧資格と案件状態を要求し、開示監査の保存と一緒に検査する。参照中の変更、本文の不整合、監査保存の失敗は503として本文を返さない。資格不足は403、存在しない版は404となる。

procedure_definitionには提案が参照した手続き版の名称、分類、説明、入力定義と判断規則のJSON原文、完了操作キーを含める。手続きの現行版を変更しても、その内容で過去の定義を置き換えない。

このAPIは保存済みの提案と判断を参照する。業務固有の保存記録や添付本体を提案本文から推測して復元せず、それらの保全・移管は[業務の無効化と除去](company-foundation.md#業務の無効化と除去)に従う。
