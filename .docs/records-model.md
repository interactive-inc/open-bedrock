...
open-bedrock は、出来事、状態、観測、主張、評価、採用、判断、記録を別の概念として保持する。データベース行を現実そのものとして扱ってはならない。
...
- 記録の主キーは小文字の UUID とし、DB の CHECK と API の入力検査は `api/src/lib/validation/uuid.schema.ts` の同じ集合を受け入れる。新規の主キーは `crypto.randomUUID()` で採番する
- 業務コード、権限キー、手続きキーは主キーにせず、一意な属性として保持する
- 主キーを別の値へ置き換えた記録は、置き換える前の主キーを同じ行の `legacy_id` に保持する。`legacy_id` は一意であり、置き換え後に作る行では NULL とする。主キーと `legacy_id` の変更、および `legacy_id` を持つ行の追加は DB で拒否する
...
業務の実行コードを撤去する場合、保存対象の記録は所有業務を識別した保全先または移行先へ引き継ぐ。元ID、存在する元版、本文、添付とそのdigest、過去の承認が参照する対象を保持する。参照・出力には保全先の明示的な閲覧資格を適用し、技術管理者や元業務の削除から開示許可を推測しない。

元の版・記録日時と、保全物の版・取得日時は区別する。元記録に存在しない値は未提供として扱い、取得時点を過去の記録日時に置き換えない。復元できない過去版は生成せず、取得できた原記録と不足する来歴を区別する。

取得後に元記録が変更された場合、欠落がある場合、添付の完全性を確認できない場合は、保全済みとして撤去を確定しない。保全先で許可された主体による元記録の参照・出力と、無資格の主体への拒否を確認する。DB行の残存、監査digest、業務コードなしでの起動成功だけを保全完了と扱わない。
