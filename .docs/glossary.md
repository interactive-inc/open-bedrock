...
管理部門の業務を領域ごとに網羅し、項目ごとの実装方針(済・予定・汎用・しない・外部)を記した守備範囲の正。[[index|index]] の全体地図の節を参照。

### ロールと権限

認可の仕組み。permission("domain:action" 形式のキー)が認可の正で、ロールは permission を集めた集合として動的に作成できる。member・manager・hr・admin のシステムロールと、評価管理者・総務・情シス・監査のプリセットロールがある。[[roles-and-permissions|ロールと権限]] を参照。
