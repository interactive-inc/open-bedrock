...
公開Assignmentの変更は所属期間へ原子的に反映し、公開ReportingRelationは業務の管理範囲と判断候補へ接続する。[公開所属と期間台帳](company-organizational-authority.md#公開所属と期間台帳)に接続の制約を記載する。人事発令から公開所属への履歴更新、既存所属と上長の移行、責務・Account対応と既存編集画面の版照合は未完成である。
