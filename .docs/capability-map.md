...
- 状態: 実装済み
- 提供面: API、Web、CLI、Schema
- 現状: 従業員の検索、登録、変更、削除と、一般業務向けの限定項目ディレクトリを扱う
- 提供面の差: 従業員台帳の検索と管理は API、Web、CLI にあり、限定項目ディレクトリは API と Web だけにある
...
- 状態: 実装済み
- 提供面: API、Web、CLI、Schema
- 現状: テンプレート、タスク、従業員への割り当て、完了、完了解除、進行状況を扱う
- 不足または境界: 採用システムからの自動起動と外部サービスへのアカウント発行は行わない
...
- 提供面の差: 汎用申請とオンボーディングの各専用操作は三面にあるが、両者を横断するケースまたはタスクの入口はない
