...
規範性: 仕様正本。open-karte の目的、責務境界、AI に許可する操作と禁止する操作を定める。

open-karte は、会社で生じる事実、関係、依頼、判断、作業、証跡を同じ意味体系で管理する。人間と人工知能（AI）エージェントは、同じ業務規則と認可を通して社内事務を実行する。
...
open-karte は次を最終計算、最終判断、実行の正本として所有しない。
...
open-karte は、これらの入力事実、依頼、社内判断、承認済み指示、外部への引渡し、外部結果の主張、採否、期限、証拠を保持する。
...
製品仕様には、導入組織の事業計画、顧客、取引、組織固有の役職名、固有の金額基準、個人情報、認証情報を含めない。導入組織は、規程、権限割当、組織構造、外部接続設定として保持する。

サンプル値は未施行の例であり、導入組織が承認して施行するまで実行規範にしてはならない。
...
本仕様は製品境界を定める。機能の実装状態は [能力台帳](./capability-map.md)、コード、migration、テストを正とする。仕様に概念が存在することを、実装済みまたは技術的に強制済みという証拠にしてはならない。
