...
CompanyのAccount対応に関する制約を追加する前には、既存の公開対応と従業員履歴の接続、対応期間、同一性の競合も読み取り専用で検査する。DDLと同じSELECTを使い、未接続や不整合があれば適用を停止する。既存の雇用schemaが不足するDBも停止する。

この検査は適用済み SQL 本文の変更や会社データ全体の移行漏れを証明するものではない。履歴が一致しない既存環境は個別に移行計画を検証し、journal の書き換えで検査を通さない。
