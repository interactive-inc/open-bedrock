...
打刻の取得には現在の全社打刻閲覧権限、または同じ操作を許可するSystemの技術管理権限を要求する。本人・配下・部署だけの閲覧権限では全体収集を許可しない。発行元・期限・付与権限は取得と確定のDB条件で再照合する。この取得資格は保全の承認資格を意味しない。打刻の公開提出・承認・確定経路は未接続である。
