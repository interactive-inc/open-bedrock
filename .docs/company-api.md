...
`GET /company/resource-history/:type/:id`は`company:read`と会社scopeを確認し、一資源の有効・取消を含む全revisionを昇順に返す。各要素はresourceの全属性と、会社版、command、actor、理由、原資料参照、訂正元、記録時刻を含む。`limit`は1から100件、`after_revision`は最後に受け取った資源revisionを指定する。最初の応答の`throughRevision`を後続requestの`through_revision`に渡すと、取得中に別の変更が確定しても同じ会社版で履歴を読み切れる。存在しない未来の会社版は400で拒否する。
...
Employmentの`contractTerm`は、確定した雇用契約の期間を同じ雇用の版付き属性として保持する。省略またはnullは未記録を表す。FULL_TIME/PART_TIME、雇用主、在籍期間から有期・無期を推定しない。

- 有期契約は`kind: FIXED_TERM`、`startsOn`、`endsBefore`を持つ。開始日は期間に含み、終了境界日は含まない。終了境界日は開始日より後でなければならない。
- 無期契約は`kind: INDEFINITE`と`startsOn`を持ち、終了境界を持たない。未記録とは区別する。

個別の雇用保存と複数資源の組織変更は同じ属性を受理する。契約の更新・訂正は会社版へ追記し、有効日と会社版から当時の記録を取得する。契約期間と改訂の有効期間は別の情報であり、契約満了予定の記録だけで雇用を終了しない。

退職と退職日の訂正は既存の契約属性を保持する。再入社は別の雇用として扱い、以前の契約を自動で引き継がない。契約書本文や契約更新の判断手続きはこの属性に保存しない。

日付不足、存在しない日付、期間の逆転、区分にない属性は保存時に拒否する。DBへの直接挿入も同じ期間制約を満たす必要がある。既存の契約属性が不正な場合は移行を停止し、履歴を自動修正しない。
...
初期化が仮に作成した組織の期間は、確認なしで確定履歴へ移さない。未変更の初期組織に限り、initializationConfirmationに確認したstartsOnと一件以上のevidenceReferencesを指定できる。根拠はcontext、kind、id、versionで識別し、開始日は確認した会社営業日より後にしない。通常の組織履歴にはこの指定を許可しない。

初期期間の原記録を保全し、公開履歴では未採用の仮期間をvoidの初版、確認した期間をactiveの第2版として同じ会社版へ保存する。既存の期間台帳にも訂正版を追記し、元の行を書き換えない。仮期間がactiveだった会社版を公開履歴へ作らず、確認した開始日より前に組織が存在したと推定しない。元のsnapshot、確認主体・理由・根拠と訂正操作を保持する。

履歴の欠落、未完了の変更、既存の公開IDとの衝突、部分接続を拒否する。確認後の会社版・組織履歴の変更と営業日の変更は409になり、保存直前にも同じsnapshotを検査する。保存済みの同じ依頼は翌日以降も元の結果を返す。接続・公開履歴・会社版・訂正・移行記録の保存失敗では全体を取り消す。
...
CLIの`employees responsibility-adoption`も同じ確認・保存APIを使う。`--employee-id`は確認内容の参照、`--data`と`--idempotency-key`は確認済みJSONの送信を表す。競合時に会社版や確認内容を自動更新して再送しない。独立して登録された公開ResponsibilityAssignmentへ接続する場合は、mappingsの各期間にexistingResourceIdを明示する。確認対象には、その従業員の公開責務の全版・変更主体・理由・記録時点を含める。接続先が未接続で、所有者・責務定義・scope・委任条件と、過去・将来を含む全有効期間が一致する場合だけ接続する。隣接する期間の分割差は許すが、空白・重複・将来の終了の違いを現在値から推測して埋めない。

同じ既存IDへ複数の旧期間を接続できる。既存の公開IDと全改訂を保ち、接続を記録する同内容の次版と、各旧期間の対応を一つのtransactionで保存する。旧期間も訂正版を追記し、元の行を変更しない。接続先の推測・既に接続された割当の再統合は行わない。
...
旧等級・役職定義と旧等級付与台帳は撤去対象であり、移行用の確認・作成APIは提供しない。削除直前に、定義の元の全項目と移行時の公開改訂・主体・理由・会社版を照合し、付与の元の全項目が本人の保全原文に残っていることを検査する。不一致や欠落があれば最初の旧台帳の削除前に停止する。

`GET /company/definition-resource-adoptions/:commandId`は既定会社へのアクセス資格とCompany管理資格を持つ主体に、確認主体・理由・会社版・確認日・記録時点と元の定義を返す。証跡は旧定義テーブルに依存せず、更新と削除を拒否する。確認時の定義を過去の状態として扱わず、不明な職務との対応を推測しない。

旧等級付与の保全原記録では、付与ID・定義ID・確認時の順位がJavaScriptの安全な整数範囲内ならJSON数値、範囲外なら正確な十進文字列になる。SQLiteの符号付き64ビット整数を丸めず保持し、既に保存したJSON原文と照合digestは書き換えない。保全後は元の等級テーブルを参照せずに原記録を取得できる。
...
旧台帳の撤去は、保全原文や移行時の公開履歴の撤去を意味しない。未保全の記録がある環境では移行を停止し、不明な過去を推測したデータで不足を埋めない。
