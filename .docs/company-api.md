...
会社profileは初期化時から公開resourceを正本とする。接続前の会社名・代表者名は変更せず保全し、接続後は公開APIと既存の会社情報APIが同じ履歴を参照・変更する。初期化が作る所属は公開Assignmentと同じ期間対応を保持し、人事発令と公開APIから更新できる。責務と既存所属の全保存経路の統合は未完成である。Account対応は公開履歴へ接続し、同じ営業日で本人対応を参照する。
...
公開APIから雇用や所属を短縮する場合も、有効な任用・決裁資格を期間外へ残す変更を拒否する。将来の取消予約がある記録も、それ以前の有効期間を検査する。関連する任用を先に終了するか、同じ組織変更で終了させる。公開APIで雇用と所属の両方を短縮する場合は、所属の終了を先に確定する。人事発令の退職ではこれらを同じtransactionで確定する。
