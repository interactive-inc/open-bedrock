...
公開resourceに未接続の既存台帳、外部同期・招待からの登録、氏名変更の既存writer、組織・所属・責務・Account対応の保存先統合は未完成である。既存の入社・再入社発令で作る新しい契約は`FULL_TIME`を使い、公開APIと同じ契約区分入力への統一は未完成である。
...
外部従業員の新規登録は、System Account、Employee、Employment、初期人事記録、期間履歴を同じtransactionで保存する。初期記録には宣言された発効日・状態・理由と内容digestを残す。共有キーで認証する外部同期は操作Accountを持たず、機械主体の認証と公開resourceへの接続は未完成である。既存データの過去の状態を現在の雇用statusから補完する処理は持たない。
