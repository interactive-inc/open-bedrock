...
訂正発令の配送では、訂正元を遡って最も近い発令から生成した割当を探す。その割当が進行中なら`superseded`へ移し、訂正後の発令が`assigned`なら新しい割当とタスクを生成する。訂正後の発令が`obsolete`なら、元の割当を置換済みにして新しい割当は生成しない。置換は訂正後の発令が発効して配送されるときに行い、発効前は元の割当を変更しない。

完了済みの割当は置換せず完了のまま残し、訂正後の発令からは別の割当を生成する。置換済みの割当のタスクは置換時点の状態を保持し、完了済みのタスクを新しい割当へ引き継がない。

置換、新しい割当、受領結果、Jobの完了、置換と生成の監査は同じtransactionで確定する。置換の監査は`onboarding.lifecycle.assignment_superseded`であり、変更前後の状態、Job、訂正後と訂正元の発令を記録する。保存直前に元の割当が進行中でなくなった場合、または監査の保存に失敗した場合は全体を巻き戻して再試行する。訂正の連鎖は20件までとし、超える場合と訂正元を確認できない場合は失敗として再試行する。

置換済みの割当は、人事発令から生成した進行中の割当からだけ作れる。置換済みの割当とタスクの更新・削除、置換済みの割当への新しいタスクはDBでも拒否する。タスクの完了・取消は`409 assignment_superseded`、割当日の変更と取消は`409 not_modifiable`を返す。本人のタスク一覧には置換済みの割当のタスクを含めず、従業員別の割当一覧は置換済みの割当を履歴として返す。同じ従業員とテンプレートの重複判定は進行中の割当だけを対象とする。

## 割当と履歴

入社・再入社には`hire`、退職には`retired`へ紐付いたテンプレートを使う。テンプレートの種別はそれぞれ`join`、`leave`で、タスクは1件以上200件以下が必要である。保存前にテンプレートの対応とタスク内容を再照合する。

同じ発令からの割当は一度だけ生成する。割当、タスク、受領結果、Jobの完了、監査は同じtransactionで確定し、どこかで失敗した場合は一緒に巻き戻す。訂正元から生成した割当を除き、同じ従業員とテンプレートの進行中の割当がある場合は、新しい割当へ暗黙に統合せず失敗にする。
...
チェックリストの生成は、備品返却、権限の停止、実作業の完了を意味しない。
