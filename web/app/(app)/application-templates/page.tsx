  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader title="申請テンプレート">
        {canManage ? (
          <Button nativeButton={false} render={<Link href="/application-templates/new" />}>
            <Plus />
            テンプレートを作成
          </Button>
        ) : null}
      ...
      <p className="text-sm text-muted-foreground">
        申請の入力項目と承認手順を管理します。共通の手続き定義と、会社の承認資格を組み合わせます。
      </p>
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {templates.map((template) => (
        <Card key={template.code} className="gap-0">
          <Link
            href={`/application-templates/${template.code}`}
