...
import AuditEventDetailPage from "@/app/(app)/admin/audit-events/[eventId]/page"
...
describe("AuditEventDetailPage", () => {
  test("checks live read permission before resolving the event identifier", async () => {
    ...
    const value = Object.defineProperty({}, "eventId", {
      enumerable: true,
      get() {
        paramsRead = true
        return "evt-001"
    ...
    const params = Promise.resolve(value as { eventId: string })
  ...
  test("shows forensic fields only on detail with four initially collapsed JSON sections", async () => {
    ...
    render(await AuditEventDetailPage({ params: Promise.resolve({ eventId: "evt-001" }) }))
  ...
  test("warns when a legacy event may predate the current redactor", async () => {
    ...
    render(await AuditEventDetailPage({ params: Promise.resolve({ eventId: "legacy-001" }) }))
  ...
  test.each([403, 404])("hides event existence for API status %s", async (status) => {
    ...
    await expect(
      AuditEventDetailPage({ params: Promise.resolve({ eventId: "evt-001" }) }),
