...
import { buildAuditEventsHref } from "@/app/(app)/audit-events/_lib/build-audit-events-href"
import { parseAuditExportSearchParams } from "@/app/(app)/audit-events/_lib/parse-audit-export-search-params"
...
describe("buildAuditEventsHref", () => {
  ...
  test("removes only the cursor when returning to the first page", () => {
    ...
    ).toBe("/audit-events?action=custom.action&limit=50")
  })
})
