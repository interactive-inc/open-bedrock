import { RingiCreateForm } from "@/app/(app)/ringi/ringis/_components/ringi-create-form"
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader title={ringi.title}>
        <Link href="/ringi/ringis">稟議一覧へ</Link>
      </PageHeader>
      <div>
      ...
      {ringi.previous_ringi_id !== null ? (
        <Link href={`/ringi/ringis/${ringi.previous_ringi_id}`}>差戻し前の稟議を見る</Link>
      ) : null}
      {ringi.next_ringi_id !== null ? (
        <Link href={`/ringi/ringis/${ringi.next_ringi_id}`}>再提出された稟議を見る</Link>
      ) : null}
