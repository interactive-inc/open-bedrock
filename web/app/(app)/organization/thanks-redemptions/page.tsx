...
import { RedemptionAdminFilterForm } from "@/app/(app)/organization/thanks-redemptions/_components/redemption-admin-filter-form"
import { RedemptionAdminTable } from "@/app/(app)/organization/thanks-redemptions/_components/redemption-admin-table"
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "感謝", href: "/organization/thanks" }, { label: "交換申請管理" }]}
        actions={
          currentUser.permissions.includes("thanks_reward:manage") ? (
            <Button
              variant="outline"
              nativeButton={false}
              render={<Link href="/organization/rewards/manage" />}
            >
              景品の管理
            </Button>
          ) : null
...
async function RedemptionAdminSection(props: {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <TablePagination
        pathname="/organization/thanks-redemptions"
        total={result.total}
        limit={props.pageSize}
        offset={props.offset}
