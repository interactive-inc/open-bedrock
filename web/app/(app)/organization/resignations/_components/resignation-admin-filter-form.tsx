...
export function ResignationAdminFilterForm(props: Props) {
  ...
  return (
    <form method="get" action="/organization/resignations">
      <FieldSet>
        <FieldGroup className="flex-row flex-wrap items-end gap-4">
          <Field className="w-full sm:w-40">
            <FieldLabel htmlFor="resignation-admin-status">ステータス</FieldLabel>

            <select
              id="resignation-admin-status"
              name="status"
          ...
          <Field className="w-full sm:w-40">
            <FieldLabel htmlFor="resignation-admin-employee">従業員 ID</FieldLabel>

            <Input
              id="resignation-admin-employee"
              name="employee_id"
              type="text"
              inputMode="numeric"
