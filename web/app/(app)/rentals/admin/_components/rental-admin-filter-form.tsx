...
export function RentalAdminFilterForm(props: Props) {
  ...
  return (
    <form method="get" action="/rentals/admin">
      <FieldSet>
        <FieldGroup className="flex-row flex-wrap items-end gap-4">
          <Field className="w-full sm:w-40">
            <FieldLabel htmlFor="rental-admin-status">ステータス</FieldLabel>

            <select
              id="rental-admin-status"
              name="status"
          ...
          <Field className="w-full sm:w-40">
            <FieldLabel htmlFor="rental-admin-employee">従業員 ID</FieldLabel>

            <Input
              id="rental-admin-employee"
              name="employee_id"
              type="text"
              inputMode="numeric"
          ...
          <div className="flex items-end gap-2">
            ...
            {hasActiveFilter ? (
              <Button
                variant="outline"
                nativeButton={false}
                render={<Link href="/rentals/admin" />}
              >
                リセット
              </Button>
            ) : null}
