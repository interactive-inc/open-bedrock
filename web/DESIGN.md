...
Badge は行の中で Button や Input と並べない。Badge は高さが小さく、同じ行に Button があると高さが揃わない。テーブルの操作列や form の隣に置く押せないラベル（アカウントの状態、付与済みロールなど）は `StatusLabel` で表し、押せるもの（剥奪など）は `Button` の submit にする。
