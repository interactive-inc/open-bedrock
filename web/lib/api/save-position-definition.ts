...
/** 確認した会社版と再送キーを変更せず、役職の改訂または取消を保存する。 */
export async function savePositionDefinition(command: PositionDefinitionCommand) {
  ...
  if (response.status >= 400)
    return toResponseError(response, {
      fallback:
        "役職の改訂を保存できませんでした。確認した情報が変更されている場合は再読込してください",
    })
