...
describe("app workflow-repair commands", () => {
  ...
  test("accepts opaque candidate IDs but rejects invalid characters before calling the API", async () => {
    const response = await request("/application-requests/workflow-repair/reassign/42", {
      candidates: "employee:2,invalid!",
      reason: "restore approvers",
    })

    expect(response.status).toBe(400)
    expect(await response.text()).toContain("--candidates は従業員 ID")
  })
