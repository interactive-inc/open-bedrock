...
const directory = mkdtempSync(join(tmpdir(), "responsibility-adoption-cli-"))
beforeAll(() => {
    ...
    Object.assign(
      async (input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
        ...
        return Response.json(
          { code: status === 409 ? "responsibility_resource_adoption_conflict" : "PROFILE-001" },
          { status },
        )
      },
...
describe("CLI responsibility adoption", () => {
  test("履歴の参照を指定された従業員IDで行う", async () => {
    const captured = capture()
    try {
      expect(
        (await request("/employees/responsibility-adoption", { "employee-id": body.employeeId }))
          .status,
      ).toBe(200)
  ...
  test("確認済みJSONとキーをそのまま送り、競合を自動再送しない", async () => {
    ...
    try {
      expect(
        (
          await request("/employees/responsibility-adoption", {
            data: path,
            "idempotency-key": key,
          })
        ).status,
  ...
  test("確認参照のないJSONや冪等キーのない保存ではAPIを呼ばない", async () => {
    ...
    try {
      ...
      expect((await request("/employees/responsibility-adoption", { data: path })).status).toBe(400)
  ...
  test("helpから参照と保存の指定を確認できる", async () => {
    const response = await request("/employees/responsibility-adoption", { help: "1" })
