...
const directory = mkdtempSync(join(tmpdir(), "assignment-adoption-cli-"))
beforeAll(() => {
    ...
    Object.assign(
      async (input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
        ...
        return Response.json(
          { code: status === 409 ? "assignment_resource_adoption_conflict" : "PROFILE-001" },
          { status },
        )
      },
...
describe("CLI assignment adoption", () => {
  test("履歴の参照を指定された従業員IDで行う", async () => {
    const captured = capture()
    try {
      expect(
        (await request("/employees/assignment-adoption", { "employee-id": body.employeeId }))
          .status,
      ).toBe(200)
      expect(captured.requests).toHaveLength(1)
      expect(captured.requests[0]?.method).toBe("GET")
      expect(new URL(captured.requests[0]!.url).pathname).toBe(
        "/company/assignment-resource-adoptions",
      )
      expect(
  ...
  test("確認済みJSONとキーをそのまま送り、競合を自動再送しない", async () => {
    ...
    try {
      expect(
        (await request("/employees/assignment-adoption", { data: path, "idempotency-key": key }))
          .status,
      ).toBe(409)
      expect(captured.requests).toHaveLength(1)
  ...
  test("確認参照のないJSONや冪等キーのない保存ではAPIを呼ばない", async () => {
    ...
    try {
      ...
      expect((await request("/employees/assignment-adoption", { data: path })).status).toBe(400)
  ...
  test("helpから参照と保存の指定を確認できる", async () => {
    const response = await request("/employees/assignment-adoption", { help: "1" })
