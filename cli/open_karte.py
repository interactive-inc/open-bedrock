"""社内HR統合システム CLI (open-karte)。

設定ファイル: ~/.open-karte/config.json
"""
...
CONFIG_DIR = Path(os.path.expanduser("~/.open-karte"))
CONFIG_FILE = CONFIG_DIR / "config.json"
DEFAULT_BASE_URL = os.environ.get("OPEN_KARTE_API", "http://127.0.0.1:8000")
console = Console()
...
def features():
    ...
    _check(r)
    data = r.json()
    console.print(f"[bold]OPEN_KARTE_FEATURES=[/bold] {data.get('raw')}")
