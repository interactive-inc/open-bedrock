...
export const help = `karte skill set <code> --level <n> [--years <y>] [--note <n>]`
...
export default factory.createHandlers(
  ...
  async (c) => {
    ...
    const json = {
      ...
      years: query.years !== undefined ? Number(query.years) : undefined,
      note: query.note,
    }

    const client = await createClient()

    const response = await client.skills.me.$put({ json })
