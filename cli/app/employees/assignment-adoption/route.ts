...
export const help = `bedrock employees assignment-adoption --employee-id <id>
bedrock employees assignment-adoption --data <confirmed-history.json> --idempotency-key <uuid>
...
export default factory.createHandlers(
  ...
  async (c) => {
    ...
    if (input.data === undefined) {
      ...
      const response = await client.company["assignment-resource-adoptions"].$get({
    ...
    const client = await createClient()
    const response = await client.company["assignment-resource-adoptions"].$post({
